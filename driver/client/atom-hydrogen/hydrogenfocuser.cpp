...
HydrogenFocuser::HydrogenFocuser(const std::string &name) : Focuser(name) {
    ...
    m_switch_switch->registerCase(
        "Mode", [this](HYDROGEN::PropertyViewSwitch *svp) {
    ...
    m_switch_switch->registerCase(
        hydrogen_focuser_cmd + "FOCUS_MOTION",
        [this](HYDROGEN::PropertyViewSwitch *svp) {
            m_motion_prop.reset(svp);
            m_current_motion.store(
                (IUFindSwitch(svp, "FOCUS_INWARD")->s == ISS_ON) ? 0 : 1);
        });

    m_switch_switch->registerCase(
        hydrogen_focuser_cmd + "FOCUS_BACKLASH_TOGGLE",
        [this](HYDROGEN::PropertyViewSwitch *svp) {
            m_backlash_prop.reset(svp);
            has_backlash = (IUFindSwitch(svp, "HYDROGEN_ENABLED")->s == ISS_ON);
        });

    m_number_switch->registerCase(
        "FOCUS_ABSOLUTE_POSITION", [this](HYDROGEN::PropertyViewNumber *nvp) {
            ...
            if (num_value) {
                ...
                DLOG_F(INFO, "{} Current Absolute Position: {}", GetName(),
                       m_current_absolute_position.load());
            }
        });

    m_number_switch->registerCase(
        "FOCUS_SPEED", [this](HYDROGEN::PropertyViewNumber *nvp) {
            m_speed_prop.reset(nvp);
            INumber *num_value = IUFindNumber(nvp, "FOCUS_SPEED");
            if (num_value) {
                m_current_speed.store(num_value->value);
                DLOG_F(INFO, "{} Current Speed: {}", GetName(),
                       m_current_speed.load());
            }
        });

    m_number_switch->registerCase("ABS_FOCUS_POSITION",
                                  [this](HYDROGEN::PropertyViewNumber *nvp) {

                                  });

    m_number_switch->registerCase(
        "DELAY", [this](HYDROGEN::PropertyViewNumber *nvp) {
    ...
    m_number_switch->registerCase(
        "FOCUS_TEMPERATURE", [this](HYDROGEN::PropertyViewNumber *nvp) {
            ...
            if (num_value) {
                ...
                DLOG_F(INFO, "{} Current Temperature: {}", GetName(),
                       m_current_temperature.load());
            }
        });

    m_number_switch->registerCase(
        "FOCUS_MAX", [this](HYDROGEN::PropertyViewNumber *nvp) {
...
bool HydrogenFocuser::reconnect(const json &params) { return true; }

bool HydrogenFocuser::isConnected() { return true; }

bool HydrogenFocuser::moveTo(const json &params) { return true; }

bool HydrogenFocuser::moveToAbsolute(const json &params) { return true; }

bool HydrogenFocuser::moveStep(const json &params) { return true; }

bool HydrogenFocuser::moveStepAbsolute(const json &params) { return true; }

bool HydrogenFocuser::AbortMove(const json &params) { return true; }

int HydrogenFocuser::getMaxPosition(const json &params) { return 0; }

bool HydrogenFocuser::setMaxPosition(const json &params) { return true; }

bool HydrogenFocuser::isGetTemperatureAvailable(const json &params) {
    return true;
}

double HydrogenFocuser::getTemperature(const json &params) { return 0.0; }

bool HydrogenFocuser::isAbsoluteMoveAvailable(const json &params) {
    return true;
}

bool HydrogenFocuser::isManualMoveAvailable(const json &params) { return true; }

int HydrogenFocuser::getCurrentPosition(const json &params) { return 0; }

bool HydrogenFocuser::haveBacklash(const json &params) { return true; }

bool HydrogenFocuser::setBacklash(const json &params) { return true; }
