...
static char device_str[MAXHYDROGENDEVICE] = "Celestron GPS";
...
{
    strncpy(device_str, name, MAXHYDROGENDEVICE);
}
...
bool CelestronDriver::set_location(double longitude, double latitude)
{
    ...
    // Convert from HYDROGEN standard to regular east/west -180 to 180
    if (longitude > 180)
        longitude -= 360;
...
bool CelestronDriver::get_location(double *longitude, double *latitude)
{
    ...
    *longitude += response[6] / 3600.0;
    ...
    // convert longitude to HYDROGEN range 0 to 359.999
    if (*longitude < 0)
        *longitude += 360.0;
