...
namespace iEQ
{
    ...
    {
        // Windows 版本的实现，使用 WinAPI 进行串口通信
        ...
        if (cmd_len > 0)
        {
            if (!WriteFile(portHandle, cmd, cmd_len, &bytesWritten, NULL))
            {
                // 写入失败
                CloseHandle(portHandle);
                return false;
        ...
        {
            if (!WriteFile(portHandle, cmd, strlen(cmd), &bytesWritten, NULL))
            {
                // 写入失败
                CloseHandle(portHandle);
                return false;
        ...
        if (res_len > 0)
        {
            if (!ReadFile(portHandle, res, res_len, &bytesRead, NULL))
            {
                // 读取失败
                CloseHandle(portHandle);
                return false;
        ...
        {
            if (!ReadFile(portHandle, res, DRIVER_LEN, &bytesRead, NULL))
            {
                // 读取失败
                CloseHandle(portHandle);
                return false;
