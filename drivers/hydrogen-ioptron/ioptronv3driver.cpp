...
namespace IOPv3
{
    ...
    bool Driver::sendCommand(const char *command, int count, char *response, uint8_t timeout, uint8_t debugLog)
    {
...
#ifdef _WIN32
        HANDLE hPortFD = reinterpret_cast<HANDLE>(PortFD);
#endif
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            DWORD bytesWritten;
            if (!WriteFile(hPortFD, command, strlen(command), &bytesWritten, nullptr))
            {
            ...
            nbytes_written = static_cast<int>(bytesWritten);
#else
            ...
            // Windows implementation
            DWORD bytesRead;
            if (!ReadFile(hPortFD, res, count, &bytesRead, nullptr))
            {
                FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, nullptr, GetLastError(),
                               MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), errMsg, MAXRBUF, nullptr);
            ...
            nbytes_read = static_cast<int>(bytesRead);
#else
            // POSIX implementation
            if (count == -1)
