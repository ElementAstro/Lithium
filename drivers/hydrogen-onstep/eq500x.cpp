...
bool EQ500X::ReadScopeStatus()
{
    ...
    if (isSimulation())
    {
        ...
        if (nullptr != adjustment)
        {
            ...
            if (RAmDecrease)
                simEQ500X.MechanicalRA = std::fmod(simEQ500X.MechanicalRA - rates[adjustment - adjustments] * delta / 15.0 + 24.0, 24.0);
            if (RAmIncrease)
                simEQ500X.MechanicalRA = std::fmod(simEQ500X.MechanicalRA + rates[adjustment - adjustments] * delta / 15.0 + 24.0, 24.0);
            if (DECmDecrease)
                simEQ500X.MechanicalDEC -= rates[adjustment - adjustments] * delta;
            if (DECmIncrease)
                simEQ500X.MechanicalDEC += rates[adjustment - adjustments] * delta;
            ...
            LOGF_DEBUG("New mechanical RA/DEC simulated as %lf°/%lf° (%+lf°,%+lf°), stored as %lfh/%lf° = %s/%s",
                       simEQ500X.MechanicalRA * 15.0, simEQ500X.MechanicalDEC, (RAmDecrease || RAmIncrease) ? rates[adjustment - adjustments] * delta : 0, (DECmDecrease || DECmIncrease) ? rates[adjustment - adjustments] * delta : 0, p.RAm(), p.DECm(), simEQ500X.MechanicalRAStr,
    ...
    if (TrackState == SCOPE_SLEWING && !_gotoEngaged)
    {
        ...
        if (RA_GRANULARITY <= abs_ra_delta || DEC_GRANULARITY <= abs_dec_delta)
        {
            ...
            // Else adjust poll timeout to adjustment speed and continue
                ...
                setCurrentPollingPeriod(static_cast<uint32_t>(adjustment->polling_interval));
        }
        ...
        if (!sendCmd(CmdString))
            if (!getReply(buf, 2))
                if (buf[0] == '1' && buf[1] == '1')
                    ...
                    LOGF_ERROR("Failed '%s', mount replied %c%c", CmdString, buf[0], buf[1]);
            else
                LOGF_ERROR("Failed getting 2-byte reply to '%s'", CmdString);
        else
            LOGF_ERROR("Failed '%s'", CmdString);
...
bool EQ500X::MechanicalPoint::parseStringDEC(char const *buf, size_t buf_length)
{
    ...
    _DECm = sgn * (atol(DecValues->degrees) * 3600 +
