...
namespace PulsarTX
{
        ...
        void _resynchronize_(const int fd)
        {
            ...
            if (resynchronize_needed)
            {
                ...
                if (LX200Pulsar2::verboseLogging)
                    LOG_INFO("tty resynchronize failed");
            }
            else
            {
                DEBUGDEVICE(lx200Name, DBG_SCOPE, "RESYNC complete");
                if (LX200Pulsar2::verboseLogging)
                    LOG_INFO("tty resynchronize complete");
            }
        }
            ...
            for (attempt = 0; !done; ++attempt)
            {
                ...
                if (errcode != TTY_OK)
                {
                    ...
                    if (attempt == MaxAttempts - 1)
                    {
                        ...
                        if (LX200Pulsar2::verboseLogging)
                            LOGF_INFO("receive: resynchronize_needed flag set for cmd: %s, previous cmd was: %s", cmd,
                                      lastCmd);
                        return false;
                    }
                    else
            ...
            for (int attempt = 0; response == Termination; ++attempt)
            {
                ...
                if (errcode != TTY_OK)
                {
                    ...
                    if (attempt == MaxAttempts - 1)
                    {
                        ...
                        if (LX200Pulsar2::verboseLogging)
                            LOGF_INFO("confirmed: resynchronize_needed flag set for cmd: %s", cmd);
    ...
    bool sendReceiveInt(const int fd, const char *cmd, int *value)
    {
        ...
        if (!success)
        {
            ...
            if (LX200Pulsar2::verboseLogging)
                LOGF_INFO("sendReceiveInt() Failed cmd is: %s, response len is: %lu ", cmd, rlen);
        }
...
namespace Pulsar2Commands
{
    ...
    int getSpeedInd(const int fd, const char *cmd)
    {
        ...
        {
            int dec_dummy;
            if (sscanf(response, "%d,%d", &result, &dec_dummy) != 2)
                result = 0;
        }
        return result;
    }
    ...
    bool getUserRate(const int fd, int usr_ind, double *ur_ra, double *ur_dec)
    {
        ...
        response[29] = LX200Pulsar2::Null;
        if (usr_ind < 1 || usr_ind > 3)
            return false; // paranoid, early exit
        char cmd[] = "#:YGZ_#";
        cmd[5] = usr_ind + '0';
    ...
    bool getSideOfPier(const int fd, OTASideOfPier *ota_side_of_pier)
    {
        *ota_side_of_pier = Pulsar2Commands::OTASideOfPier::EastOfPier; // effectively a fail-safe default
        int ival;
        if (!PulsarTX::sendReceiveInt(fd, "#:YGN#", &ival))
            return false;
        if (ival == 1)
            *ota_side_of_pier = Pulsar2Commands::OTASideOfPier::WestOfPier;
        return true;
    }
    ...
    bool unpark(const int fd)
    {
        ...
        if (!PulsarTX::sendReceiveInt(fd, "#:YL#", &result))
        {
            // retry
            if (LX200Pulsar2::verboseLogging)
                LOG_INFO("Unpark retry compensating for failed unpark return value...");
            if (!PulsarTX::sendReceiveInt(fd, "#:YL#", &result))
                result = 0;
        }
        return (result == 1);
            ...
            if (this->initialization_complete)
            {
                ...
                if (Pulsar2Commands::check_ota_side_of_pier)
                {
                    ...
                    else
                    {
                        ...
                        if (LX200Pulsar2::verboseLogging)
                            LOG_INFO("Could not read OTA side of pier from controller");
                    }
                } // side of pier check
            }     // init complete
        }         // not a simulation
    ...
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (strcmp(name, PeriodicErrorCorrectionSP.name) == 0)
        {
            ...
            if (!isSimulation())
            {
                ...
                const bool success = Pulsar2Commands::setPECorrection(PortFD,
                                                                      (PeriodicErrorCorrectionS[1].s == ISS_ON ? Pulsar2Commands::PECorrectionOn : Pulsar2Commands::PECorrectionOff),
        ...
        if (strcmp(name, RefractionCorrectionSP.name) == 0)
        {
            ...
            if (!isSimulation())
            {
                ...
                const Pulsar2Commands::RCorrection rc =
                    (RefractionCorrectionS[1].s == ISS_ON ? Pulsar2Commands::RCorrectionOn : Pulsar2Commands::RCorrectionOff);
        ...
        if (strcmp(name, TrackingRateIndSP.name) == 0)
        {
            ...
            if (!isSimulation())
            {
                ...
                {
                    if (TrackingRateIndS[idx].s == ISS_ON)
                        break;
                }
        ...
        if (Pulsar2Commands::getVersion(PortFD, versionResponse))
        {
            ...
            if (*vp != LX200Pulsar2::Null)
            {
                ...
                for (int i = 0; i < 19 && (isalnum(*vp) || *vp == '.'); *(vs++) = *(vp++), i++)
                    ;
                if (strcmp(versionString, "5.7") < 0)
                    Pulsar2Commands::speedsExtended = true;
...
bool LX200Pulsar2::storeScopeLocation()
{
    ...
    if (isSimulation() || Pulsar2Commands::getSiteLatitudeLongitude(PortFD, &lat, &lon))
    {
        ...
        if (LX200Pulsar2::verboseLogging)
            LOGF_INFO("Controller location read and stored; lat: %+f, lon: %+f", lat, stdLon);
    }
    else
    {
