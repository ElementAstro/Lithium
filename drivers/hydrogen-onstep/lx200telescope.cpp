...
bool parseDateTime(const char *datetime, struct tm *tm)
{
    const char *format = "%Y-%m-%dT%H:%M:%S";
    memset(tm, 0, sizeof(struct tm));

#ifdef _WIN32
    // Windows 下没有 strptime 函数，使用 sscanf 进行替代
    if (sscanf(datetime, format, &tm->tm_year, &tm->tm_mon, &tm->tm_mday, &tm->tm_hour, &tm->tm_min, &tm->tm_sec) != 6)
        return false;
#else
    // 非 Windows 平台可以直接使用 strptime 函数
    if (strptime(datetime, format, tm) == nullptr)
        return false;
#endif

    tm->tm_year -= 1900; // 年份应该是从1900年开始计数
    tm->tm_mon -= 1;     // 月份应该是从0开始计数
    tm->tm_isdst = -1;   // 让系统确定是否使用夏令时
...
#ifdef _WIN32
    localtime_s(&utm, &time_epoch); // 在 Windows 下使用 localtime_s
#else
    localtime_r(&time_epoch, &utm); // 在非 Windows 平台使用 localtime_r
#endif
