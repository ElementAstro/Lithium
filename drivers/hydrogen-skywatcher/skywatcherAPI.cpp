...
bool SkywatcherAPI::GetHighSpeedRatio(AXISID Axis)
{
    ...
    if (highSpeedRatio == 0)
    {
        MYDEBUG(HYDROGEN::Logger::DBG_ERROR, "Invalid highspeed ratio value from mount. Cycle power and reconnect again.");
        return false;
    }
...
bool SkywatcherAPI::GetMicrostepsPerRevolution(AXISID Axis)
{
    ...
    {
        MYDEBUG(HYDROGEN::Logger::DBG_ERROR, "Invalid microstep value from mount. Cycle power and reconnect again.");
        return false;
    }
...
bool SkywatcherAPI::GetMicrostepsPerWormRevolution(AXISID Axis)
{
    ...
    if (value == 0)
        MYDEBUGF(HYDROGEN::Logger::DBG_WARNING, "Zero Microsteps per worm revolution for Axis %d. Possible corrupted data.", Axis);
...
bool SkywatcherAPI::GetMotorBoardVersion(AXISID Axis)
{
    ...
    MYDEBUGF(HYDROGEN::Logger::DBG_DEBUG, "Motor Board Version: %#X", MCVersion);
...
bool SkywatcherAPI::GetStepperClockFrequency(AXISID Axis)
{
    ...
    if (value == 0)
    {
        MYDEBUG(HYDROGEN::Logger::DBG_ERROR,
...
bool SkywatcherAPI::GetStatus(AXISID Axis)
{
    ...
    else
    {
        ...
        if (AxesStatus[(int)Axis].SlewingTo)
        {
            ...
            GetEncoder(Axis);
            //            MYDEBUGF(HYDROGEN::Logger::DBG_SESSION,
...
bool SkywatcherAPI::InquireFeatures()
{
    ...
    {
        MYDEBUGF(HYDROGEN::Logger::DBG_WARNING, "%s(): Found different features for RA (%d) and DEC (%d)", __FUNCTION__,
                 rafeatures, defeatures);
    }
    if (rafeatures & 0x00000010)
    {
        MYDEBUGF(HYDROGEN::Logger::DBG_WARNING, "%s(): Found RA PPEC training on", __FUNCTION__);
    }
    if (defeatures & 0x00000010)
    {
        MYDEBUGF(HYDROGEN::Logger::DBG_WARNING, "%s(): Found DE PPEC training on", __FUNCTION__);
    }
...
void SkywatcherAPI::SlewTo(AXISID Axis, long OffsetInMicrosteps, bool verbose)
{
    ...
    if (verbose)
    {
        MYDEBUGF(HYDROGEN::Logger::DBG_DEBUG, "SlewTo Axis %d Offset %ld CurrentEncoder %ld SlewToTarget %ld", Axis,
...
bool SkywatcherAPI::TalkWithAxis(AXISID Axis, SkywatcherCommand Command, std::string &cmdDataStr, std::string &responseStr)
{
    ...
    for (int retries = 0; retries < SKYWATCHER_MAX_RETRTY; retries++)
    {
        ...
        if ( (errorCode = tty_write_string(MyPortFD, command, &bytesWritten)) != TTY_OK)
        {
            if (retries == SKYWATCHER_MAX_RETRTY - 1)
            {
                ...
                MYDEBUGF(HYDROGEN::Logger::DBG_ERROR, "Communication error: %s", errorMessage);
                return false;
            }
            else
            {
    ...
    if (response[0] == '!')
    {
        ...
        if (errorCodes.count(code) > 0)
            MYDEBUGF(HYDROGEN::Logger::DBG_ERROR, "Mount error: %s", errorCodes.at(code).c_str());
        return false;
    }
