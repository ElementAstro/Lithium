...
#define HYDROGEN_DEBUG_LOGGING
#ifdef HYDROGEN_DEBUG_LOGGING
#include "hydrogentelescope.h"
#define MYDEBUG(priority, msg) \
    HYDROGEN::Logger::getInstance().print(pChildTelescope->getDeviceName(), priority, __FILE__, __LINE__, msg)
#define MYDEBUGF(priority, msg, ...) \
    HYDROGEN::Logger::getInstance().print(pChildTelescope->getDeviceName(), priority, __FILE__, __LINE__, msg, __VA_ARGS__)
#else
#define MYDEBUG(priority, msg)
#define MYDEBUGF(priority, msg, ...)
#endif
...
class SkywatcherAPI
{
    public:
        ...
        enum SkywatcherCommand
        {
            ...
            ActivateMotor             = 'B', // See eq6direct implementation http://pierre.nerzic.free.fr/HYDROGEN/
...
#ifdef HYDROGEN_DEBUG_LOGGING
    public:
        HYDROGEN::Telescope *pChildTelescope { nullptr };
#endif
};
