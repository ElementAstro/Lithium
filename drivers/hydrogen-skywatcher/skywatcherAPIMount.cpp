...
using namespace HYDROGEN::AlignmentSubsystem;
...
bool SkywatcherAPIMount::initProperties()
{
    ...
    strncpy(SlewRateSP.sp[SlewRateSP.nsp - 1].name, "SLEW_MAX", MAXHYDROGENNAME);
    ...
    // AUX Encoders
    AUXEncoderSP[HYDROGEN_ENABLED].fill("HYDROGEN_ENABLED", "Enabled", ISS_ON);
    AUXEncoderSP[HYDROGEN_DISABLED].fill("HYDROGEN_DISABLED", "Disabled", ISS_OFF);
    AUXEncoderSP.fill(getDeviceName(), "AUX_ENCODERS", "AUX Encoders", TRACKING_TAB, IP_RW, ISR_1OFMANY, 60, IPS_IDLE);

    // Snap port
    SnapPortSP[HYDROGEN_ENABLED].fill("HYDROGEN_ENABLED", "On", ISS_OFF);
    SnapPortSP[HYDROGEN_DISABLED].fill("HYDROGEN_DISABLED", "Off", ISS_ON);
...
bool SkywatcherAPIMount::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (AUXEncoderSP.isNameMatch(name))
        {
            ...
            AUXEncoderSP.apply();
            auto enabled = AUXEncoderSP.findOnSwitchIndex() == HYDROGEN_ENABLED;
        ...
        if (SnapPortSP.isNameMatch(name))
        {
            ...
            auto enabled = SnapPortSP.findOnSwitchIndex() == HYDROGEN_ENABLED;
...
bool SkywatcherAPIMount::Goto(double ra, double dec)
{
    if (m_IterativeGOTOPending)
    {
        ...
        DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "Iterative GOTO RA %lf DEC %lf (Current Sky RA %s DE %s)", ra, dec, RAStr,
               DecStr);
    }
    else
    {
        ...
        DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "GOTO RA %lf DEC %lf", ra, dec);
    ...
    HYDROGEN::IHorizontalCoordinates AltAz { 0, 0 };
    ...
    {
        HYDROGEN::IEquatorialCoordinates EquatorialCoordinates { 0, 0 };
        AltitudeAzimuthFromTelescopeDirectionVector(TDV, AltAz);
        HYDROGEN::HorizontalToEquatorial(&AltAz, &m_Location, ln_get_julian_from_sys(), &EquatorialCoordinates);

        char RAStr[32], DecStr[32];
        fs_sexa(RAStr, EquatorialCoordinates.rightascension, 2, 3600);
        fs_sexa(DecStr, EquatorialCoordinates.declination, 2, 3600);

        DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "Sky -> Mount RA %s DE %s (TDV x %lf y %lf z %lf)", RAStr, DecStr, TDV.x,
               TDV.y, TDV.z);
    }
    else
    {
        ...
        HYDROGEN::EquatorialToHorizontal(&EquatorialCoordinates, &m_Location, ln_get_julian_from_sys(), &AltAz);
    ...
    DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT,
    ...
    DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "Current Axis1 %ld microsteps (Zero %ld) Axis2 %ld microsteps (Zero %ld)",
           CurrentEncoders[AXIS1], ZeroPositionEncoders[AXIS1], CurrentEncoders[AXIS2], ZeroPositionEncoders[AXIS2]);
    DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "Azimuth offset %ld microsteps | Altitude offset %ld microsteps",
...
///
//////////////////////////////////////////////////////////////////////////////////////////////////
bool SkywatcherAPIMount::MoveNS(HYDROGEN_DIR_NS dir, TelescopeMotionCommand command)
{
    double speed =
...
///
//////////////////////////////////////////////////////////////////////////////////////////////////
bool SkywatcherAPIMount::MoveWE(HYDROGEN_DIR_WE dir, TelescopeMotionCommand command)
{
    double speed =
...
bool SkywatcherAPIMount::ReadScopeStatus()
{
    ...
    DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "Axis1 encoder %ld (Zero %ld) -> AZ %lf°",
           CurrentEncoders[AXIS1], ZeroPositionEncoders[AXIS1], AltAz.azimuth);
    DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "Axis2 encoder %ld (Zero %ld) -> AL %lf°",
    ...
    fs_sexa(RAStr, m_SkyCurrentRADE.rightascension, 2, 3600);
    fs_sexa(DecStr, m_SkyCurrentRADE.declination, 2, 3600);
    DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "Sky RA %s DE %s", RAStr, DecStr);
...
///
//////////////////////////////////////////////////////////////////////////////////////////////////
bool SkywatcherAPIMount::getCurrentAltAz(HYDROGEN::IHorizontalCoordinates &altaz)
{
    // Update Axis Position
...
///
//////////////////////////////////////////////////////////////////////////////////////////////////
bool SkywatcherAPIMount::getCurrentRADE(HYDROGEN::IHorizontalCoordinates altaz, HYDROGEN::IEquatorialCoordinates &rade)
{
    TelescopeDirectionVector TDV = TelescopeDirectionVectorFromAltitudeAzimuth(altaz);
    DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "TDV x %lf y %lf z %lf", TDV.x, TDV.y, TDV.z);
    ...
    if (!TransformTelescopeToCelestial(TDV, RightAscension, Declination))
    {
        ...
        HYDROGEN::IEquatorialCoordinates EquatorialCoordinates;
        HYDROGEN::HorizontalToEquatorial(&altaz, &m_Location, ln_get_julian_from_sys(), &EquatorialCoordinates);
...
bool SkywatcherAPIMount::saveConfigItems(FILE *fp)
{
    ...
    return HYDROGEN::Telescope::saveConfigItems(fp);
}
...
{
    DEBUG(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "SkywatcherAPIMount::Sync");
    ...
    DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "Axis1 encoder %ld initial %ld AZ %lf°",
           CurrentEncoders[AXIS1], ZeroPositionEncoders[AXIS1], AltAz.azimuth);
    DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "Axis2 encoder %ld initial %ld AL %lf°",
           CurrentEncoders[AXIS2], ZeroPositionEncoders[AXIS2], AltAz.altitude);
    ...
    DEBUGF(HYDROGEN::AlignmentSubsystem::DBG_ALIGNMENT, "New sync point Date %lf RA %lf DEC %lf TDV(x %lf y %lf z %lf)",
    ...
    switch (TrackState)
    {
        ...
        case SCOPE_TRACKING:
        {
            ...
            else
            {
                ...
                {
                    HYDROGEN::IEquatorialCoordinates EquatorialCoordinates { ra, de };
                    HYDROGEN::EquatorialToHorizontal(&EquatorialCoordinates, &m_Location, ln_get_julian_from_sys() + JDOffset, &AltAz);
                }
...
{
    HYDROGEN::IHorizontalCoordinates OldAltAz { 0, 0 };
    HYDROGEN::IHorizontalCoordinates NewAltAz { 0, 0 };
