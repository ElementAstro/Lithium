...
bool SynscanDriver::ReadScopeStatus()
{
    ...
    if (m_isAltAz == false && sendCommand("p", res))
    {
        ...
        // HYDROGEN and mount pier sides are opposite to each other
    ...
    char Axis1Coords[MAXHYDROGENNAME] = {0}, Axis2Coords[MAXHYDROGENNAME] = {0};
    ...
    memset(Axis1Coords, 0, MAXHYDROGENNAME);
    memset(Axis2Coords, 0, MAXHYDROGENNAME);
...
bool SynscanDriver::Goto(double ra, double dec)
{
    ...
    // HYDROGEN is JNow. Synscan Controll uses J2000 Epoch
    HYDROGEN::IEquatorialCoordinates epochPos { 0, 0 }, J2000Pos { 0, 0 };
    ...
    {
        HYDROGEN::IHorizontalCoordinates altaz;
        HYDROGEN::EquatorialToHorizontal(&epochPos, &m_Location, ln_get_julian_from_sys(), &altaz);
...
bool SynscanDriver::MoveNS(HYDROGEN_DIR_NS dir, TelescopeMotionCommand command)
{
    if (isSimulation())
        return true;
...
bool SynscanDriver::MoveWE(HYDROGEN_DIR_WE dir, TelescopeMotionCommand command)
{
    if (isSimulation())
        return true;
...
bool SynscanDriver::sendTime()
{
    ...
    if (isSimulation())
    {
        char timeString[MAXHYDROGENNAME] = {0};
        time_t now = time (nullptr);
        strftime(timeString, MAXHYDROGENNAME, "%T", gmtime(&now));
...
{
    HYDROGEN_DIR_NS direction = static_cast<HYDROGEN_DIR_NS>(IUFindOnSwitchIndex(&MovementNSSP));
...
{
    HYDROGEN_DIR_WE direction = static_cast<HYDROGEN_DIR_WE>(IUFindOnSwitchIndex(&MovementWESP));
