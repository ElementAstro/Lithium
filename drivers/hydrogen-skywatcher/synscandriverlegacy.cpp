...
bool SynscanLegacyDriver::Connect()
{
    ...
    bool rc = HYDROGEN::Telescope::Connect();
...
bool SynscanLegacyDriver::ReadScopeStatus()
{
    ...
    */
    ...
    {
        HYDROGEN::IHorizontalCoordinates CurrentAltAz { 0, 0 };
        double DiffAlt { 0 };
        double DiffAz { 0 };

        HYDROGEN::EquatorialToHorizontal(&epochPos, &m_Location, ln_get_julian_from_sys(), &CurrentAltAz);
...
bool SynscanLegacyDriver::Goto(double ra, double dec)
{
    char res[MAX_SYN_BUF] = {0};
    int bytesWritten, bytesRead;
    HYDROGEN::IHorizontalCoordinates TargetAltAz { 0, 0 };
    ...
    HYDROGEN::IEquatorialCoordinates epochPos { ra, dec };
    HYDROGEN::EquatorialToHorizontal(&epochPos, &m_Location, ln_get_julian_from_sys(), &TargetAltAz);
...
bool SynscanLegacyDriver::MoveNS(HYDROGEN_DIR_NS dir, TelescopeMotionCommand command)
{
    if (isSimulation())
        return true;
...
bool SynscanLegacyDriver::MoveWE(HYDROGEN_DIR_WE dir, TelescopeMotionCommand command)
{
    if (isSimulation())
        return true;
