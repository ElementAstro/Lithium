...
// http://hydrogenlib.org/images/juli_tommy.jpg
std::unique_ptr<WatchDog> juli = std::make_unique<WatchDog>();
...
bool WatchDog::initProperties()
{
    ...
    // HYDROGEN Server Settings
    IUFillText(&SettingsT[HYDROGENSERVER_HOST], "HYDROGENSERVER_HOST", "hydrogenserver host", "localhost");
    IUFillText(&SettingsT[HYDROGENSERVER_PORT], "HYDROGENSERVER_PORT", "hydrogenserver port", "7624");
...
bool WatchDog::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        if (!strcmp(SettingsTP.name, name))
        {
            ...
            try
            {
                m_HYDROGENServerPort = std::stoi(SettingsT[HYDROGENSERVER_PORT].text);
            }
            catch(...)
            {
...
bool WatchDog::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
{
    if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
    {
        ...
        // Heart Beat
        // Client must set this property to hydrogencate it is alive.
...
void WatchDog::TimerHit()
{
    ...
    switch (m_ShutdownStage)
    {
        ...
        case WATCHDOG_IDLE:
            ...
            // Set hydrogenserver host and port
            m_WatchDogClientInstance->setServer(SettingsT[HYDROGENSERVER_HOST].text, m_HYDROGENServerPort);

            LOG_DEBUG("Connecting to HYDROGEN server...");
        ...
        case WATCHDOG_CLIENT_STARTED:
            ...
            {
                LOGF_DEBUG("Connected to HYDROGEN server %s @ %s", SettingsT[0].text,
                           SettingsT[1].text);
                ...
                LOG_DEBUG("Waiting for HYDROGEN server connection...");
            break;
