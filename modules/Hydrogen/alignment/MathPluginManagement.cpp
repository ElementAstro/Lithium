...
void MathPluginManagement::ProcessTextProperties(Telescope *pTelescope, const char *name, char *texts[], char *names[],
        int n)
{
    //DEBUGFDEVICE(pTelescope->getDeviceName(), HYDROGEN::Logger::DBG_DEBUG, "ProcessTextProperties - name(%s)", name);
        ...
        if (0 != strcmp(AlignmentSubsystemMathPlugins.get()[0].label, AlignmentSubsystemCurrentMathPlugin.text))
        {
            ...
            if (nullptr != LoadedMathPluginHandle)
            {
                ...
                if (nullptr != Destroy)
                {
                    ...
                    {
                        DEBUGFDEVICE(pTelescope->getDeviceName(), HYDROGEN::Logger::DBG_ERROR,
            ...
            if (nullptr != (LoadedMathPluginHandle = dlopen(AlignmentSubsystemCurrentMathPlugin.text, RTLD_NOW)))
            {
                ...
                if (nullptr != Create)
                {
                    ...
                    {
                        DEBUGFDEVICE(pTelescope->getDeviceName(), HYDROGEN::Logger::DBG_WARNING,
                                     "MathPluginManagement - cannot find %s in list of plugins", MathPluginFiles[i].c_str());
                    }
                }
                else
                {
                    DEBUGFDEVICE(pTelescope->getDeviceName(), HYDROGEN::Logger::DBG_ERROR, "MathPluginManagement - cannot get Create function - %s",
                                 dlerror());
                }
            }
            else
            {
                DEBUGFDEVICE(pTelescope->getDeviceName(), HYDROGEN::Logger::DBG_ERROR, "MathPluginManagement - cannot load plugin %s error %s",
...
void MathPluginManagement::ProcessSwitchProperties(Telescope *pTelescope, const char *name, ISState *states,
        char *names[], int n)
{
    //DEBUGFDEVICE(pTelescope->getDeviceName(), HYDROGEN::Logger::DBG_DEBUG, "ProcessSwitchProperties - name(%s)", name);
    HYDROGEN_UNUSED(pTelescope);
...
void MathPluginManagement::EnumeratePlugins()
{
    ...
    const char *hydrogenprefix = getenv("HYDROGENPREFIX");
    if (hydrogenprefix)
        snprintf(MATH_PLUGINS_DIRECTORY, 2048 - 1, "%s/Contents/Resources/MathPlugins", hydrogenprefix);
    else
        snprintf(MATH_PLUGINS_DIRECTORY, 2048 - 1, HYDROGEN_MATH_PLUGINS_DIRECTORY);
#else
    snprintf(MATH_PLUGINS_DIRECTORY, 2048 - 1, HYDROGEN_MATH_PLUGINS_DIRECTORY);
#endif
