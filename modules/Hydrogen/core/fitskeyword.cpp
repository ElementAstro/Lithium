...
namespace HYDROGEN
{
    ...
    FITSRecord::FITSRecord() : val_int64(0),
                               m_type(VOID)
    {
    }

    FITSRecord::FITSRecord(const char *key, const char *value, const char *comment) : m_key(key),
                                                                                      m_type(STRING)
    {
        if (value)
    ...
    FITSRecord::FITSRecord(const char *key, int64_t value, const char *comment) : val_int64(value),
    ...
    FITSRecord::FITSRecord(const char *key, double value, int decimal, const char *comment) : val_double(value),
                                                                                              m_key(key),
                                                                                              m_type(DOUBLE),
                                                                                              m_decimal(decimal)
    {
    ...
    FITSRecord::FITSRecord(const char *comment) : m_key("COMMENT"),
                                                  m_type(COMMENT)
    {
        if (comment)
            m_comment = std::string(comment);
