...
namespace HYDROGEN
{
    ...
    void RotatorInterface::initProperties(const char *groupName)
    {
        ...
        // Reverse Direction
        IUFillSwitch(&ReverseRotatorS[DefaultDevice::HYDROGEN_ENABLED], "HYDROGEN_ENABLED", "Enabled", ISS_OFF);
        IUFillSwitch(&ReverseRotatorS[DefaultDevice::HYDROGEN_DISABLED], "HYDROGEN_DISABLED", "Disabled", ISS_ON);
        ...
        IUFillSwitch(&RotatorBacklashS[DefaultDevice::HYDROGEN_ENABLED], "HYDROGEN_ENABLED", "Enabled", ISS_OFF);
        IUFillSwitch(&RotatorBacklashS[DefaultDevice::HYDROGEN_DISABLED], "HYDROGEN_DISABLED", "Disabled", ISS_ON);
    ...
    bool RotatorInterface::processNumber(const char *dev, const char *name, double values[], char *names[], int n)
    {
        ...
        if (dev != nullptr && strcmp(dev, m_defaultDevice->getDeviceName()) == 0)
        {
            ...
            {
                if (RotatorBacklashS[DefaultDevice::HYDROGEN_ENABLED].s != ISS_ON)
                {
    ...
    bool RotatorInterface::processSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
    {
        ...
        if (dev != nullptr && strcmp(dev, m_defaultDevice->getDeviceName()) == 0)
        {
            ...
            else if (strcmp(name, ReverseRotatorSP.name) == 0)
            {
                ...
                const bool enabled = IUFindOnSwitchIndex(&ReverseRotatorSP) == DefaultDevice::HYDROGEN_ENABLED;
            ...
            else if (strcmp(name, RotatorBacklashSP.name) == 0)
            {
                ...
                const bool enabled = IUFindOnSwitchIndex(&RotatorBacklashSP) == DefaultDevice::HYDROGEN_ENABLED;
