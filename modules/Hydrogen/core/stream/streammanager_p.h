...
namespace HYDROGEN
{
    ...
    public:
        struct FrameInfo
        {
            ...
            FrameInfo()
                : x(0), y(0), w(0), h(0), bytesPerColor(0)
            {
            }

            explicit FrameInfo(const CCDChip &ccdChip, size_t bytesPerColor = 1)
                : x(ccdChip.getSubX() / ccdChip.getBinX()), y(ccdChip.getSubY() / ccdChip.getBinY()), w(ccdChip.getSubW() / ccdChip.getBinX()), h(ccdChip.getSubH() / ccdChip.getBinY()), bytesPerColor(bytesPerColor)
            {
            }

            explicit FrameInfo(const SensorInterface &sensorInterface, size_t bytesPerColor = 1)
                : x(0), y(0), w(sensorInterface.getBufferSize() * 8 / sensorInterface.getBPS()), h(1), bytesPerColor(bytesPerColor)
            {
            }
    ...
    public:
        ...
        bool setPixelFormat(HYDROGEN_PIXEL_FORMAT pixelFormat, uint8_t pixelDepth);
    ...
    public:
        ...
        /* Stream switch */
        HYDROGEN::PropertySwitch StreamSP{2};

        HYDROGEN::PropertyNumber StreamTimeNP{1};

        /* Record switch */
        HYDROGEN::PropertySwitch RecordStreamSP{4};
        enum
        {
            RECORD_ON,
            RECORD_TIME,
        ...
        /* Record File Info */
        HYDROGEN::PropertyText RecordFileTP{2};

        HYDROGEN::PropertyNumber StreamExposureNP{2};
        enum
        {
            STREAM_EXPOSURE,
            STREAM_DIVISOR,
        ...
        /* Measured FPS */
        HYDROGEN::PropertyNumber FpsNP{2};
        enum
        {
            FPS_INSTANT,
            FPS_AVERAGE
        ...
        /* Record Options */
        HYDROGEN::PropertyNumber RecordOptionsNP{2};

        // Stream Frame
        HYDROGEN::PropertyNumber StreamFrameNP{4};

        /* BLOBs */
        HYDROGEN::PropertyBlob imageBP{HYDROGEN::Property()};

        // Encoder Selector. It's static now but should this implemented as plugin interface?
        HYDROGEN::PropertySwitch EncoderSP{2};
        enum
        {
            ENCODER_RAW,
            ENCODER_MJPEG
        ...
        HYDROGEN::PropertySwitch RecorderSP{2};
        enum
        {
            RECORDER_RAW,
            RECORDER_OGV
        ...
        HYDROGEN::PropertyNumber LimitsNP{2};
        enum
        {
            LIMITS_BUFFER_MAX,
            LIMITS_PREVIEW_FPS
        ...
        HYDROGEN_PIXEL_FORMAT PixelFormat = HYDROGEN_MONO;
