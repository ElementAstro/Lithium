...
bool ClientSharedBlobs::parseAttachedBlobs(const HYDROGEN::LilXmlElement &root, ClientSharedBlobs::Blobs &blobs)
{
    ...
    clientSocket.onData([this](const char *data, size_t size)
    {
        ...
        for (const auto &doc : documents)
        {
            ...
            if (err_code < 0)
            {
                ...
                if (err_code != HYDROGEN_PROPERTY_DUPLICATED)
                {
...
bool BaseClientPrivate::connectToHostAndWait(std::string hostname, unsigned short port)
{
    ...
    {
        hostname = "localhost:/tmp/hydrogenserver";
    }
...
bool BaseClient::connectServer()
{
    ...
    {
        IDLog("HYDROGEN::BaseClient::connectServer: Already connected.\n");
        return false;
    }

    IDLog("HYDROGEN::BaseClient::connectServer: creating new connection...\n");
...
bool BaseClient::disconnectServer(int exit_code)
{
    ...
    {
        IDLog("HYDROGEN::BaseClient::disconnectServer: Already disconnected.\n");
        return false;
    }
