...
/** @class HYDROGEN::BaseClient
 *  @brief Class to provide basic client functionality.
 *
 *  BaseClient enables accelerated development of HYDROGEN Clients by providing a framework that facilitates communication, device
 *  handling, and event notification. By subclassing BaseClient, clients can quickly connect to an HYDROGEN server, and query for
 ...
 *
 *  Upon connecting to an HYDROGEN server, it creates a dedicated thread to handle all incoming traffic. The thread is terminated
 ...
 *
 *  @see <a href="http://hydrogenlib.org/develop/tutorials/107-client-development-tutorial.html">HYDROGEN Client Tutorial</a> for more details.
 *  @author Jasem Mutlaq
...
class HYDROGEN::BaseClient : public HYDROGEN::AbstractBaseClient
{
        DECLARE_PRIVATE_D(d_ptr_hydrogen, BaseClient)
