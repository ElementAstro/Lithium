...
class AbstractBaseClient : public HYDROGEN::BaseMediator
{
        DECLARE_PRIVATE_D(d_ptr_hydrogen, AbstractBaseClient)
    ...
    public:
         ...
         *  @param hostname HYDROGEN server host name or IP address.
         *  @param port HYDROGEN server port.
         */
    ...
    public:
         ...
         *  HYDROGEN::BaseDevice object to handle them. If no devices are watched, then all devices owned by HYDROGEN server
         *  will be created and handled.
         */
        void watchDevice(const char *deviceName);
        void watchDevice(const char *deviceName, const std::function<void (BaseDevice)> &callback);

        /** @brief watchProperties Add a property to the watch list. When communicating with HYDROGEN server.
         *
    ...
    public:
        /** @brief Disconnect HYDROGEN driver
    ...
    public:
        /** @param deviceName Name of device to search for in the list of devices owned by HYDROGEN server,
         *  @returns If \e deviceName exists, it returns an instance of the device. Otherwise, it returns NULL.
         */
        HYDROGEN::BaseDevice getDevice(const char *deviceName);

        /** @returns Returns a vector of all devices created in the client. */
        std::vector<HYDROGEN::BaseDevice> getDevices() const;

        /** @brief getDevices Returns list of devices that belong to a particular @ref HYDROGEN::BaseDevice::DRIVER_INTERFACE "DRIVER_INTERFACE" class.
         *
         *  For example, to get a list of guide cameras:
         *  @code{.cpp}
         *  std::vector<HYDROGEN::BaseDevice *> guideCameras;
         *  getDevices(guideCameras, CCD_INTERFACE | GUIDER_INTERFACE);
         *  for (HYDROGEN::BaseDevice *device : guideCameras)
         ...
         */
        bool getDevices(std::vector<HYDROGEN::BaseDevice> &deviceList, uint16_t driverInterface);
    ...
    public:
        ...
        void sendNewProperty(HYDROGEN::Property pp);

        /** @brief Send new Text command to server */
        void sendNewText(HYDROGEN::Property pp);
        /** @brief Send new Text command to server */
        ...
        void sendNewNumber(HYDROGEN::Property pp);
        /** @brief Send new Number command to server */
        ...
        void sendNewSwitch(HYDROGEN::Property pp);
        /** @brief Send new Switch command to server */
        ...
        void sendOneBlob(HYDROGEN::WidgetViewBlob *blob);
        /** @brief Send ONE blob content to server. The BLOB data in raw binary format and will be converted to base64 and sent to server */
    ...
    protected:
        /**
         * @brief newUniversalMessage Universal messages are sent from HYDROGEN server without a specific device. It is addressed to the client overall.
    ...
    protected:
        std::unique_ptr<AbstractBaseClientPrivate> d_ptr_hydrogen;
};

}

#ifdef SWIG
%template(BaseDeviceVectorShared) std::vector<HYDROGEN::BaseDevice>;
#endif
