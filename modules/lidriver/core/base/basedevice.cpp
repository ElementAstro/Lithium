...
#ifdef ENABLE_LITHIUM_SHARED_MEMORY
...
namespace LITHIUM
{
    ...
    LITHIUM::PropertyNumber BaseDevice::getNumber(const char *name) const
    {
        return getProperty(name, LITHIUM_NUMBER);
    }

    LITHIUM::PropertyText BaseDevice::getText(const char *name) const
    {
        return getProperty(name, LITHIUM_TEXT);
    }

    LITHIUM::PropertySwitch BaseDevice::getSwitch(const char *name) const
    {
        return getProperty(name, LITHIUM_SWITCH);
    }

    LITHIUM::PropertyLight BaseDevice::getLight(const char *name) const
    {
        return getProperty(name, LITHIUM_LIGHT);
    }

    LITHIUM::PropertyBlob BaseDevice::getBLOB(const char *name) const
    {
        return getProperty(name, LITHIUM_BLOB);
    }
    ...
    void *BaseDevice::getRawProperty(const char *name, LITHIUM_PROPERTY_TYPE type) const
    {
        LITHIUM::Property prop = getProperty(name, type);
        return prop ? prop.getProperty() : nullptr;
    }

    LITHIUM::Property BaseDevice::getProperty(const char *name, LITHIUM_PROPERTY_TYPE type) const
    {
        ...
        for (const auto &oneProp : getProperties())
        {
            if (type != oneProp.getType() && type != LITHIUM_UNKNOWN)
                continue;
    ...
    {
        D_PTR(BaseDevice);
        int result = LITHIUM_PROPERTY_INVALID;

        std::lock_guard<std::mutex> lock(d->m_Lock);

        d->pAll.erase_if([&name, &result](LITHIUM::Property &prop) -> bool
                         {
#if 0
    ...
    int BaseDevice::buildProp(const LITHIUM::LilXmlElement &root, char *errmsg, bool isDynamic)
    {
        D_PTR(BaseDevice);
        ...
        // find type of tag
        static const std::map<LITHIUM_PROPERTY_TYPE, std::string> tagTypeName =
            {
                {LITHIUM_NUMBER, "defNumberVector"},
                {LITHIUM_SWITCH, "defSwitchVector"},
                {LITHIUM_TEXT, "defTextVector"},
                {LITHIUM_LIGHT, "defLightVector"},
                {LITHIUM_BLOB, "defBLOBVector"}};
        ...
        case LITHIUM_TEXT:
        {
            LITHIUM::PropertyText typedProperty{0};
            for (const auto &element : root.getElementsByTagName("defText"))
            {
                LITHIUM::WidgetViewText widget;
        ...
        case LITHIUM_LIGHT:
        {
            LITHIUM::PropertyLight typedProperty{0};
            for (const auto &element : root.getElementsByTagName("defLight"))
            {
                LITHIUM::WidgetViewLight widget;
        ...
        case LITHIUM_BLOB:
        {
            LITHIUM::PropertyBlob typedProperty{0};
        ...
        case LITHIUM_UNKNOWN: // it will never happen
            return -1;
        }
        ...
        if (rootTagType->first != LITHIUM_LIGHT)
        {
    ...
    {
        auto svp = getSwitch(LITHIUM::SP::CONNECTION);
        if (!svp)
            return false;
    ...
    static void for_property(
        ...
        LITHIUM::Property &property,
        const std::function<void(const LilXmlElement &, LITHIUM::WidgetView<typename TypedProperty::ViewType> *)> &function)
    {
     ...
     */
    int BaseDevice::setValue(const LITHIUM::LilXmlElement &root, char *errmsg)
    {
        D_PTR(BaseDevice);
        ...
        static const std::map<LITHIUM_PROPERTY_TYPE, std::string> tagTypeName =
            {
                {LITHIUM_NUMBER, "setNumberVector"},
                {LITHIUM_SWITCH, "setSwitchVector"},
                {LITHIUM_TEXT, "setTextVector"},
                {LITHIUM_LIGHT, "setLightVector"},
                {LITHIUM_BLOB, "setBLOBVector"}};
        ...
        LITHIUM::Property property = getProperty(propertyName, rootTagType->first);
        ...
        {
            AutoCNumeric locale;
            for_property<LITHIUM::PropertyNumber>(root, property, [](const LilXmlElement &element, auto *item)
                                                  {
        ...
        case LITHIUM_SWITCH:
        {
            for_property<LITHIUM::PropertySwitch>(root, property, [](const LilXmlElement &element, auto *item)
        ...
        case LITHIUM_TEXT:
        {
            for_property<LITHIUM::PropertyText>(root, property, [](const LilXmlElement &element, auto *item)
        ...
        case LITHIUM_LIGHT:
        {
            for_property<LITHIUM::PropertyLight>(root, property, [](const LilXmlElement &element, auto *item)
                                                 { item->setState(element.context()); });
            break;
        }
...
#ifdef ENABLE_LITHIUM_SHARED_MEMORY
    static bool sSharedToBlob(const LITHIUM::LilXmlElement &element, LITHIUM::WidgetViewBlob &widget)
    {
     ...
     */
    int BaseDevicePrivate::setBLOB(LITHIUM::PropertyBlob property, const LilXmlElement &root, char *errmsg)
    {
    ...
    void BaseDevice::watchProperty(const char *name, const std::function<void(LITHIUM::Property)> &callback, WATCH watch)
    {
        D_PTR(BaseDevice);
        ...
        LITHIUM::Property property = getProperty(name);
        if (property.isValid())
        {
            callback(property);
        }
    ...
    void BaseDevice::registerProperty(const LITHIUM::Property &property)
    {
        D_PTR(BaseDevice);

        if (property.getType() == LITHIUM_UNKNOWN)
            return;
    ...
    // #PS: TODO remove
    void BaseDevice::registerProperty(const LITHIUM::Property &property, LITHIUM_PROPERTY_TYPE)
    {
    ...
    void BaseDevice::setMediator(LITHIUM::BaseMediator *mediator)
    {
        D_PTR(BaseDevice);
        d->mediator = mediator;
    }

    LITHIUM::BaseMediator *BaseDevice::getMediator() const
    {
        D_PTR(const BaseDevice);
        return d->mediator;
    }
