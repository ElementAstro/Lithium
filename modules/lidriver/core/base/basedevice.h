...
/** @class LITHIUM::BaseDevice
 *  @brief Class to provide basic INDI device functionality.
 *
 *  LITHIUM::BaseDevice is the base device for all INDI devices and contains a list of all properties defined by the device either explicity or via a skeleton file.
 *  You don't need to subclass LITHIUM::BaseDevice class directly, it is inheritied by LITHIUM::DefaultDevice which takes care of building a standard INDI device. Moreover, LITHIUM::BaseClient
 *  maintains a list of LITHIUM::BaseDevice objects as they get defined from the INDI server, and those objects may be accessed to retrieve information on the object properties or message log.
 *
 *  @author Jasem Mutlaq
 */
namespace LITHIUM
{
    ...
    public:
        typedef LITHIUM::Properties Properties;
        // typedef std::vector<LITHIUM::Property*> Properties;

        /*! INDI error codes. */
        enum LITHIUM_ERROR
        {
            LITHIUM_DEVICE_NOT_FOUND = -1,    /*!< INDI Device was not found. */
            LITHIUM_PROPERTY_INVALID = -2,    /*!< Property has an invalid syntax or attribute. */
            LITHIUM_PROPERTY_DUPLICATED = -3, /*!< INDI Device was not found. */
            LITHIUM_DISPATCH_ERROR = -4       /*!< Dispatching command to driver failed. */
        };
        ...
        enum DRIVER_INTERFACE
        {
            ...
            TELESCOPE_INTERFACE = (1 << 0),     /**< Telescope interface, must subclass LITHIUM::Telescope */
            CCD_INTERFACE = (1 << 1),           /**< CCD interface, must subclass LITHIUM::CCD */
            GUIDER_INTERFACE = (1 << 2),        /**< Guider interface, must subclass LITHIUM::GuiderInterface */
            FOCUSER_INTERFACE = (1 << 3),       /**< Focuser interface, must subclass LITHIUM::FocuserInterface */
            FILTER_INTERFACE = (1 << 4),        /**< Filter interface, must subclass LITHIUM::FilterInterface */
            DOME_INTERFACE = (1 << 5),          /**< Dome interface, must subclass LITHIUM::Dome */
            GPS_INTERFACE = (1 << 6),           /**< GPS interface, must subclass LITHIUM::GPS */
            WEATHER_INTERFACE = (1 << 7),       /**< Weather interface, must subclass LITHIUM::Weather */
            ...
            DETECTOR_INTERFACE = (1 << 11),     /**< Detector interface, must subclass LITHIUM::Detector */
            ROTATOR_INTERFACE = (1 << 12),      /**< Rotator interface, must subclass LITHIUM::RotatorInterface */
    ...
    public: // property
         ...
         *  @param property any property from the LITHIUM::PropertyXXX family.
         */
        void registerProperty(const LITHIUM::Property &property);
        void registerProperty(const LITHIUM::Property &property,
                              LITHIUM_PROPERTY_TYPE type); // backward compatiblity (PentaxCCD, PkTriggerCordCCD)
         ...
         *  @param callback as an argument of the function you can use LITHIUM::PropertyNumber, LITHIUM::PropertySwitch etc.
         ...
         */
        void watchProperty(const char *name, const std::function<void(LITHIUM::Property)> &callback, WATCH watch = WATCH_NEW);
         ...
         *  returned.
         */
        Property getProperty(const char *name, LITHIUM_PROPERTY_TYPE type = LITHIUM_UNKNOWN) const;
    ...
    public:
        ...
        LITHIUM::PropertyNumber getNumber(const char *name) const;
        /** @return Return vector text property given its name */
        LITHIUM::PropertyText getText(const char *name) const;
        /** @return Return vector switch property given its name */
        LITHIUM::PropertySwitch getSwitch(const char *name) const;
        /** @return Return vector light property given its name */
        LITHIUM::PropertyLight getLight(const char *name) const;
        /** @return Return vector BLOB property given its name */
        LITHIUM::PropertyBlob getBLOB(const char *name) const;
    ...
    public: // deprecated
         ...
         *  returned. For example, INumberVectorProperty *num = static_cast<INumberVectorProperty> getRawProperty("FOO", LITHIUM_NUMBER);
         *
         ...
         */
        void *getRawProperty(const char *name, LITHIUM_PROPERTY_TYPE type = LITHIUM_UNKNOWN) const;
    ...
    public:
        ...
        void setMediator(LITHIUM::BaseMediator *mediator);

        /** @returns Get the meditator assigned to this driver */
        LITHIUM::BaseMediator *getMediator() const;
        ...
        /** @brief getDriverInterface returns ORed values of @ref LITHIUM::BaseDevice::DRIVER_INTERFACE "DRIVER_INTERFACE". It presents the device classes supported by the driver.
    ...
    public:
         ...
         */
        int buildProp(const LITHIUM::LilXmlElement &root, char *errmsg, bool isDynamic = false);

        /** @brief handle SetXXX commands from client */
        int setValue(const LITHIUM::LilXmlElement &root, char *errmsg);
    ...
    public:
        LITHIUM_DEPRECATED("Do not use BaseDevice as pointer.")
        operator BaseDevice *();
