...
namespace Connection
{
    ...
    Serial::Serial(LITHIUM::DefaultDevice *dev) : Interface(dev, CONNECTION_SERIAL)
    {
        char configPort[256] = {0};
        // Try to load the port from the config file. If that fails, use default port.
        if (IUGetConfigText(dev->getDeviceName(), LITHIUM::SP::DEVICE_PORT, "PORT", configPort, 256) == 0)
        {
        ...
        IUFillTextVector(&PortTP, PortT, 1, dev->getDeviceName(), LITHIUM::SP::DEVICE_PORT, "Ports", CONNECTION_TAB, IP_RW, 60,
                         IPS_IDLE);

        int autoSearchIndex = 0;
        // Try to load the port from the config file. If that fails, use default port.
        IUGetConfigOnSwitchIndex(dev->getDeviceName(), LITHIUM::SP::DEVICE_AUTO_SEARCH, &autoSearchIndex);
        IUFillSwitch(&AutoSearchS[LITHIUM::DefaultDevice::LITHIUM_ENABLED], "LITHIUM_ENABLED", "Enabled",
                     autoSearchIndex == 0 ? ISS_ON : ISS_OFF);
        IUFillSwitch(&AutoSearchS[LITHIUM::DefaultDevice::LITHIUM_DISABLED], "LITHIUM_DISABLED", "Disabled",
                     autoSearchIndex == 0 ? ISS_OFF : ISS_ON);
        IUFillSwitchVector(&AutoSearchSP, AutoSearchS, 2, dev->getDeviceName(), LITHIUM::SP::DEVICE_AUTO_SEARCH, "Auto Search",
        ...
        IUFillSwitchVector(&BaudRateSP, BaudRateS, 6, dev->getDeviceName(), LITHIUM::SP::DEVICE_BAUD_RATE, "Baud Rate", CONNECTION_TAB,
                           IP_RW, ISR_1OFMANY, 60, IPS_IDLE);

        // Try to load the port from the config file. If that fails, use default port.
        IUGetConfigOnSwitchIndex(dev->getDeviceName(), LITHIUM::SP::DEVICE_BAUD_RATE, &m_ConfigBaudRate);
        ...
        if (AutoSearchS[0].s == ISS_ON && SystemPortS != nullptr && SystemPortSP.nsp > 1)
        {
            ...
            for (const auto &port : systemPorts)
            {
                ...
                if (Connect(port.c_str(), baud) && processHandshake())
                {
                    ...
                    if (AutoSearchS[LITHIUM::DefaultDevice::LITHIUM_ENABLED].s == ISS_ON)
                    {
                        saveConfig = true;
                        AutoSearchS[LITHIUM::DefaultDevice::LITHIUM_ENABLED].s = ISS_OFF;
                        AutoSearchS[LITHIUM::DefaultDevice::LITHIUM_DISABLED].s = ISS_ON;
                        IDSetSwitch(&AutoSearchSP, nullptr);
                    }
    ...
    bool Serial::processHandshake()
    {
        ...
        if (rc)
        {
            ...
            {
                m_Device->saveConfig(true, LITHIUM::SP::DEVICE_PORT);
                m_Device->saveConfig(true, LITHIUM::SP::DEVICE_BAUD_RATE);
            }
        }
        else
