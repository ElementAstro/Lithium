...
namespace Connection
{
    ...
    ///
    //////////////////////////////////////////////////////////////////////////////////////////////////
    TCP::TCP(LITHIUM::DefaultDevice *dev) : Interface(dev, CONNECTION_TCP)
    {
        char defaultHostname[MAXLITHIUMNAME] = {0};
        char defaultPort[MAXLITHIUMNAME] = {0};

        // Try to load the port from the config file. If that fails, use default port.
        if (IUGetConfigText(dev->getDeviceName(), LITHIUM::SP::DEVICE_ADDRESS, "ADDRESS", defaultHostname, MAXLITHIUMNAME) == 0)
            m_ConfigHost = defaultHostname;
        if (IUGetConfigText(dev->getDeviceName(), LITHIUM::SP::DEVICE_ADDRESS, "PORT", defaultPort, MAXLITHIUMNAME) == 0)
        ...
        // Try to load the port from the config file. If that fails, use default port.
        IUGetConfigOnSwitchIndex(dev->getDeviceName(), LITHIUM::SP::DEVICE_AUTO_SEARCH, &autoSearchIndex);
        IUFillSwitch(&LANSearchS[LITHIUM::DefaultDevice::LITHIUM_ENABLED], "LITHIUM_ENABLED", "Enabled",
                     autoSearchIndex == 0 ? ISS_ON : ISS_OFF);
        IUFillSwitch(&LANSearchS[LITHIUM::DefaultDevice::LITHIUM_DISABLED], "LITHIUM_DISABLED", "Disabled",
                     autoSearchIndex == 0 ? ISS_OFF : ISS_ON);
        IUFillSwitchVector(&LANSearchSP, LANSearchS, 2, dev->getDeviceName(), LITHIUM::SP::DEVICE_LAN_SEARCH, "LAN Search",
                           CONNECTION_TAB, IP_RW, ISR_1OFMANY, 60, IPS_IDLE);
    }
    ...
    bool TCP::establishConnection(const std::string &hostname, const std::string &port, int timeout)
    {
        ...
        if (LANSearchS[LITHIUM::DefaultDevice::LITHIUM_ENABLED].s == ISS_OFF)
    ...
    bool TCP::Connect()
    {
        ...
        else
        {
            ...
            if (establishConnection(hostname, port))
            {
                ...
                if (handshakeResult == false && LANSearchS[LITHIUM::DefaultDevice::LITHIUM_ENABLED].s == ISS_OFF)
                {
            ...
            if (handshakeResult == false &&
                LANSearchS[LITHIUM::DefaultDevice::LITHIUM_ENABLED].s == ISS_ON &&
                isIPv4)
            {
        ...
        if (handshakeResult)
        {
            ...
            if (LANSearchS[LITHIUM::DefaultDevice::LITHIUM_ENABLED].s == ISS_ON)
            {
                LANSearchS[LITHIUM::DefaultDevice::LITHIUM_ENABLED].s = ISS_OFF;
                LANSearchS[LITHIUM::DefaultDevice::LITHIUM_DISABLED].s = ISS_ON;
    ...
    {
        LANSearchS[LITHIUM::DefaultDevice::LITHIUM_ENABLED].s = enabled ? ISS_ON : ISS_OFF;
        LANSearchS[LITHIUM::DefaultDevice::LITHIUM_DISABLED].s = enabled ? ISS_OFF : ISS_ON;
        if (m_Device->isInitializationComplete())
            IDSetSwitch(&LANSearchSP, nullptr);
    }
}
