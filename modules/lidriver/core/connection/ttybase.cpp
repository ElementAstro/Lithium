...
#ifdef _WIN32
    retval = select(static_cast<int>(m_PortFD) + 1, &readout, nullptr, nullptr, &tv);
#else
...
TTYBase::TTY_RESPONSE TTYBase::write(const uint8_t *buffer, uint32_t nbytes, uint32_t *nbytes_written)
{
    ...
    *nbytes_written = 0;
    ...
    while (nbytes > 0)
    {
        if (!WriteFile(m_PortFD, buffer + (*nbytes_written), nbytes, &bytes_w, nullptr))
...
TTYBase::TTY_RESPONSE TTYBase::read(uint8_t *buffer, uint32_t nbytes, uint8_t timeout, uint32_t *nbytes_read)
{
    ...
    *nbytes_read = 0;
    ...
    while (num_bytes_to_read > 0)
    {
        ...
        if (!ReadFile(m_PortFD, buffer + (*nbytes_read), num_bytes_to_read, &bytes_read, nullptr))
            return TTY_READ_ERROR;

        DEBUGFDEVICE(m_DriverName, m_DebugChannel, "%d bytes read and %d bytes remaining...", bytes_read,
                                           ...
                                           uint32_t *nbytes_read)
{
    ...
    *nbytes_read = 0;
    ...
    for (;;)
    {
        ...
        if (!ReadFile(m_PortFD, (buffer + *nbytes_read), 1, &bytes_read, nullptr))
            return TTY_READ_ERROR;

        DEBUGFDEVICE(m_DriverName, m_DebugChannel, "%s: buffer[%d]=%#X (%c)", __FUNCTION__, (*nbytes_read),
                     *(buffer + *nbytes_read), *(buffer + *nbytes_read));
                                       ...
                                       uint8_t stop_bits)
{
    ...
    t_fd = CreateFileA(device, GENERIC_READ | GENERIC_WRITE, 0, NULL, OPEN_EXISTING, 0, NULL);
    ...
    {
        DEBUGDEVICE(m_DriverName, m_DebugChannel, "connect: failed getting DCB settings.");
        CloseHandle(t_fd);
        return TTY_PORT_FAILURE;
    }
    ...
    {
        DEBUGDEVICE(m_DriverName, m_DebugChannel, "connect: failed setting DCB settings on serial port.");
        CloseHandle(t_fd);
        return TTY_PORT_FAILURE;
    }
    ...
    {
        DEBUGDEVICE(m_DriverName, m_DebugChannel, "connect: failed setting timeouts on serial port.");
        CloseHandle(t_fd);
        return TTY_PORT_FAILURE;
    }
