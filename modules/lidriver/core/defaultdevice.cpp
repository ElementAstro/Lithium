...
std::list<LITHIUM::DefaultDevicePrivate *> LITHIUM::DefaultDevicePrivate::devices;
std::recursive_mutex LITHIUM::DefaultDevicePrivate::devicesLock;
...
extern "C"
{
    ...
    {
        const std::unique_lock<std::recursive_mutex> lock(LITHIUM::DefaultDevicePrivate::devicesLock);
        for (auto &it : LITHIUM::DefaultDevicePrivate::devices)
...
{
    LITHIUM::DefaultDevice *devPtr = static_cast<LITHIUM::DefaultDevice *>(t);
    if (devPtr != nullptr)
    {
...
namespace LITHIUM
{
    ...
    bool DefaultDevice::loadConfig(LITHIUM::Property &property)
    {
    ...
    bool DefaultDevice::saveConfigItems(FILE *fp)
    {
        ...
        return LITHIUM::Logger::saveConfigItems(fp);
    }
    ...
    bool DefaultDevice::saveAllConfigItems(FILE *fp)
    {
        ...
        {
            if (oneProperty.getType() == LITHIUM_SWITCH)
            {
                ...
                if (
                    (svp->isNameMatch(LITHIUM::SP::CONNECTION)) ||
    ...
    bool DefaultDevice::saveConfig(LITHIUM::Property &property)
    {
    ...
    bool DefaultDevice::loadDefaultConfig()
    {
        ...
        if (getenv("LITHIUMCONFIG"))
            snprintf(configDefaultFileName, MAXRBUF, "%s.default", getenv("LITHIUMCONFIG"));
        else
    ...
    bool DefaultDevice::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
    {
        ...
        LITHIUM::PropertySwitch property = getProperty(name, LITHIUM_SWITCH);
    ...
    bool DefaultDevice::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
    {
        ...
        LITHIUM::PropertyNumber property = getProperty(name, LITHIUM_NUMBER);
    ...
    bool DefaultDevice::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
    {
        ...
        LITHIUM::PropertyText property = getProperty(name, LITHIUM_TEXT);
                                  ...
                                  char *formats[], char *names[], int n)
    {
        ...
        LITHIUM::PropertyBlob property = getProperty(name, LITHIUM_BLOB);
    ...
    bool DefaultDevice::ISSnoopDevice(XMLEle *root)
    {
        D_PTR(DefaultDevice);
        char errmsg[MAXRBUF];
        return d->watchDevice.processXml(LITHIUM::LilXmlElement(root), errmsg) < 0;
    }
    ...
    void DefaultDevice::setConnected(bool status, IPState state, const char *msg)
    {
        ...
        svp[LITHIUM_ENABLED].setState(status ? ISS_ON : ISS_OFF);
        svp[LITHIUM_DISABLED].setState(status ? ISS_OFF : ISS_ON);
        svp.setState(state);
    ...
    bool DefaultDevice::initProperties()
    {
        ...
        // Connection
        d->ConnectionSP[LITHIUM_ENABLED].fill("CONNECT", "Connect", ISS_OFF);
        d->ConnectionSP[LITHIUM_DISABLED].fill("DISCONNECT", "Disconnect", ISS_ON);
        d->ConnectionSP.fill(getDeviceName(), LITHIUM::SP::CONNECTION, "Connection", "Main Control", IP_RW, ISR_1OFMANY, 60, IPS_IDLE);
        d->ConnectionSP.onNewValues([this](const LITHIUM::PropertySwitch::NewValues &values)
                                    {
        ...
        // Debug
        d->DebugSP[LITHIUM_ENABLED].fill("ENABLE", "Enable", ISS_OFF);
        d->DebugSP[LITHIUM_DISABLED].fill("DISABLE", "Disable", ISS_ON);
        ...
        // Simulation
        d->SimulationSP[LITHIUM_ENABLED].fill("ENABLE", "Enable", ISS_OFF);
        d->SimulationSP[LITHIUM_DISABLED].fill("DISABLE", "Disable", ISS_ON);
        ...
        LITHIUM::Logger::initProperties(this);
    ...
    bool DefaultDevice::deleteProperty(LITHIUM::Property &property)
    {
    ...
    bool DefaultDevice::deleteProperty(const char *propertyName)
    {
        ...
        {
            LITHIUM::Property prop = getProperty(propertyName);
    ...
    {
        registerProperty(LITHIUM::Property(property));
    ...
    void DefaultDevice::defineProperty(LITHIUM::Property &property)
    {
        ...
        if (!d->ConnectionModeSP.isEmpty())
        {
            ...
            if (it != d->connections.end())
            {
                ...
                if (index >= 0)
                {
                    ...
                    LITHIUM::Property connectionProperty = getProperty(d->ConnectionModeSP.getName(), LITHIUM_SWITCH);
