...
namespace DSP
{
    ...
    Interface::Interface(LITHIUM::DefaultDevice *dev, Type type, const char *name, const char *label) : m_Device(dev),
                                                                                                        m_Name(name), m_Label(label), m_Type(type)
    {
        char activatestrname[MAXLITHIUMNAME];
        char activatestrlabel[MAXLITHIUMLABEL];
    ...
    {
        LITHIUM_UNUSED(buf);
        LITHIUM_UNUSED(ndims);
        LITHIUM_UNUSED(dims);
        LITHIUM_UNUSED(bits_per_sample);
        DEBUG(LITHIUM::Logger::DBG_WARNING, "Interface::Callback -  Should never get here");
        return nullptr;
    }
    ...
    void Interface::addFITSKeywords(fitsfile *fptr)
    {
        ...
        char fitsString[MAXLITHIUMDEVICE];

        // Telescope
        strncpy(fitsString, m_Device->getText("ACTIVE_DEVICES")[0].getText(), MAXLITHIUMDEVICE);
        fits_update_key_s(fptr, TSTRING, "TELESCOP", fitsString, "Telescope name", &status);

        // Observer
        strncpy(fitsString, m_Device->getText("FITS_HEADER")[0].getText(), MAXLITHIUMDEVICE);
        fits_update_key_s(fptr, TSTRING, "OBSERVER", fitsString, "Observer name", &status);

        // Object
        strncpy(fitsString, m_Device->getText("FITS_HEADER")[1].getText(), MAXLITHIUMDEVICE);
        ...
        if (!nv)
        {
            ...
            char lat_str[MAXLITHIUMFORMAT];
            char lon_str[MAXLITHIUMFORMAT];
            char el_str[MAXLITHIUMFORMAT];
            fs_sexa(lat_str, Lat, 2, 360000);
            fs_sexa(lon_str, Lon, 2, 360000);
            snprintf(el_str, MAXLITHIUMFORMAT, "%lf", El);
        ...
        fits_write_comment(fptr, "Generated by LITHIUM", &status);
    ...
    dsp_stream_p Interface::loadFITS(char *buffer, int len)
    {
        ...
        char filename[MAXLITHIUMNAME];
        sprintf(filename, "LITHIUM_DSP_INTERFACE_XXXXXX");
    ...
    bool Interface::sendFITS(uint8_t *buf, bool sendCapture, bool saveCapture)
    {
        ...
        default:
            DEBUGF(LITHIUM::Logger::DBG_ERROR, "Unsupported bits per sample value %d", getBPS());
            return false;
        }
        ...
        char error_status[MAXLITHIUMNAME];
    ...
    bool Interface::uploadFile(const void *fitsData, size_t totalBytes, bool sendCapture, bool saveCapture, const char *format)
    {
        ...
        DEBUGF(LITHIUM::Logger::DBG_DEBUG, "Uploading file. Ext: %s, Size: %d, sendCapture? %s, saveCapture? %s",
        ...
        snprintf(FitsB.format, MAXLITHIUMBLOBFMT, ".%s", captureExtention);
        ...
        if (saveCapture)
        {
            ...
            if (maxIndex < 0)
            {
                DEBUGF(LITHIUM::Logger::DBG_ERROR, "Error iterating directory %s. %s", m_Device->getText("UPLOAD_SETTINGS")[0].getText(),
                       strerror(errno));
                return false;
            }
            ...
            char processedFileName[MAXLITHIUMNAME];

            snprintf(processedFileName, MAXLITHIUMNAME, "%s/%s_%s.%s", m_Device->getText("UPLOAD_SETTINGS")[0].getText(), prefix.c_str(),
                     m_Name, format);

            fp = fopen(processedFileName, "w");
            if (fp == nullptr)
            {
                DEBUGF(LITHIUM::Logger::DBG_ERROR, "Unable to save image file (%s). %s", processedFileName, strerror(errno));
                return false;
            }
        ...
        DEBUG(LITHIUM::Logger::DBG_DEBUG, "Upload complete");
    ...
    {
        snprintf(captureExtention, MAXLITHIUMBLOBFMT, "%s", ext);
    }
}
