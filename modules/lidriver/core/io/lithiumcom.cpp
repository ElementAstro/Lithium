...
int fs_sexa(char *out, double a, int w, int fracbase)
{
    ...
    if (isneg && d == 0)
        out += snprintf(out, MAXLITHIUMFORMAT, "%*s-0", w - 2, "");
    else
        out += snprintf(out, MAXLITHIUMFORMAT, "%*d", w, isneg ? -d : d);

    switch (fracbase)
    {
    case 60: // dd:mm
        m = f / (fracbase / 60);
        out += snprintf(out, MAXLITHIUMFORMAT, ":%02d", m);
        break;
    case 600: // dd:mm.m
        out += snprintf(out, MAXLITHIUMFORMAT, ":%02d.%1d", f / 10, f % 10);
        break;
    case 3600: // dd:mm:ss
        m = f / (fracbase / 60);
        s = f % (fracbase / 60);
        out += snprintf(out, MAXLITHIUMFORMAT, ":%02d:%02d", m, s);
        break;
    case 36000: // dd:mm:ss.s
        m = f / (fracbase / 60);
        s = f % (fracbase / 60);
        out += snprintf(out, MAXLITHIUMFORMAT, ":%02d:%02d.%1d", m, s / 10, s % 10);
        break;
    case 360000: // dd:mm:ss.ss
        m = f / (fracbase / 60);
        s = f % (fracbase / 60);
        out += snprintf(out, MAXLITHIUMFORMAT, ":%02d:%02d.%02d", m, s / 100, s % 100);
        break;
    default:
        return -1;
    }
...
int numberFormat(char *buf, const char *format, double value)
{
    ...
    else
    {
        ...
        return (snprintf(buf, MAXLITHIUMFORMAT, format, value));
    }
}
...
{
    va_list ap;
    /* JM: Since all LITHIUM's stderr are timestampped now, we don't need to time stamp ID Log */
