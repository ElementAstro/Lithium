...
/** @file lithiumdevapi.h
 *  @brief Interface to the reference LITHIUM C API device implementation on the Device Driver side.
 *
 *  @author Elwood C. Downey
 *  @author Jasem Mutlaq
 *
 *  This file is divided into two main sections:\n
 *  <ol><li> Functions the LITHIUM device driver framework defines which the Driver may
 *  call:</li>
 *
 *  <ul><li>IDxxx functions to send messages to an LITHIUM client. Please note that it is recommended to use the
 *   LITHIUM Library wrapper functions instead of calling IDxxx directly.
 *   <ul>
 *    <li>IDMessage: Use @ref LITHIUM::Logger "LITHIUM Logging Framework" functions (e.g. LOG_DEBUG..etc) instead.</li>
 *    <li>IDDefXXX: use @ref LITHIUM::DefaultDevice "LITHIUM Default Device" defXXX functions intead.</li>
 *   </ul>
 *   </li>
 *  <li>IExxx functions to implement the event driven model.</li>
 *  <li>IUxxx functions to perform handy utility functions.</li></ul>
 *
 *  <li>Functions the LITHIUM device driver framework calls which the Driver must
 *  define:</li>
 *
 *  <ul><li>ISxxx to respond to messages from a Client.</li></ul></ol>
 *
 *  <p>These functions are the interface to the LITHIUM C-language Device Driver
 *  reference implementation library. Any driver that uses this interface is
 *  expected to \#include "lithiumdevapi.h" and to link with indidrivermain.o and
 *  eventloop.o. Indidevapi.h further includes lithiumapi.h. The former contains
 *  the prototypes for the functions documented here, although many functions
 *  take arguments defined in the latter.</p>
 *
 *  <p>These functions make it much easier to write a compliant LITHIUM driver than
 *  starting from scratch, and also serve as a concrete
 *  example of the interactions an LITHIUM driver, in any language, is expected to
 *  accommodate.</p>
 *
 ...
 *
 *  <p>A major point to realize is that an LITHIUM driver built with this framework
 *  does not contain the C main() function. As soon as a driver begins executing,
 *  it listens on stdin for LITHIUM messages. Only when a valid and appropriate
 ...
 *******************************************************************************
 *
 *  Functions the LITHIUM device driver framework defines which the Driver may call
 *
 *******************************************************************************
 *******************************************************************************
 */
 ...
 * LITHIUM message from its arguments and writes it to stdout. From there, is it
 ...
 * attribute within the LITHIUM message. If the format argument is NULL, no message
...
#define ATTRIBUTE_FORMAT_PRINTF(A, B)
    ...
    /**
     * @defgroup deventFunctions IE Functions: Functions drivers call to register with the LITHIUM event utilities.
     ...
     *******************************************************************************
     *
     *   Functions the LITHIUM Device Driver framework calls which the Driver must
     *   define.
     *
     *******************************************************************************
     *******************************************************************************
     ...
     *  getProperties message from an LITHIUM client. The argument @param dev is either a
     ...
     *  @note You do not need to call this function, it is called by LITHIUM when new text values arrive from the client.
     */
     ...
     *  @note You do not need to call this function, it is called by LITHIUM when new number values arrive from the client.
     */
     ...
     *  @note You do not need to call this function, it is called by LITHIUM when new switch values arrive from the client.
     */
     ...
     *  @note You do not need to call this function, it is called by LITHIUM when new blob values arrive from the client.
    ...
    /** @brief Function defined by Drivers that is called when another Driver it is snooping (by having previously called IDSnoopDevice()) sent any LITHIUM message.
     ...
     *         The buffer size must be at least MAXLITHIUMDEVICE bytes.
     *  @param name pointer to an allocated buffer to save the extracted elemented name attribute.
     *         The buffer size must be at least MAXLITHIUMNAME bytes.
...
#define LITHIUM_SHARED_BLOB_SUPPORT
