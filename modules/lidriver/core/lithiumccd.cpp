...
uint16_t LITHIUMWSServer::m_global_port = 11623;
#endif
...
namespace LITHIUM
{
    ...
    bool CCD::initProperties()
    {
        ...
        IUFillSwitch(&PrimaryCCD.CompressS[LITHIUM_ENABLED], "LITHIUM_ENABLED", "Enabled", ISS_OFF);
        IUFillSwitch(&PrimaryCCD.CompressS[LITHIUM_DISABLED], "LITHIUM_DISABLED", "Disabled", ISS_ON);
    ...
    IUFillNumberVector(&PrimaryCCD.RapidGuideDataNP, PrimaryCCD.RapidGuideDataN, 3, getDeviceName(),
        ...
        IUFillSwitch(&GuideCCD.CompressS[LITHIUM_ENABLED], "LITHIUM_ENABLED", "Enabled", ISS_OFF);
        IUFillSwitch(&GuideCCD.CompressS[LITHIUM_DISABLED], "LITHIUM_DISABLED", "Disabled", ISS_ON);
    ...
    IUFillNumberVector(&GuideCCD.RapidGuideDataNP, GuideCCD.RapidGuideDataN, 3, getDeviceName(),
        ...
        if (IUGetConfigOnSwitchName(getDeviceName(), "CCD_CAPTURE_FORMAT", configName, MAXLITHIUMNAME) == 0)
        ...
        IUFillSwitch(&FastExposureToggleS[LITHIUM_ENABLED], "LITHIUM_ENABLED", "Enabled",
                     m_ConfigFastExposureIndex == LITHIUM_ENABLED ? ISS_ON : ISS_OFF);
        IUFillSwitch(&FastExposureToggleS[LITHIUM_DISABLED], "LITHIUM_DISABLED", "Disabled",
                     m_ConfigFastExposureIndex == LITHIUM_DISABLED ? ISS_ON : ISS_OFF);
        ...
        // Load from config
        char telescope[MAXLITHIUMDEVICE] = {"Telescope Simulator"};
        IUGetConfigText(getDeviceName(), "ACTIVE_DEVICES", "ACTIVE_TELESCOPE", telescope, MAXLITHIUMDEVICE);
        char rotator[MAXLITHIUMDEVICE] = {"Rotator Simulator"};
        IUGetConfigText(getDeviceName(), "ACTIVE_DEVICES", "ACTIVE_ROTATOR", rotator, MAXLITHIUMDEVICE);
        char focuser[MAXLITHIUMDEVICE] = {"Focuser Simulator"};
        IUGetConfigText(getDeviceName(), "ACTIVE_DEVICES", "ACTIVE_FOCUSER", focuser, MAXLITHIUMDEVICE);
        char filter[MAXLITHIUMDEVICE] = {"CCD Simulator"};
        IUGetConfigText(getDeviceName(), "ACTIVE_DEVICES", "ACTIVE_FILTER", filter, MAXLITHIUMDEVICE);
        char skyquality[MAXLITHIUMDEVICE] = {"SQM"};
        IUGetConfigText(getDeviceName(), "ACTIVE_DEVICES", "ACTIVE_SKYQUALITY", skyquality, MAXLITHIUMDEVICE);
    ...
    bool CCD::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
    {
        ...
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
            ...
            if (!strcmp(name, ActiveDeviceTP.name))
            {
                ...
                strncpy(EqNP.device, ActiveDeviceT[ACTIVE_TELESCOPE].text, MAXLITHIUMDEVICE);
                strncpy(J2000EqNP.device, ActiveDeviceT[ACTIVE_TELESCOPE].text, MAXLITHIUMDEVICE);
    ...
    bool CCD::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
    {
        ...
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
            if (!strcmp(name, "CCD_EXPOSURE"))
            {
                ...
                // if (PrimaryCCD.ImageExposureNP.s == IPS_BUSY && FastExposureToggleS[LITHIUM_DISABLED].s == ISS_ON)
    ...
    bool CCD::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
    {
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
            ...
            if (!strcmp(name, FastExposureToggleSP.name))
            {
                ...
                if (FastExposureToggleSP.s == IPS_IDLE && FastExposureToggleS[LITHIUM_ENABLED].s == ISS_ON)
                    LOG_WARN("Experimental Feature: After a frame is downloaded, the next frame capture immediately starts to avoid any delays.");

                if (FastExposureToggleS[LITHIUM_DISABLED].s == ISS_ON)
                {
            ...
            if (strcmp(name, PrimaryCCD.CompressSP.name) == 0)
            {
                ...
                PrimaryCCD.SendCompressed = PrimaryCCD.CompressS[LITHIUM_ENABLED].s == ISS_ON;
                return true;
            }
            ...
            if (strcmp(name, GuideCCD.CompressSP.name) == 0)
            {
                ...
                GuideCCD.SendCompressed = GuideCCD.CompressS[LITHIUM_ENABLED].s == ISS_ON;
                return true;
            }
    ...
    {
        char dev_name[MAXLITHIUMNAME] = {0};
        ...
        strncpy(dev_name, getDeviceName(), MAXLITHIUMNAME);
        ...
        if (targetChip->getFrameType() == CCDChip::LIGHT_FRAME && !std::isnan(J2000RA) && !std::isnan(J2000DE))
        {
            if (!std::isnan(Latitude) && !std::isnan(Longitude))
            {
                ...
                // Convert from JNow to J2000
                LITHIUM::J2000toObserved(&J2000Pos, ln_get_julian_from_sys(), &epochPos);
        ...
        fitsKeywords.push_back(FITSRecord("Generated by LITHIUM"));
    }
    ...
    bool CCD::ExposureCompletePrivate(CCDChip *targetChip)
    {
        ...
        strncpy(exposureStartTime, targetChip->getExposureStartTime(), MAXLITHIUMNAME);
        ...
        if (FastExposureToggleS[LITHIUM_ENABLED].s != ISS_ON)
                         ...
                         bool saveImage)
    {
        ...
        if (saveImage)
        {
            ...
            snprintf(targetChip->FitsB.format, MAXLITHIUMBLOBFMT, ".%s", targetChip->getImageExtension());
        ...
        if (targetChip->SendCompressed && EncodeFormatSP[FORMAT_XISF].getState() != ISS_ON)
        {
            if (EncodeFormatSP[FORMAT_FITS].getState() == ISS_ON && !strcmp(targetChip->getImageExtension(), "fits"))
            {
                ...
                snprintf(targetChip->FitsB.format, MAXLITHIUMBLOBFMT, ".%s.fz", targetChip->getImageExtension());
            }
            else
            {
                ...
                targetChip->FitsB.blob = compressedData;
                targetChip->FitsB.bloblen = compressedBytes;
                snprintf(targetChip->FitsB.format, MAXLITHIUMBLOBFMT, ".%s.z", targetChip->getImageExtension());
            }
        }
        else
        {
    ...
    void CCD::GuideComplete(LITHIUM_EQ_AXIS axis)
    {
