...
namespace LITHIUM
{
      ...
      *
      * - To use the class in an LITHIUM::DefaultDevice based driver:
      *
      ...
      *       For example:
      *       \code{.cpp}
      *       mapController("ABORT", "Abort Telescope", LITHIUM::Controller::CONTROLLER_BUTTON, "BUTTON_1");
      *       \endcode
      ...
      *
      * The class communicates with LITHIUM joystick driver which in turn enumerates the game pad and provides
      ...
      * \warning Both the indi_joystick driver and the driver using this class must be running in the same LITHIUM server
      *         (or chained LITHIUM servers) in order for it to work as it depends on snooping among drivers.
      * \author Jasem Multaq
      */
     class Controller
     {
     public:
           ...
           * @param cdevice LITHIUM::DefaultDevice device
           */
          Controller(LITHIUM::DefaultDevice *cdevice);
          virtual ~Controller();
