...
namespace LITHIUM
{
    ...
    bool Dome::initProperties()
    {
        ...
        //    IUFillSwitch(&AutoParkS[0], "LITHIUM_ENABLED", "Enable", ISS_OFF);
        //    IUFillSwitch(&AutoParkS[1], "LITHIUM_DISABLED", "Disable", ISS_ON);
        ...
        IUFillSwitch(&DomeBacklashS[LITHIUM_ENABLED], "LITHIUM_ENABLED", "Enabled", ISS_OFF);
        IUFillSwitch(&DomeBacklashS[LITHIUM_DISABLED], "LITHIUM_DISABLED", "Disabled", ISS_ON);
    ...
    bool Dome::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
    {
        ...
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
            ...
            {
                if (DomeBacklashS[LITHIUM_ENABLED].s != ISS_ON)
                {
    ...
    bool Dome::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
    {
        ...
        else if (!strcmp(name, AutoParkSP.name))
        {
            ...
            else if (strcmp(name, DomeBacklashSP.name) == 0)
            {
                ...
                const bool enabled = IUFindOnSwitchIndex(&DomeBacklashSP) == LITHIUM_ENABLED;
    ...
    */
    ...
    void Dome::SetParkDataType(Dome::DomeParkData type)
    {
        ...
        {
        case PARK_NONE:
            strncpy(DomeMotionS[DOME_CW].label, "Open", MAXLITHIUMLABEL);
            strncpy(DomeMotionS[DOME_CCW].label, "Close", MAXLITHIUMLABEL);
            break;
