...
/* crack the given LITHIUM XML element and call driver's IS* entry points as they
 *   are recognized.
        ...
        for (n = 0, ep = nextXMLEle(root, 1); ep; ep = nextXMLEle(root, 0))
        {
            if (strcmp(tagXMLEle(ep), "oneBLOB") == 0)
            {
                ...
                if (na && fa && sa)
                {
                    ...
                    // enclen is optional and not required by LITHIUM protocol
                    if (el)
...
void IDDefTextVA(const ITextVectorProperty *tvp, const char *fmt, va_list ap)
{
    ...
    rosc_add_unique(tvp->name, tvp->device, tvp->p, tvp, LITHIUM_TEXT);
}
...
void IDDefNumberVA(const INumberVectorProperty *nvp, const char *fmt, va_list ap)
{
    ...
    /* Add this property to insure proper sanity check */
    rosc_add_unique(nvp->name, nvp->device, nvp->p, nvp, LITHIUM_NUMBER);
}
...
void IDDefSwitchVA(const ISwitchVectorProperty *svp, const char *fmt, va_list ap)
{
    ...
    /* Add this property to insure proper sanity check */
    rosc_add_unique(svp->name, svp->device, svp->p, svp, LITHIUM_SWITCH);
}
...
void IDDefBLOBVA(const IBLOBVectorProperty *bvp, const char *fmt, va_list ap)
{
    ...
    /* Add this property to insure proper sanity check */
    rosc_add_unique(bvp->name, bvp->device, bvp->p, bvp, LITHIUM_BLOB);
}
