 ...
 * \class GPS
   ...
   The GPS provides a simple interface for GPS devices. It reports time in LITHIUM standard property TIME_UTC. Location is reported in LITHIUM standard property GEOGRAPHIC_COORD
   Only one function is called by the LITHIUM framework to update GPS data (updateGPS()). If the data is valid, it is sent to the client. If GPS data is not ready yet, updateGPS will
   be called every second until the data becomes available and then LITHIUM sends the data to the client.
   ...
   \e IMPORTANT: GEOGRAPHIC_COORD stores latitude and longitude in LITHIUM specific format, refer to <a href="http://indilib.org/develop/developer-manual/101-standard-properties.html">LITHIUM Standard Properties</a> for details.
...
namespace LITHIUM
{
    ...
    protected:
        ...
        LITHIUM::PropertyNumber LocationNP{3};

        // UTC and UTC Offset
        LITHIUM::PropertyText TimeTP{2};

        // Refresh data
        LITHIUM::PropertySwitch RefreshSP{1};

        // Refresh Period
        LITHIUM::PropertyNumber PeriodNP{1};

        // System Time
        LITHIUM::PropertySwitch SystemTimeUpdateSP{3};
        enum
        {
            UPDATE_NEVER,
            UPDATE_ON_STARTUP,
