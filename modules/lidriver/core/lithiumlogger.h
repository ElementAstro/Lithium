/*******************************************************************************
 ...
 Adapted to LITHIUM Library by Jasem Mutlaq & Geehalel.
...
*/
#define DEBUG(priority, msg) LITHIUM::Logger::getInstance().print(getDeviceName(), priority, __FILE__, __LINE__, msg)
#define DEBUGF(priority, msg, ...) \
    LITHIUM::Logger::getInstance().print(getDeviceName(), priority, __FILE__, __LINE__, msg, __VA_ARGS__)

#define DEBUGDEVICE(device, priority, msg) LITHIUM::Logger::getInstance().print(device, priority, __FILE__, __LINE__, msg)
#define DEBUGFDEVICE(device, priority, msg, ...) \
    LITHIUM::Logger::getInstance().print(device, priority, __FILE__, __LINE__, msg, __VA_ARGS__)
 ...
 */
#define LOG_ERROR(txt) DEBUG(LITHIUM::Logger::DBG_ERROR, (txt))
#define LOG_WARN(txt) DEBUG(LITHIUM::Logger::DBG_WARNING, (txt))
#define LOG_INFO(txt) DEBUG(LITHIUM::Logger::DBG_SESSION, (txt))
#define LOG_DEBUG(txt) DEBUG(LITHIUM::Logger::DBG_DEBUG, (txt))
#define LOG_EXTRA1(txt) DEBUG(LITHIUM::Logger::DBG_EXTRA_1, (txt))
#define LOG_EXTRA2(txt) DEBUG(LITHIUM::Logger::DBG_EXTRA_2, (txt))
#define LOG_EXTRA3(txt) DEBUG(LITHIUM::Logger::DBG_EXTRA_3, (txt))

#define LOGF_ERROR(fmt, ...) DEBUGF(LITHIUM::Logger::DBG_ERROR, (fmt), __VA_ARGS__)
#define LOGF_WARN(fmt, ...) DEBUGF(LITHIUM::Logger::DBG_WARNING, (fmt), __VA_ARGS__)
#define LOGF_INFO(fmt, ...) DEBUGF(LITHIUM::Logger::DBG_SESSION, (fmt), __VA_ARGS__)
#define LOGF_DEBUG(fmt, ...) DEBUGF(LITHIUM::Logger::DBG_DEBUG, (fmt), __VA_ARGS__)
#define LOGF_EXTRA1(fmt, ...) DEBUGF(LITHIUM::Logger::DBG_EXTRA_1, (fmt), __VA_ARGS__)
#define LOGF_EXTRA2(fmt, ...) DEBUGF(LITHIUM::Logger::DBG_EXTRA_2, (fmt), __VA_ARGS__)
#define LOGF_EXTRA3(fmt, ...) DEBUGF(LITHIUM::Logger::DBG_EXTRA_3, (fmt), __VA_ARGS__)

namespace LITHIUM
{
    /**
     * @class LITHIUM::Logger
     * @brief The Logger class is a simple logger to log messages to file and LITHIUM clients. This is the implementation of a simple
     ...
     *      -# Errors: Use macro DEBUG(LITHIUM::Logger::DBG_ERROR, "My Error Message)
     *
     *      -# Warnings: Use macro DEBUG(LITHIUM::Logger::DBG_WARNING, "My Warning Message)
     *
     *      -# Session: Use macro DEBUG(LITHIUM::Logger::DBG_SESSION, "My Message) Session messages are the regular status messages from the driver.
     *
     *      -# Driver Debug: Use macro DEBUG(LITHIUM::Logger::DBG_DEBUG, "My Driver Debug Message)
     *
     * @note Use DEBUGF macro if you have a variable list message. e.g. DEBUGF(LITHIUM::Logger::DBG_SESSION, "Hello %s!", "There")
     *
     ...
     *
     * Check LITHIUM Tutorial two for an example simple implementation.
     */
    class Logger
    {
        ...
        static LITHIUM::DefaultDevice *parentDevice;
    ...
    public:
        ...
        struct switchinit
        {
            char name[MAXLITHIUMNAME];
            char label[MAXLITHIUMLABEL];
            ISState state;
        ...
        static bool initProperties(LITHIUM::DefaultDevice *device);
        static bool updateProperties(bool enable);
        static char Tags[nlevels][MAXLITHIUMNAME];
