...
namespace LITHIUM
{
    ...
    SensorInterface::SensorInterface()
    {
        ...
        strncpy(integrationExtention, "raw", MAXLITHIUMBLOBFMT);
    }
    ...
    {
        LITHIUM::DefaultDevice::ISGetProperties(dev);
    ...
    bool SensorInterface::processText(const char *dev, const char *name, char *texts[], char *names[], int n)
    {
        ...
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
            ...
            if (!strcmp(name, ActiveDeviceTP.name))
            {
                ...
                strncpy(EqNP.device, ActiveDeviceT[0].text, MAXLITHIUMDEVICE);
                strncpy(LocationNP.device, ActiveDeviceT[0].text, MAXLITHIUMDEVICE);
                strncpy(ScopeParametersNP.device, ActiveDeviceT[0].text, MAXLITHIUMDEVICE);
    ...
    {
        LITHIUM::DefaultDevice::initProperties(); //  let the base class flesh in what it wants
    ...
    {
        strncpy(integrationExtention, ext, MAXLITHIUMBLOBFMT);
        if (HasDSP())
    ...
    void SensorInterface::addFITSKeywords(fitsfile *fptr, uint8_t *buf, int len)
    {
        ...
        // SENSOR
        strncpy(fitsString, getDeviceName(), MAXLITHIUMDEVICE);
        fits_update_key_s(fptr, TSTRING, "INSTRUME", fitsString, "Sensor Name", &status);

        // Telescope
        strncpy(fitsString, ActiveDeviceT[0].text, MAXLITHIUMDEVICE);
        fits_update_key_s(fptr, TSTRING, "TELESCOP", fitsString, "Telescope name", &status);

        // Observer
        strncpy(fitsString, FITSHeaderT[FITS_OBSERVER].text, MAXLITHIUMDEVICE);
        fits_update_key_s(fptr, TSTRING, "OBSERVER", fitsString, "Observer name", &status);

        // Object
        strncpy(fitsString, FITSHeaderT[FITS_OBJECT].text, MAXLITHIUMDEVICE);
        ...
        if (Latitude != -1000 && Longitude != -1000 && Elevation != -1000)
        {
            ...
            snprintf(el_str, MAXLITHIUMFORMAT, "%lf", Elevation);
                                     ...
                                     bool saveIntegration)
    {
        ...
        snprintf(FitsB.format, MAXLITHIUMBLOBFMT, ".%s", getIntegrationFileExtension());
        if (saveIntegration)
        {
    ...
    {
        LITHIUM::DefaultDevice::saveConfigItems(fp);
