...
namespace LITHIUM
{
    ...
    bool Telescope::ISSnoopDevice(XMLEle *root)
    {
        ...
        if (isConnected())
        {
            ...
            else if (HasTime() && !strcmp(propName, "TIME_UTC"))
            {
                ...
                char utc[MAXLITHIUMTSTAMP], offset[MAXLITHIUMTSTAMP];
                ...
                for (ep = nextXMLEle(root, 1); ep != nullptr; ep = nextXMLEle(root, 0))
                {
                    ...
                    if (!strcmp(elemName, "UTC"))
                        strncpy(utc, pcdataXMLEle(ep), MAXLITHIUMTSTAMP);
                    else if (!strcmp(elemName, "OFFSET"))
                        strncpy(offset, pcdataXMLEle(ep), MAXLITHIUMTSTAMP);
                }
            ...
            else if (!strcmp(propName, "DOME_PARK") /* || !strcmp(propName, "DOME_SHUTTER")*/)
            {
                ...
                // Mount shouldn't park due to dome closing in LITHIUM::Telescope
#if 0
            if (strcmp(findXMLAttValu(root, "state"), "Ok"))
            {
    ...
    bool Telescope::MoveNS(LITHIUM_DIR_NS dir, TelescopeMotionCommand command)
    {
        LITHIUM_UNUSED(dir);
        LITHIUM_UNUSED(command);
    ...
    bool Telescope::MoveWE(LITHIUM_DIR_WE dir, TelescopeMotionCommand command)
    {
        LITHIUM_UNUSED(dir);
        LITHIUM_UNUSED(command);
    ...
    ***************************************************************************************/
    ...
    void Telescope::updateObserverLocation(double latitude, double longitude, double elevation)
    {
        ...
        m_Location.latitude = latitude;
        m_Location.elevation = elevation;
        char lat_str[MAXLITHIUMFORMAT] = {0}, lng_str[MAXLITHIUMFORMAT] = {0};

        // Make display longitude to be in the standard 0 to +180 East, and 0 to -180 West.
        // No need to confuse new users with LITHIUM format.
    ...
    {
        LITHIUM_UNUSED(Axis1Value);
        LITHIUM_UNUSED(Axis2Value);
        return true;
    }
    ...
    void Telescope::SetTelescopeCapability(uint32_t cap, uint8_t slewRateCount)
    {
        ...
        if (nSlewRate >= 4)
        {
            ...
            //        strncpy((SlewRateS + (step * 0))->name, "SLEW_GUIDE", MAXLITHIUMNAME);
            //        strncpy((SlewRateS + (step * 1))->name, "SLEW_CENTERING", MAXLITHIUMNAME);
            //        strncpy((SlewRateS + (step * 2))->name, "SLEW_FIND", MAXLITHIUMNAME);
            //        strncpy((SlewRateS + (nSlewRate - 1))->name, "SLEW_MAX", MAXLITHIUMNAME);

            // If number of slew rate is EXACTLY 4, then let's use common labels
            if (nSlewRate == 4)
            {
                strncpy((SlewRateS + (0))->label, "Guide", MAXLITHIUMLABEL);
                strncpy((SlewRateS + (1))->label, "Centering", MAXLITHIUMLABEL);
                strncpy((SlewRateS + (2))->label, "Find", MAXLITHIUMLABEL);
                strncpy((SlewRateS + (3))->label, "Max", MAXLITHIUMLABEL);
            }
    ...
    Telescope::TelescopePierSide Telescope::expectedPierSide(double ra)
    {
        ...
        if (!HasPierSide() && !HasPierSideSimulation())
            return LITHIUM::Telescope::PIER_UNKNOWN;
        ...
        return hourAngle <= 0 ? LITHIUM::Telescope::PIER_WEST : LITHIUM::Telescope::PIER_EAST;
    }
