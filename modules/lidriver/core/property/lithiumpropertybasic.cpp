...
#include "lithiumpropertybasic.h"
#include "lithiumpropertybasic_p.h"
#include <cassert>
...
namespace LITHIUM
{
...
#ifdef LITHIUM_PROPERTY_RAW_CAST
        : PropertyContainer<T>{
              *new PropertyView<T>()},
          PropertyPrivate(&this->typedProperty), raw{false}
#else
    ...
    PropertyBasicPrivateTemplate<T>::PropertyBasicPrivateTemplate(RawPropertyType *rawProperty)
        : PropertyContainer<T>{
          ...
          PropertyPrivate(PropertyView<T>::cast(rawProperty)), raw{true}
    {
    }
#endif
    ...
    {
        D_PTR(PropertyBasic);
        return static_cast<PropertyView<T> *>(static_cast<LITHIUM::PropertyPrivate *>(d)->property);
    }

    template <typename T>
    LITHIUM::PropertyView<T> PropertyBasic<T>::operator*()
    {
        D_PTR(PropertyBasic);
        return *static_cast<PropertyView<T> *>(static_cast<LITHIUM::PropertyPrivate *>(d)->property);
    }
