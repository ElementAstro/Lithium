...
{
    char message[MAXLITHIUMMESSAGE];
    if (fmt)
    {
        vsnprintf(message, MAXLITHIUMMESSAGE, fmt, ap);
...
{
    userio_printf(io, user, "<getProperties version='%g'", LITHIUMV); // safe
    // special case for LITHIUM::BaseClient::listenLITHIUM LITHIUM::BaseClientQt::connectServer
    if (dev && dev[0])
    {
    ...
    const userio *io, void *user, int ctag)
{
    ...
    if (ctag == 0)
    {
        userio_prints(io, user, "<LITHIUMDriver>\n");
    }
    /* Closing tag */
    else
    {
