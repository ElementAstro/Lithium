...
namespace LITHIUM
{
    ...
    public:
        ...
        virtual void init(LITHIUM::DefaultDevice *mainDevice);

        virtual bool setPixelFormat(LITHIUM_PIXEL_FORMAT pixelFormat, uint8_t pixelDepth);

        virtual bool setSize(uint16_t width, uint16_t height);

        virtual bool upload(LITHIUM::WidgetViewBlob *bp, const uint8_t *buffer, uint32_t nbytes, bool isCompressed = false) = 0;

        const char *getName();

    protected:
        LITHIUM::DefaultDevice *currentDevice;
        const char *name;
        LITHIUM_PIXEL_FORMAT pixelFormat; // LITHIUM Pixel Format
