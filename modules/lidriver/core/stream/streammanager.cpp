...
namespace LITHIUM
{
    ...
    bool StreamManagerPrivate::initProperties()
    {
        ...
        if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::SENSOR_INTERFACE)
        ...
        if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::CCD_INTERFACE)
        {
            // CCD Streaming Frame
    ...
    StreamManagerPrivate::FrameInfo StreamManagerPrivate::updateSourceFrameInfo()
    {
        ...
        uint8_t components = (PixelFormat == LITHIUM_RGB) ? 3 : 1;
        ...
        if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::CCD_INTERFACE)
        {
            srcFrameInfo = FrameInfo(
                dynamic_cast<const LITHIUM::CCD *>(currentDevice)->PrimaryCCD,
                components * bytesPerComponent);
        }
        else if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::SENSOR_INTERFACE)
        {
            srcFrameInfo = FrameInfo(
                *dynamic_cast<const LITHIUM::SensorInterface *>(currentDevice),
                components * bytesPerComponent);
        }
        ...
        {
            // if (dynamic_cast<LITHIUM::CCD*>(currentDevice)->PrimaryCCD.getNAxis() == 2)
            //     binFactor = dynamic_cast<LITHIUM::CCD*>(currentDevice)->PrimaryCCD.getBinX();
    ...
    void StreamManagerPrivate::asyncStreamThread()
    {
        ...
        LITHIUM::SingleThreadPool previewThreadPool;
        LITHIUM::ElapsedTimer previewElapsed;
        ...
        while (!framesThreadTerminate)
        {
            ...
            if (PixelFormat != LITHIUM_JPG && sourceBuffer->size() != srcFrameInfo.totalSize())
            {
            ...
            if (isStreaming && FPSPreview.newFrame())
            {
                ...
                if (PixelFormat != LITHIUM_JPG && PixelDepth > 8)
                {
    ...
    bool StreamManagerPrivate::setPixelFormat(LITHIUM_PIXEL_FORMAT pixelFormat, uint8_t pixelDepth)
    {
    ...
    bool StreamManager::setPixelFormat(LITHIUM_PIXEL_FORMAT pixelFormat, uint8_t pixelDepth)
    {
        D_PTR(StreamManager);
    ...
    bool StreamManagerPrivate::startRecording()
    {
        ...
        if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::CCD_INTERFACE)
        {
            ...
            if (dynamic_cast<LITHIUM::CCD *>(currentDevice)->CurrentFilterSlot != -1 && dynamic_cast<LITHIUM::CCD *>(currentDevice)->CurrentFilterSlot <= static_cast<int>(dynamic_cast<LITHIUM::CCD *>(currentDevice)->FilterNames.size()))
            {
                filtername = dynamic_cast<LITHIUM::CCD *>(currentDevice)->FilterNames.at(dynamic_cast<LITHIUM::CCD *>(currentDevice)->CurrentFilterSlot - 1);
    ...
    else
    {
        ...
        if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::CCD_INTERFACE)
        {
            if (isStreaming == false && dynamic_cast<LITHIUM::CCD *>(currentDevice)->StartStreaming() == false)
            {
        ...
        {
            if (isStreaming == false && dynamic_cast<LITHIUM::SensorInterface *>(currentDevice)->StartStreaming() == false)
            {
                LOG_ERROR("Failed to start recording.");
                RecordStreamSP.setState(IPS_ALERT);
                RecordStreamSP.reset();
    ...
    bool StreamManagerPrivate::stopRecording(bool force)
    {
        ...
        {
            if (!isStreaming)
                dynamic_cast<LITHIUM::CCD *>(currentDevice)->StopStreaming();
        }
        else if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::SENSOR_INTERFACE)
        {
            if (!isStreaming)
                dynamic_cast<LITHIUM::SensorInterface *>(currentDevice)->StopStreaming();
        }
    ...
    bool StreamManagerPrivate::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
    {
        ...
        if (StreamFrameNP.isNameMatch(name))
        {
            ...
            if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::CCD_INTERFACE)
            {
                srcFrameInfo = FrameInfo(dynamic_cast<const LITHIUM::CCD *>(currentDevice)->PrimaryCCD);
            }
            else if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::SENSOR_INTERFACE)
            {
                srcFrameInfo = FrameInfo(*dynamic_cast<const LITHIUM::SensorInterface *>(currentDevice));
            }
    ...
    bool StreamManagerPrivate::setStream(bool enable)
    {
        if (enable)
        {
            ...
            if (StreamOptionsN[OPTION_RATE_DIVISOR].value > 0)
                DEBUGF(LITHIUM::Logger::DBG_SESSION,
                ...
                if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::CCD_INTERFACE)
                {
                    if (dynamic_cast<LITHIUM::CCD *>(currentDevice)->StartStreaming() == false)
                    {
                        StreamSP.reset();
                ...
                {
                    if (dynamic_cast<LITHIUM::SensorInterface *>(currentDevice)->StartStreaming() == false)
                    {
                        StreamSP.reset();
                        StreamSP[1].setState(ISS_ON);
                        StreamSP.setState(IPS_ALERT);
        ...
        else
        {
            ...
            if (isStreaming)
            {
                if (!isRecording)
                {
                    if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::CCD_INTERFACE)
                    {
                        if (dynamic_cast<LITHIUM::CCD *>(currentDevice)->StopStreaming() == false)
                        {
                            StreamSP.setState(IPS_ALERT);
                    ...
                    {
                        if (dynamic_cast<LITHIUM::SensorInterface *>(currentDevice)->StopStreaming() == false)
                        {
                            StreamSP.setState(IPS_ALERT);
                            LOG_ERROR("Failed to stop streaming.");
                            StreamSP.apply();
    ...
    bool StreamManagerPrivate::uploadStream(const uint8_t *buffer, uint32_t nbytes)
    {
        ...
        if (PixelFormat == LITHIUM_JPG)
        {
            ...
            if (dynamic_cast<LITHIUM::CCD *>(currentDevice)->HasWebSocket() && dynamic_cast<LITHIUM::CCD *>(currentDevice)->WebSocketS[CCD::WEBSOCKET_ENABLED].s == ISS_ON)
            {
                if (Format != ".streajpg")
                {
                    Format = ".streajpg";
                    dynamic_cast<LITHIUM::CCD *>(currentDevice)->wsServer.send_text(Format);
                }

                dynamic_cast<LITHIUM::CCD *>(currentDevice)->wsServer.send_binary(buffer, nbytes);
                return true;
            }
#endif
    ...
    if (dynamic_cast<LITHIUM::CCD*>(currentDevice)->PrimaryCCD.getNAxis() == 2)
    {
        dynamic_cast<LITHIUM::CCD*>(currentDevice)->PrimaryCCD.binFrame();
        nbytes /= dynamic_cast<LITHIUM::CCD*>(currentDevice)->PrimaryCCD.getBinX() * dynamic_cast<LITHIUM::CCD*>
        ...
        if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::CCD_INTERFACE)
        {
            if (encoder->upload(&imageBP[0], buffer, nbytes, dynamic_cast<LITHIUM::CCD *>(currentDevice)->PrimaryCCD.isCompressed()))
            {
#ifdef HAVE_WEBSOCKET
                if (dynamic_cast<LITHIUM::CCD *>(currentDevice)->HasWebSocket() && dynamic_cast<LITHIUM::CCD *>(currentDevice)->WebSocketS[CCD::WEBSOCKET_ENABLED].s == ISS_ON)
                {
        ...
        else if (currentDevice->getDriverInterface() & LITHIUM::DefaultDevice::SENSOR_INTERFACE)
        {
            if (encoder->upload(&imageBP[0], buffer, nbytes,
                                false)) // dynamic_cast<LITHIUM::SensorInterface*>(currentDevice)->isCompressed()))
            {
                // Upload to client now
                imageBP.setState(IPS_OK);
                imageBP.apply();
