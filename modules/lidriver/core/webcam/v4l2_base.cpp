...
namespace LITHIUM
{
    ...
    V4L2_Base::V4L2_Base()
    {
        ...
        DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,
        ...
        for (std::vector<unsigned int>::const_iterator it = vsuppformats.begin(); it != vsuppformats.end(); ++it)
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%c%c%c%c ", (*it >> 0), (*it >> 8), (*it >> 16),
                         (*it >> 24));
        // DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,LITHIUM::Logger::DBG_SESSION,"Default decoder: %s", decoder->getName());
    ...
    bool V4L2_Base::is_compressed() const
    {
        ...
        case V4L2_PIX_FMT_MJPEG:
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: format %c%c%c%c patched to be considered compressed",
                         __FUNCTION__, fmt.fmt.pix.pixelformat, fmt.fmt.pix.pixelformat >> 8,
                         fmt.fmt.pix.pixelformat >> 16, fmt.fmt.pix.pixelformat >> 24);
            return true;

        default:
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: format %c%c%c%c has compressed flag %d",
                         __FUNCTION__, fmt.fmt.pix.pixelformat, fmt.fmt.pix.pixelformat >> 8,
                         fmt.fmt.pix.pixelformat >> 16, fmt.fmt.pix.pixelformat >> 24,
    ...
    int V4L2_Base::xioctl(int fd, int request, void *arg, char const *const request_str)
    {
        ...
        if (-1 == r)
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: ioctl 0x%08X/%s received errno %d (%s)", __FUNCTION__,
    ...
    int V4L2_Base::ioctl_set_format(struct v4l2_format new_fmt, char *errmsg)
    {
        ...
        if (streamedonce && new_fmt.type)
        {
            ...
            {
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: failed reopening device %s (%s)", __FUNCTION__, path,
                             errmsg);
                return -1;
            }
        }
        ...
        if (false)
        {
            ...
            {
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: failed VIDIOC_TRY_FMT with " DBG_STR_FMT,
        ...
        if (new_fmt.type)
        {
            ...
            {
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: failed VIDIOC_S_FMT with " DBG_STR_FMT, __FUNCTION__,
                             DBG_FMT(new_fmt));
        ...
        else
        {
            ...
            {
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: failed VIDIOC_G_FMT", __FUNCTION__);
                return errno_exit("VIDIOC_G_FMT", errmsg);
            }
        }

        DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: current format " DBG_STR_FMT, __FUNCTION__,
                     DBG_FMT(new_fmt));
    ...
    {
        LITHIUM_UNUSED(pixelFormat);
        LITHIUM_UNUSED(width);
        LITHIUM_UNUSED(height);
        selectCallBackID = -1;
        cancrop = true;
        cansetrate = true;
        streamedonce = false;
    ...
    //     //DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,"%s: epoch shift is %ld",__FUNCTION__,epoch_shift);
    //
    //     return epoch_shift;
    // }
    ...
    int V4L2_Base::read_frame(char *errmsg)
    {
        ...
        case IO_METHOD_MMAP:
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: using MMAP to recover frame buffer", __FUNCTION__);
            CLEAR(buf);
            ...
            if (false)
            {
                for (i = 0; i < n_buffers; ++i)
                {
                    ...
                    DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: " DBG_STR_BUF, __FUNCTION__, DBG_BUF(buf));
                }
            }
            ...
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: buffer #%d dequeued from fd:%d\n", __FUNCTION__,
                         buf.index, fd);
            ...
            case V4L2_BUF_FLAG_TIMESTAMP_MONOTONIC:
            {
                ...
                else if (V4L2_BUF_FLAG_TSTAMP_SRC_EOF == (buf.flags & V4L2_BUF_FLAG_TSTAMP_SRC_MASK))
                {
                    ...
                    DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: unsupported timestamp in frame",
                                 __FUNCTION__);
            ...
            default:
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: no usable timestamp found in frame",
                             __FUNCTION__);
            }
#endif
            ...
            if (dodecode)
            {
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: [%p] decoding %d-byte buffer %p cropset %c",
            ...
            if (dorecord)
            {
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%s: [%p] recording %d-byte buffer %p", __FUNCTION__,
                             recorder, buf.bytesused, buffers[buf.index].start);
                recorder->writeFrame((unsigned char *)(buffers[buf.index].start));
            }
            */

            // DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,"lxstate is %d, dropFrame %c\n", lxstate, (dropFrame?'Y':'N'));
    ...
    int V4L2_Base::start_capturing(char *errmsg)
    {
        ...
        case IO_METHOD_MMAP:
            for (i = 0; i < n_buffers; ++i)
            {
                ...
                buf.memory = V4L2_MEMORY_MMAP;
                buf.index = i;
                // DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,"v4l2_start_capturing: enqueuing buffer %d for fd=%d\n", buf.index, fd);
                /*if (-1 == XIOCTL(fd, VIDIOC_QBUF, &buf))
    ...
    int V4L2_Base::check_device(char *errmsg)
    {
        ...
        DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Driver %.*s (version %u.%u.%u)", (int)sizeof(cap.driver),
                     cap.driver, (cap.version >> 16) & 0xFF, (cap.version >> 8) & 0xFF, (cap.version & 0xFF));
        DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  card: %.*s", (int)sizeof(cap.card), cap.card);
        DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  bus:  %.*s", (int)sizeof(cap.bus_info), cap.bus_info);
        ...
        //  DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,"ioctl: can't set pwc video quality to High (uncompressed).\n");
        //}
        // else
        //  DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,"  Setting pwc video quality to High (uncompressed)\n");
        // setframerate=&V4L2_Base::pwcsetframerate;
        //    }

        DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Driver capabilities:");
        if (cap.capabilities & V4L2_CAP_VIDEO_CAPTURE)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_VIDEO_CAPTURE");
        if (cap.capabilities & V4L2_CAP_VIDEO_OUTPUT)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_VIDEO_OUTPUT");
        if (cap.capabilities & V4L2_CAP_VIDEO_OVERLAY)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_VIDEO_OVERLAY");
        if (cap.capabilities & V4L2_CAP_VBI_CAPTURE)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_VBI_CAPTURE");
        if (cap.capabilities & V4L2_CAP_VBI_OUTPUT)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_VBI_OUTPUT");
        if (cap.capabilities & V4L2_CAP_SLICED_VBI_CAPTURE)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_SLICED_VBI_CAPTURE");
        if (cap.capabilities & V4L2_CAP_SLICED_VBI_OUTPUT)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_SLICED_VBI_OUTPUT");
        if (cap.capabilities & V4L2_CAP_RDS_CAPTURE)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_RDS_CAPTURE");
        if (cap.capabilities & V4L2_CAP_VIDEO_OUTPUT_OVERLAY)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_VIDEO_OUTPUT_OVERLAY");
        if (cap.capabilities & V4L2_CAP_TUNER)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_TUNER");
        if (cap.capabilities & V4L2_CAP_AUDIO)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_AUDIO");
        if (cap.capabilities & V4L2_CAP_RADIO)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_RADIO");
        if (cap.capabilities & V4L2_CAP_READWRITE)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_READWRITE");
        if (cap.capabilities & V4L2_CAP_ASYNCIO)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_ASYNCIO");
        if (cap.capabilities & V4L2_CAP_STREAMING)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "  V4L2_CAP_STREAMING");
        /*if (cap.capabilities & V4L2_CAP_EXT_PIX_FORMAT) {
          has_ext_pix_format=true;
          DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,"  V4L2_CAP_EXT_PIX_FORMAT\n");
        }*/
        if (!(cap.capabilities & V4L2_CAP_VIDEO_CAPTURE))
        {
        ...
        DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Enumerating available Inputs:");
        for (input_avail.index = 0; ioctl(fd, VIDIOC_ENUMINPUT, &input_avail) != -1; input_avail.index++)
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%2d. %.*s (type %s)%s", input_avail.index,
        ...
        if (errno != EINVAL)
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "(problem enumerating inputs)");
        ...
        if (cancrop)
        {
            ...
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, " Crop capabilities: pixelaspect = %d / %d",
                         cropcap.pixelaspect.numerator, cropcap.pixelaspect.denominator);
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Explicitly resetting crop area to default...");
        ...
        // Enumerating capture format
        {
            ...
            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Enumerating available Capture Image formats:");
            for (fmt_avail.index = 0; ioctl(fd, VIDIOC_ENUM_FMT, &fmt_avail) != -1; fmt_avail.index++)
            {
                // DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,LITHIUM::Logger::DBG_SESSION,"\t%d. %s (%c%c%c%c) %s\n", fmt_avail.index, fmt_avail.description, (fmt_avail.pixelformat)&0xFF, (fmt_avail.pixelformat >> 8)&0xFF,
                ...
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%2d. Format %.*s (%c%c%c%c) is %s", fmt_avail.index,
                             ...
                             (decoder->issupportedformat(fmt_avail.pixelformat) ? "supported" : "UNSUPPORTED"));
                {
                    ...
                    DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,
                         ...
                         frm_sizeenum.index++)
                    {
                        ...
                        case V4L2_FRMSIZE_TYPE_CONTINUOUS:
                            ...
                            break;
                        default:
                            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "        Unknown Frame size type: %d\n",
                                         frm_sizeenum.type);
                            break;
                        }
                        {
                            ...
                            DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "        Frame intervals:");
                                 ...
                                 frmi_valenum.index++)
                            {
                                ...
                                case V4L2_FRMIVAL_TYPE_DISCRETE:
                                    DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "        %d/%d s",
                                                 frmi_valenum.discrete.numerator, frmi_valenum.discrete.denominator);
                                    break;
                                case V4L2_FRMIVAL_TYPE_STEPWISE:
                                    DEBUGFDEVICE(
                                        deviceName, LITHIUM::Logger::DBG_DEBUG,
                            ...
                            // DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,"error %d, %s\n", errno, strerror (errno));
                        }
                    }
                }
            }
            if (errno != EINVAL)
                DEBUGDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Problem enumerating capture formats.");
    ...
    void V4L2_Base::getinputs(ISwitchVectorProperty *inputssp)
    {
        ...
        for (input_avail.index = 0; (int)input_avail.index < enumeratedInputs; input_avail.index++)
        {
            ...
            strncpy(inputs[input_avail.index].name, (const char *)input_avail.name, MAXLITHIUMNAME);
            strncpy(inputs[input_avail.index].label, (const char *)input_avail.name, MAXLITHIUMLABEL);
        }
        ...
        DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Current video input is   %d. %.*s", input.index,
                     (int)sizeof(inputs[input.index].name), inputs[input.index].name);
    }

    int V4L2_Base::setinput(unsigned int inputindex, char *errmsg)
    {
        DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Setting Video input to %d", inputindex);
        if (streamedonce)
        {
            close_device();
    ...
    void V4L2_Base::getcaptureformats(ISwitchVectorProperty *captureformatssp)
    {
        ...
        for (fmt_avail.index = 0; (int)fmt_avail.index < enumeratedCaptureFormats; fmt_avail.index++)
        {
            ...
            strncpy(formats[fmt_avail.index].name, (const char *)fmt_avail.description, MAXLITHIUMNAME);
            strncpy(formats[fmt_avail.index].label, (const char *)fmt_avail.description, MAXLITHIUMLABEL);
        ...
        for (unsigned int i = 0; i < fmt_avail.index; i++)
        {
            ...
            {
                formats[i].s = ISS_ON;
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Current capture format is %d. %c%c%c%c.", i,
    ...
    void V4L2_Base::getcapturesizes(ISwitchVectorProperty *capturesizessp, INumberVectorProperty *capturesizenp)
    {
        ...
        // DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,"\t  Available Frame sizes/rates for this format:\n");
        for (frm_sizeenum.index = 0; XIOCTL(fd, VIDIOC_ENUM_FRAMESIZES, &frm_sizeenum) != -1; frm_sizeenum.index++)
        {
            ...
            case V4L2_FRMSIZE_TYPE_DISCRETE:
                ...
                snprintf(sizes[frm_sizeenum.index].name, MAXLITHIUMNAME, "%dx%d", frm_sizeenum.discrete.width,
                         frm_sizeenum.discrete.height);
                snprintf(sizes[frm_sizeenum.index].label, MAXLITHIUMNAME, "%dx%d", frm_sizeenum.discrete.width,
                         frm_sizeenum.discrete.height);
                ...
                if (!sizefound)
                {
                        ...
                        (fmt.fmt.pix.height == frm_sizeenum.discrete.height))
                    {
                        ...
                        sizefound = true;
                        DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Current capture size is (%d.)  %dx%d",
            ...
            case V4L2_FRMSIZE_TYPE_CONTINUOUS:
                ...
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Current capture size is %dx%d", fmt.fmt.pix.width,
                             fmt.fmt.pix.height);
                break;
            default:
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Unknown Frame size type: %d", frm_sizeenum.type);
                break;
            }
        }
    ...
    void V4L2_Base::getframerates(ISwitchVectorProperty *frameratessp, INumberVectorProperty *frameratenp)
    {
        ...
        for (frmi_valenum.index = 0; XIOCTL(fd, VIDIOC_ENUM_FRAMEINTERVALS, &frmi_valenum) != -1; frmi_valenum.index++)
        {
            ...
            case V4L2_FRMIVAL_TYPE_DISCRETE:
                ...
                snprintf(rates[frmi_valenum.index].name, MAXLITHIUMNAME, "%d/%d", frmi_valenum.discrete.numerator,
                         frmi_valenum.discrete.denominator);
                snprintf(rates[frmi_valenum.index].label, MAXLITHIUMNAME, "%d/%d", frmi_valenum.discrete.numerator,
                         frmi_valenum.discrete.denominator);
                if ((frate.numerator == frmi_valenum.discrete.numerator) &&
                    (frate.denominator == frmi_valenum.discrete.denominator))
                {
                    DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Current frame interval is %d/%d",
                                 frmi_valenum.discrete.numerator, frmi_valenum.discrete.denominator);
            ...
            case V4L2_FRMIVAL_TYPE_CONTINUOUS:
                ...
                break;
            default:
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Unknown Frame rate type: %d", frmi_valenum.type);
                break;
            }
        }
    ...
    int V4L2_Base::setcroprect(int x, int y, int w, int h, char *errmsg)
    {
        ...
        if (cancrop)
        {
            ...
            {
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_WARNING,
            ...
            {
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_SESSION, "V4L2 hardware crop request 0x%08X accepted as %dx%d at (%d, %d)",
                             (unsigned int)VIDIOC_S_CROP, hardware_crop.c.width, hardware_crop.c.height,
                             hardware_crop.c.left, hardware_crop.c.top);
            }
        }
        ...
        DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "V4L2 base setcroprect %dx%d at (%d, %d)", crop.c.width,
        ...
        for (queryctrl.id = V4L2_CID_BASE; queryctrl.id < V4L2_CID_LASTP1; queryctrl.id++)
        {
            if (0 == ioctl(fd, VIDIOC_QUERYCTRL, &queryctrl))
            {
                ...
                if (queryctrl.type == V4L2_CTRL_TYPE_INTEGER)
                {
                    ...
                    strncpy(numbers[nnum].name, (const char *)entityXML((char *)queryctrl.name), MAXLITHIUMNAME);
                    strncpy(numbers[nnum].label, (const char *)entityXML((char *)queryctrl.name), MAXLITHIUMLABEL);
                    strncpy(numbers[nnum].format, "%0.f", MAXLITHIUMFORMAT);
                ...
                if (queryctrl.type == V4L2_CTRL_TYPE_BOOLEAN)
                {
                    ...
                    *(unsigned int *)(opt[nopt].aux) = (queryctrl.id);
                    ...
                    DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Adding switch  %.*s (%s)\n",
...
#endif
                {
                    ...
                    for (querymenu.index = queryctrl.minimum; (int)querymenu.index <= queryctrl.maximum; querymenu.index++)
                    {
                        if (0 == XIOCTL(fd, VIDIOC_QUERYMENU, &querymenu))
                        {
                            ...
                            sname[31] = '\0';
#endif
                            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Adding menu item %.*s %.*s item %d",
                    ...
                    *(unsigned int *)(opt[nopt].aux) = (queryctrl.id);
                    ...
                    DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Adding menu  %.*s (item %d set)",
            ...
            if (0 == ioctl(fd, VIDIOC_QUERYCTRL, &queryctrl))
            {
                ...
                if (queryctrl.type == V4L2_CTRL_TYPE_BOOLEAN)
                {
                    ...
                    *(unsigned int *)(opt[nopt].aux) = (queryctrl.id);
                    ...
                    DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Adding ext. switch  %.*s (%s)\n",
                                 (int)sizeof(queryctrl.name), queryctrl.name, (control.value ? "On" : "Off"));
                    nopt += 1;
                }
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0))
...
#endif
                {
                    ...
                    *(unsigned int *)(opt[nopt].aux) = (queryctrl.id);
                    ...
                    DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Adding ext. menu  %.*s (item %d set)",
                                 (int)sizeof(queryctrl.name), queryctrl.name, control.value);
                    nopt += 1;
                }
            }
        ...
        for (queryctrl.id = V4L2_CID_BASE; queryctrl.id < V4L2_CID_LASTP1; queryctrl.id++)
        {
            if (0 == ioctl(fd, VIDIOC_QUERYCTRL, &queryctrl))
            {
                ...
                if (queryctrl.type == V4L2_CTRL_TYPE_INTEGER)
                {
                    ...
                    strncpy(numbers[nnum].name, ((char *)queryctrl.name), MAXLITHIUMNAME);
                    strncpy(numbers[nnum].label, ((char *)queryctrl.name), MAXLITHIUMLABEL);
                    strncpy(numbers[nnum].format, "%0.f", MAXLITHIUMFORMAT);
                    numbers[nnum].min = queryctrl.minimum;
                    numbers[nnum].max = queryctrl.maximum;
                    numbers[nnum].step = queryctrl.step;
                    ...
                    DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%.*s -- min: %d max: %d step: %d value: %d",
    ...
    int V4L2_Base::setINTControl(unsigned int ctrl_id, double new_value, char *errmsg)
    {
        ...
        {
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_WARNING, "Setting INT control %.*s will fail, currently %s%s%s%s",
        ...
        {
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_ERROR, "Setting INT control %.*s failed (%s)",
    ...
    int V4L2_Base::setOPTControl(unsigned int ctrl_id, unsigned int new_value, char *errmsg)
    {
        ...
        if ((queryctrl.flags & V4L2_CTRL_FLAG_READ_ONLY) || (queryctrl.flags & V4L2_CTRL_FLAG_GRABBED) ||
            (queryctrl.flags & V4L2_CTRL_FLAG_INACTIVE) || (queryctrl.flags & V4L2_CTRL_FLAG_VOLATILE))
        {
            DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Setting OPT control %.*s will fail, currently %s%s%s%s",
                         (int)sizeof(queryctrl.name), queryctrl.name,
                         queryctrl.flags & V4L2_CTRL_FLAG_READ_ONLY ? "read only " : "",
                         queryctrl.flags & V4L2_CTRL_FLAG_GRABBED ? "grabbed " : "",
                         queryctrl.flags & V4L2_CTRL_FLAG_INACTIVE ? "inactive " : "",
                                     ...
                                     unsigned int *noptions, const char *dev, const char *group)
    {
        ...
        *nnumber = 0;
        ...
        while (0 == XIOCTL(fd, VIDIOC_QUERYCTRL, &queryctrl))
        {
            if (queryctrl.type == V4L2_CTRL_TYPE_CTRL_CLASS)
            {
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Control Class %.*s", (int)sizeof(queryctrl.name),
                             queryctrl.name);
                queryctrl.id |= V4L2_CTRL_FLAG_NEXT_CTRL;
                continue;
            }
            ...
            if (queryctrl.flags & V4L2_CTRL_FLAG_DISABLED)
            {
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "%.*s is disabled.", (int)sizeof(queryctrl.name),
                             queryctrl.name);
                queryctrl.id |= V4L2_CTRL_FLAG_NEXT_CTRL;
                continue;
            }
            ...
            if (queryctrl.type == V4L2_CTRL_TYPE_INTEGER)
            {
                ...
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Adding %.*s -- min: %d max: %d step: %d value: %d",
                             (int)sizeof(queryctrl.name), queryctrl.name, queryctrl.minimum, queryctrl.maximum,
                             queryctrl.step, numbers[nnum].value);
            ...
            if (queryctrl.type == V4L2_CTRL_TYPE_BOOLEAN)
            {
                ...
                *(unsigned int *)(opt[nopt].aux) = (queryctrl.id);
                ...
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Adding switch  %.*s (%s)", (int)sizeof(queryctrl.name),
            ...
            if (queryctrl.type == V4L2_CTRL_TYPE_BUTTON)
            {
                ...
                opt[nopt].aux = malloc(sizeof(unsigned int));
                *(unsigned int *)(opt[nopt].aux) = (queryctrl.id);
                DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Adding Button %.*s", (int)sizeof(queryctrl.name),
                             queryctrl.name);
                nopt += 1;
            }
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 5, 0))
...
#endif
            {
                ...
                for (querymenu.index = queryctrl.minimum; (int)querymenu.index <= queryctrl.maximum; querymenu.index++)
                {
                    if (0 == XIOCTL(fd, VIDIOC_QUERYMENU, &querymenu))
                    {
                        ...
                        sname[31] = '\0';
#endif
                        DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG, "Adding menu item %.*s %.*s item %d index %d",
                                     (int)sizeof(sname), sname, (int)sizeof(menuoptname), menuoptname, nmenuopt,
                                     querymenu.index);
                        // IUFillSwitch(&sw[nmenuopt], menuoptname, (const char *)sname, (control.value==nmenuopt?ISS_ON:ISS_OFF));
                        IUFillSwitch(&sw[nmenuopt], menuoptname, (const char *)entityXML((char *)querymenu.name),
            ...
            // if (queryctrl.type == V4L2_CTRL_TYPE_INTEGER_MENU)
            //	{
            //   DEBUGFDEVICE(deviceName, LITHIUM::Logger::DBG_DEBUG,"Control type not implemented\n");
            // }
    ...
    {
        strncpy(deviceName, name, MAXLITHIUMDEVICE);
    }
