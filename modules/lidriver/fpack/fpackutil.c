...
int fp_info_hdu(fitsfile *infptr)
{
    ...
    for (hdupos = 1; !stat; hdupos++)
    {
        ...
        {
            snprintf(msg, SZ_STR, "  %d IMAGE", hdupos);
            fp_msg(msg);
            snprintf(msg, SZ_STR, " SUMS=%lu/%lu", (unsigned long)(~((int)hdusum)), datasum);
            fp_msg(msg);

            fits_get_img_param(infptr, 9, &bitpix, &naxis, naxes, &stat);

            snprintf(msg, SZ_STR, " BITPIX=%d", bitpix);
            fp_msg(msg);

            if (naxis == 0)
            {
                snprintf(msg, SZ_STR, " [no_pixels]");
                fp_msg(msg);
            }
            else if (naxis == 1)
            {
                snprintf(msg, SZ_STR, " [%ld]", naxes[1]);
                fp_msg(msg);
            }
            else
            {
                snprintf(msg, SZ_STR, " [%ld", naxes[0]);
                fp_msg(msg);
                for (ii = 1; ii < naxis; ii++)
                {
                    snprintf(msg, SZ_STR, "x%ld", naxes[ii]);
                    fp_msg(msg);
                }
                fp_msg("]");
            }
        ...
        {
            snprintf(msg, SZ_STR, "  %d ASCII_TBL", hdupos);
            fp_msg(msg);
            snprintf(msg, SZ_STR, " SUMS=%lu/%lu\n", (unsigned long)(~((int)hdusum)), datasum);
            fp_msg(msg);
        }
        else if (hdutype == BINARY_TBL)
        {
            snprintf(msg, SZ_STR, "  %d BINARY_TBL", hdupos);
            fp_msg(msg);
            snprintf(msg, SZ_STR, " SUMS=%lu/%lu\n", (unsigned long)(~((int)hdusum)), datasum);
            fp_msg(msg);
        }
        ...
        {
            snprintf(msg, SZ_STR, "  %d OTHER", hdupos);
            fp_msg(msg);
            snprintf(msg, SZ_STR, " SUMS=%lu/%lu", (unsigned long)(~((int)hdusum)), datasum);
            fp_msg(msg);
...
int fp_preflight(int argc, char *argv[], int unpack, fpstate *fpptr)
{
    ...
    for (iarg = fpptr->firstfile; iarg < argc; iarg++)
    {
        ...
        if (unpack)
        {
            ...
            { /* if not reading from stdin stream */
                namelen = strlen(outfits);
                if (!strcmp(".fz", outfits + namelen - 3))
                { /* suffix is present */
                    outfits[namelen - 3] = '\0';
                }
                else if (fpptr->delete_suffix)
...
int fp_loop(int argc, char *argv[], int unpack, char *output_filename, fpstate fpvar)
{
    ...
    for (iarg = fpvar.firstfile; iarg < argc; iarg++)
    {
        ...
        else if (fpvar.clobber || fpvar.delete_input)
                          ...
                          float *whole_cpu, float *row_elapse, float *row_cpu, int *status)
{
    ...
    *whole_cpu = 0;
    ...
    if (bitpix == BYTE_IMG)
    {
        ...
        marktime(status);
        fits_read_subset(infptr, 11, fpixel, lpixel, inc, &cnull,
        ...
        if (row_elapse)
        {
            ...
            {
                fpixel[1] = ii + 1;
                fits_read_pix(infptr, 11, fpixel, naxes[0], &cnull,
                              carray, &anynull, status);
            }
            /* get elapsped times */
    ...
    if (whole_elapse)
        *whole_elapse = *whole_elapse / filesize;
    if (row_elapse)
        *row_elapse = *row_elapse / filesize;
    if (whole_cpu)
        *whole_cpu = *whole_cpu / filesize;
    if (row_cpu)
        *row_cpu = *row_cpu / filesize;
