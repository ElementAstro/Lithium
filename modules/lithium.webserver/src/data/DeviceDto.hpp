...
class RunDeviceFuncDTO : public oatpp::DTO {
    ...
    DTO_FIELD_INFO(task_params) {
        info->description =
            "Parameters for the task to run of the specified device.(in JSON "
            "format)";
        info->required = true;
    }
...
class ConnectDeviceDTO : public oatpp::DTO {
    ...
    DTO_FIELD_INFO(auto_reconnect) {
        info->description =
            "Whether to automatically reconnect the device (default: true).";
        info->required = false;
    }
    ...
    DTO_FIELD_INFO(times_to_connect) {
        info->description =
            "Times to connect the device (default: 3). Automatically try to "
            "connect the device 3 times at a time";
        info->required = false;
    }
...
class DisconnectDeviceDTO : public oatpp::DTO {
    ...
    DTO_FIELD_INFO(device_name) {
        info->description =
            "Name of the device to disconnect(must match the unique device "
            "name)";
        info->required = true;
    }
    DTO_FIELD(String, device_name);
    ...
    DTO_FIELD_INFO(must_success) {
        info->description =
            "If this option is true, the device will be erased from device "
            "vector until it is disconnected successfully";
        info->required = false;
    }
