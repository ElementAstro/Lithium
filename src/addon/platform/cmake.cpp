...
auto CMakeBuilder::cleanProject(const fs::path& buildDir) -> BuildResult {
    if (!fs::exists(buildDir)) {
        return {false, "",
...
auto CMakeBuilder::installProject(const fs::path& buildDir,
...
auto CMakeBuilder::generateDocs(const fs::path& buildDir,
...
auto CMakeBuilder::buildTarget(const fs::path& buildDir,
...
auto CMakeBuilder::setCacheVariable(const fs::path& buildDir,
...
auto CMakeBuilder::runTests(const fs::path& buildDir,
        ...
        command += " -R \"" +
                   std::accumulate(
                       ...
                       [](const std::string& first, const std::string& second) {
                           return first + (first.empty() ? "" : "|") + second;
                       }) +
                   "\"";
    }
...
auto CMakeBuilder::loadConfig(const fs::path& configPath) -> bool {
...
auto CMakeBuilder::getAvailableTargets(const fs::path& buildDir)
...
auto CMakeBuilder::getCacheVariables(const fs::path& buildDir)
