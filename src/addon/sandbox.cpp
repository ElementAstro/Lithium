...
bool Sandbox::run() {
    ...
    BOOL success = CreateProcess(NULL, const_cast<char *>(commandLine.c_str()),
    ...
    if (m_memoryLimit > 0) {
        ...
        success = SetProcessWorkingSetSize(processHandle, memoryLimitKb,
                                           memoryLimitKb);
        if (!success) {
            return false;
        }
...
int Sandbox::getTimeUsed() const { return m_timeUsed; }

long Sandbox::getMemoryUsed() const { return m_memoryUsed; }
}  // namespace Lithium
