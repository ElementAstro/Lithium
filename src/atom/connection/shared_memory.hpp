...
#endif
{
    ...
    if (is_creator_) {
        handle_ =
            CreateFileMappingA(INVALID_HANDLE_VALUE, nullptr, PAGE_READWRITE, 0,
                               sizeof(T) + sizeof(bool), name.c_str());
        if (handle_ == nullptr) {
    ...
    buffer_ = mmap(nullptr, sizeof(T) + sizeof(bool), PROT_READ | PROT_WRITE,
                   MAP_SHARED, fd_, 0);
    close(fd_);
...
T SharedMemory<T>::read(std::chrono::milliseconds timeout) const {
    ...
    T data;
    std::memcpy(&data, static_cast<const char *>(buffer_) + sizeof(bool),
                sizeof(T));
