...
{
public:
    SocketHub() : running(false) {}

    void start(int port)
    {
        if (running)
        {
            std::cout << "SocketHub is already running." << std::endl;
            return;
        }
        ...
        running = true;
        std::cout << "SocketHub started on port " << port << std::endl;

        // 启动接受连接的线程
        acceptThread = std::thread(&SocketHub::acceptConnections, this);
    }
...
private:
    ...
    void acceptConnections()
    {
        while (running)
        {
            ...
            // 启动处理客户端消息的线程
            std::thread clientThread(&SocketHub::handleClientMessages, this, clientSocket);
            clientThread.detach();
        }
    }
