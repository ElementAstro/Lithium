...
namespace Atom::Connection
{
     ...
     *
     * The SocketHub class offers functionalities to start and stop a socket service,
     * while managing multiple client connections. It supports accepting connections
     * from clients and starts a thread for each client to handle messages.
     */
    class SocketHub
    {
    public:
    ...
    private:
        ...
        std::vector<std::unique_ptr<std::jthread>> clientThreads; ///< 客户端处理线程列表。
                                                                  ///< List of threads for handling clients.
#else
        std::unique_ptr<std::thread> acceptThread;               ///< 用于接受连接的线程。
                                                                 ///< Thread for accepting connections.
        std::vector<std::unique_ptr<std::thread>> clientThreads; ///< 客户端处理线程列表。
                                                                 ///< List of threads for handling clients.
#endif
