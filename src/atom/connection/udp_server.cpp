...
UdpSocketHub::~UdpSocketHub() { stop(); }
...
void UdpSocketHub::start(int port) {
    ...
    m_acceptThread =
        std::make_unique<std::jthread>([this]() { this->handleMessages(); });
#else
    m_acceptThread =
        std::make_unique<std::thread>([this]() { this->handleMessages(); });
#endif
}
...
void UdpSocketHub::sendTo(const std::string &message, const std::string &ip,
    ...
    int sentBytes = sendto(
        m_serverSocket, message.c_str(), static_cast<int>(message.length()), 0,
...
void UdpSocketHub::handleMessages() {
    ...
    while (m_running.load()) {
        int bytesReceived = recvfrom(
            m_serverSocket, buffer, sizeof(buffer), 0,
