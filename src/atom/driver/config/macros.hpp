...
Description: Property Macros

*************************************************/

#ifndef ATOM_DRIVER_PROPERTY_MACROS_HPP
#define ATOM_DRIVER_PROPERTY_MACROS_HPP

/**
 * @brief Unused variable
 *
 * Indicates to the compiler that the parameter with the specified name is not
 * used in the body of a function. This can be used to suppress compiler
 * warnings while allowing functions to be defined with meaningful parameter
 * names in their signatures.
 *
 */
#ifndef INDI_UNUSED
...
/**
 * @brief The POSIX C type "ssize_t" is available on Unix-like systems in
 * unistd.h. For Windows Visual Studio based compilers without unistd.h, the
 * following stanza allows use of ssize_t.
 */
#ifdef _MSC_VER
#include <BaseTsd.h>
#ifndef HAVE_SSIZE_T
...
/**
 * @brief Expressions to test whether the attribute is recognized by the C++
 * compiler
 */
 ...
 * another case label, or user-defined label in a switch statement is
 * intentional and thus the -Wimplicit-fallthrough warning must not trigger. The
 * fallthrough attribute may appear at most once in each attribute list, and may
 * not be mixed with other attributes. It can only be used in a switch statement
 * (the compiler will issue an error otherwise), after a preceding statement and
 * before a logically succeeding case label, or user-defined label.
 *
 * switch (cond)
 * {
 * case 1:
 ...
 *
 * The D_PTR macro is part of a design pattern called the d-pointer (also called
 * the opaque pointer) where the implementation details of a library may be
 * hidden from its users and changes to the implementation can be made to a
 * library without breaking binary compatibility.
 */
#if defined(__cplusplus)
...
#define DECLARE_PRIVATE(Class)                                                \
    inline Class##Private *d_func() {                                         \
        return reinterpret_cast<Class##Private *>(getPtrHelper(this->d_ptr)); \
    }                                                                         \
    inline const Class##Private *d_func() const {                             \
        return reinterpret_cast<const Class##Private *>(                      \
            getPtrHelper(this->d_ptr));                                       \
    }                                                                         \
