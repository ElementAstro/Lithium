...
PropertyText::PropertyText(INDI::Property property)
    : PropertyBasic<IText>(
          property_private_cast<PropertyTextPrivate>(property.d_ptr)) {}

PropertyText::~PropertyText() {}

bool PropertyText::update(const char *const texts[], const char *const names[],
                          int n) {
    D_PTR(PropertyText);
    return d->typedProperty.update(texts, names, n) && (emitUpdate(), true);
}

void PropertyText::fill(const char *device, const char *name, const char *label,
                        const char *group, IPerm permission, double timeout,
                        IPState state) {
    D_PTR(PropertyText);
    d->typedProperty.setWidgets(d->widgets.data(), d->widgets.size());
    d->typedProperty.fill(device, name, label, group, permission, timeout,
                          state);
}
