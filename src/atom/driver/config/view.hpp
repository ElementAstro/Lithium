 ...
 * INDI::PropertyView
 *
 * A class that will allow a easy transition to the new widget handling
 * interface (future).
 ...
 *
 * The PropertyView<IXXX> class is compatible with low-level IXXXVectorProperty
 * structures.
 *
 * INDI::WidgetView
 *
...
struct PropertyView : PROPERTYVIEW_BASE_ACCESS WidgetTraits<T>::PropertyType {
    ...
    using enable_if_is_same_t =
        typename std::enable_if<std::is_same<X, Needed>::value, bool>::type;

public:
    PropertyView();

    // #PS: do not delete items, they may be on the stack.
    //~PropertyView()                                        { for(auto &p:
    //*this) {p.clear();} free(widget()); }
...
public:
    ...
    void setTimestamp(
        const std::string &timestamp); /* outside implementation */
...
public:  // only for INumber
    ...
    void updateMinMax(); /* outside implementation - only driver side, see
...
public:  // getters
    ...
    WidgetType *findWidgetByName(
        const char *name) const; /* outside implementation */
...
public:  // only driver side
    ...
    void vapply(const char *format,
                va_list args) const; /* outside implementation - only driver
                                        side, see indipropertyview_driver.cpp */
    void vdefine(const char *format, va_list args)
        const; /* outside implementation - only driver side, see
                  indipropertyview_driver.cpp */

    void apply(const char *format, ...) const
        ATTRIBUTE_FORMAT_PRINTF(2,
                                3); /* outside implementation - only driver
                                       side, see indipropertyview_driver.cpp */
    void define(const char *format, ...) const
        ATTRIBUTE_FORMAT_PRINTF(2,
                                3); /* outside implementation - only driver
                                       side, see indipropertyview_driver.cpp */
...
public:
    ...
    void fill(const char *device, const char *name, const char *label,
              ...
              IPState state); /* outside implementation - only driver side, see
                                 indipropertyview_driver.cpp */
    ...
    template <typename X = T, enable_if_is_same_t<X, ISwitch> = true>
    void fill(const char *device, const char *name, const char *label,
              const char *group, IPerm permission, ISRule rule, double timeout,
              IPState state); /* outside implementation - only driver side, see
                                 indipropertyview_driver.cpp */
...
public:
    ...
    /* outside implementation - only driver side, see
     * indipropertyview_driver.cpp */
    ...
    */
    ...
    bool update(const int sizes[], const int blobsizes[],
                ...
                int n); /* outside implementation - only driver side, see
                           indipropertyview_driver.cpp */
...
public:
    void clear() {
        ...
        // free(widget()); // #PS: do not delete items, they may be on the
        // stack.
...
public:
    ...
    // reinterpret_cast<const void*>(this) == nullptr; }
...
public:  // setters
    ...
    void setName(const char *name) { INDI::strlcpy(this->name, name); }
    void setName(const std::string &name) { setName(name.data()); }

    void setLabel(const char *label) { INDI::strlcpy(this->label, label); }
    void setLabel(const std::string &label) { setLabel(label.data()); }

    // void setText(const char *text)                         {
    // free(this->text); this->text = strndup(text, strlen(text)); }
    void setText(const char *text, size_t size) {
        INDI::strlcpy(
            this->text = static_cast<char *>(realloc(this->text, size + 1)),
            text, size + 1);
    }
...
public:  // getters
    const char *getName() const { return this->name; }
    const char *getLabel() const { return this->label; }
    const char *getText() const { return this->text ? this->text : ""; }
...
public:  // tests
    ...
    bool isEmpty() const { return getText()[0] == '\0'; }

public:
    void fill(
        const char *name, const char *label,
        const char *initialText); /* outside implementation - only driver side,
                                     see indipropertyview_driver.cpp */

    void fill(const std::string &name, const std::string &label,
...
public:  // setters
    ...
    void setFormat(const char *format) { INDI::strlcpy(this->format, format); }
...
public:
    void fill(const char *name, const char *label, const char *format,
              ...
              double value); /* outside implementation - only driver side, see
                                indipropertyview_driver.cpp */

    void fill(const std::string &name, const std::string &label,
              const std::string &format, double min, double max, double step,
              double value) {
        fill(name.c_str(), label.c_str(), format.c_str(), min, max, step,
             value);
    }
};
...
public:
    void fill(
        const char *name, const char *label,
        ISState state = ISS_OFF); /* outside implementation - only driver side,
                                     see indipropertyview_driver.cpp */
...
public:
    void fill(
        const char *name, const char *label,
        IPState state = IPS_OK); /* outside implementation - only driver side,
                                    see indipropertyview_driver.cpp */
...
public:  // setters
    ...
    void setFormat(const char *format) { INDI::strlcpy(this->format, format); }
    void setFormat(const std::string &format) { setFormat(format.data()); }
...
public:  // getters
    ...
    const char *getLabel() const { return this->label; }
    const char *getFormat() const { return this->format; }
    void *getBlob() { return this->blob; }
    const void *getBlob() const { return this->blob; }
    std::string getBlobAsString() const {
        return std::string(static_cast<const char *>(this->blob),
                           this->bloblen);
    }
...
public:
    void fill(const char *name, const char *label,
              const char *format); /* outside implementation - only driver side,
                                      see indipropertyview_driver.cpp */
...
template <>
inline WidgetView<IText> *PropertyView<IText>::findWidgetByName(
    const char *name) const {
    return static_cast<WidgetView<IText> *>(IUFindText(this, name));
}

template <>
inline WidgetView<INumber> *PropertyView<INumber>::findWidgetByName(
    const char *name) const {
    return static_cast<WidgetView<INumber> *>(IUFindNumber(this, name));
}

template <>
inline WidgetView<ISwitch> *PropertyView<ISwitch>::findWidgetByName(
    const char *name) const {
    return static_cast<WidgetView<ISwitch> *>(IUFindSwitch(this, name));
}

template <>
inline WidgetView<ILight> *PropertyView<ILight>::findWidgetByName(
    const char *name) const {
    return static_cast<WidgetView<ILight> *>(IUFindLight(this, name));
}

template <>
inline WidgetView<IBLOB> *PropertyView<IBLOB>::findWidgetByName(
    const char *name) const {
...
template <>
inline PropertyView<INumber>::WidgetType *PropertyView<INumber>::widget()
    const {
    return static_cast<WidgetType *>(this->np);
}

template <>
inline PropertyView<ISwitch>::WidgetType *PropertyView<ISwitch>::widget()
    const {
