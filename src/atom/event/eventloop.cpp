...
Result EventLoop::Impl::registerFd(SOCKET_FD fd, uint32_t events,
                                   IOCallback cb) {
...
Result EventLoop::Impl::appendObserver(ObserverCallback cb,
...
Result EventLoop::Impl::appendTask(Task task, EventLoopToken *token,
                                   const char *debugStr) {
    if (token && token->eventLoop().get() != this) {
        return Result::INVALID_PARAM;
    }
    ...
    if (token) {
        auto p =
            std::make_shared<TokenTaskSlot>(std::move(task), std::move(dstr));
...
Result EventLoop::Impl::appendDelayedTask(uint32_t delay_ms, Task task,
                                          EventLoopToken *token,
                                          const char *debugStr) {
    if (token && token->eventLoop().get() != this) {
        return Result::INVALID_PARAM;
    ...
    std::string dstr{debugStr ? debugStr : ""};
    auto ptr = std::make_shared<DelayedTaskSlot>(this, std::move(task),
                                                 std::move(dstr));
    if (token) {
        token->appendDelayedTaskNode(ptr);
    }
    // NOTE: ptr is stored in TimerCallback, so no queue is needed for
    // DelayedTaskSlot. ptr will be released after timer cancelled or executed,
    // or TimerManager destructed
...
Result EventLoop::Impl::sync(Task task, EventLoopToken *token,
                             const char *debugStr) {
    if (inSameThread()) {
        task();
        return Result::OK;
        ...
        auto clean = [&] {
            ...
            ready = true;
            cv.notify_one();  // the waiting thread may block again since m is
                              // not released
        };
...
Result EventLoop::Impl::async(Task task, EventLoopToken *token,
                              const char *debugStr) {
    if (inSameThread()) {
        task();
        return Result::OK;
...
Result EventLoop::Impl::post(Task task, EventLoopToken *token,
                             const char *debugStr) {
    return appendTask(std::move(task), token, debugStr);
}

Result EventLoop::Impl::postDelayed(uint32_t delay_ms, Task task,
                                    EventLoopToken *token,
                                    const char *debugStr) {
    return appendDelayedTask(delay_ms, std::move(task), token, debugStr);
}

void EventLoop::Impl::wakeup() { poll_->notify(); }

/////////////////////////////////////////////////////////////////
// DelayedTaskSlot
DelayedTaskSlot::DelayedTaskSlot(EventLoop::Impl *loop, EventLoop::Task &&t,
                                 std::string debugStr)
...
EventLoop::Token::Impl::~Impl() { reset(); }

void EventLoop::Token::Impl::eventLoop(const EventLoopPtr &loop) {
    loop_ = loop;
}

EventLoopPtr EventLoop::Token::Impl::eventLoop() { return loop_.lock(); }
...
void EventLoop::Token::Impl::clearAllTasks() {
    ...
    pending_ttask_nodes_.splice(pending_ttask_nodes_.end(),
                                std::move(ttask_nodes_));
    pending_dtask_nodes_.splice(pending_dtask_nodes_.end(),
                                std::move(dtask_nodes_));
    ...
    while (!pending_ttask_nodes_.empty()) {
        ...
        if (ts->isActive()) {
            ...
            ul.lock();
            pop_task = !pending_ttask_nodes_.empty() &&
                       ts == pending_ttask_nodes_.front();
        }
        if (pop_task) {
    ...
    while (!pending_dtask_nodes_.empty()) {
        ...
        if (ds->isActive()) {
            ...
            ds->cancel();
            ul.lock();
            pop_task = !pending_dtask_nodes_.empty() &&
                       ds == pending_dtask_nodes_.front();
        }
        if (pop_task) {
