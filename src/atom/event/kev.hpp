...
public:
    ...
    public:
        ...
        /* clear all tasks that are scheduled to this token. you cannot cancel
         * the task that is in running, but will wait untill the task completion
         */
        void reset();
...
public:
    ...
    /* create a token, it can be used to cancel the tasks that are scheduled
     * with it if caller can guarantee the resources used by tasks are valid
     * when task running, then token is no need, otherwise the caller should
     * cancel the tasks queued in loop before the resources are unavailable
     */
    Token createToken();
     ...
     *
     * @param f the task to be executed. it will always be executed when call
     * success
     *
    ...
    template <typename F,
              std::enable_if_t<!std::is_void<decltype(std::declval<F>()())>{},
                               int> = 0>
    auto invoke(F &&f, Result &err, Token *token = nullptr,
    ...
    template <typename F,
              std::enable_if_t<std::is_void<decltype(std::declval<F>()())>{},
                               int> = 0>
    void invoke(F &&f, Result &err, Token *token = nullptr,
                const char *debugStr = nullptr) {
     ...
     * token is always unnecessary for sync task
     *
     * @param task the task to be executed. it will always be executed when call
     * success
     */
    template <typename F,
    ...
    Result sync(Task task, Token *token = nullptr,
                const char *debugStr = nullptr);
     ...
     * @param task the task to be executed. it will always be executed when call
     * success
     * @param token to be used to cancel the task. If token is null, the caller
     * should make sure the resources referenced by task are valid when task
     * running
     * @param debugStr debug message of the f, e.g. file name and line where f
     * is generated
     */
    template <typename F,
              std::enable_if_t<!std::is_copy_constructible<F>{}, int> = 0>
    Result async(F &&f, Token *token = nullptr,
                 const char *debugStr = nullptr) {
        lambda_wrapper<F> wf{std::forward<F>(f)};
        return async(Task(std::move(wf)), token, debugStr);
    }
    Result async(Task task, Token *token = nullptr,
                 const char *debugStr = nullptr);
    ...
    Result post(Task task, Token *token = nullptr,
                const char *debugStr = nullptr);

    template <typename F,
              std::enable_if_t<!std::is_copy_constructible<F>{}, int> = 0>
    Result postDelayed(uint32_t delay_ms, F &&f, Token *token = nullptr,
                       const char *debugStr = nullptr) {
        lambda_wrapper<F> wf{std::forward<F>(f)};
        return postDelayed(delay_ms, Task(std::move(wf)), token, debugStr);
    }
    Result postDelayed(uint32_t delay_ms, Task task, Token *token = nullptr,
                       const char *debugStr = nullptr);

    void wakeup();

    /* cancel the tasks that are scheduled with token. you cannot cancel the
     * task that is in running, but will wait untill the task completion
     *
...
public:
    ...
    enum class Mode { ONE_SHOT, REPEATING };
