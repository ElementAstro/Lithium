...
SelectPoll::SelectPoll() : max_fd_(0) {
    FD_ZERO(&read_fds_);
    FD_ZERO(&write_fds_);
    FD_ZERO(&except_fds_);
}

SelectPoll::~SelectPoll() { poll_fds_.clear(); }
...
Result SelectPoll::unregisterFd(SOCKET_FD fd) {
    ...
    KM_INFOTRACE("SelectPoll::unregisterFd, fd=" << fd
                                                 << ", max_fd=" << max_fd);
    if (fd < 0 || fd > max_fd) {
...
Result SelectPoll::updateFd(SOCKET_FD fd, KMEvent events) {
    ...
    if (fd < 0 || -1 == max_fd || fd > max_fd) {
        KM_WARNTRACE(
            "SelectPoll::updateFd, failed, fd=" << fd << ", max_fd=" << max_fd);
        return Result::INVALID_PARAM;
    }
    if (poll_items_[fd].fd != fd) {
        KM_WARNTRACE("SelectPoll::updateFd, failed, fd=" << fd << ", item_fd="
                                                         << poll_items_[fd].fd);
        return Result::INVALID_PARAM;
    }
    int idx = poll_items_[fd].idx;
    ...
    if (poll_fds_[idx].fd != fd) {
        KM_WARNTRACE("SelectPoll::updateFd, failed, fd=" << fd << ", pfds_fd="
                                                         << poll_fds_[idx].fd);
        return Result::INVALID_PARAM;
    }
    ...
    if (events != 0) {
        ...
        if (max_fd_ == fd) {
            auto it = std::max_element(
                ...
                [](PollFD &pf1, PollFD &pf2) { return pf1.fd < pf2.fd; });
...
Result SelectPoll::wait(uint32_t wait_ms) {
    ...
    int nready = ::select(static_cast<int>(max_fd_ + 1), &readfds, &writefds,
                          &exceptfds, wait_ms == -1 ? nullptr : &tval);
    if (nready <= 0) {
        return Result::OK;
    }
...
void SelectPoll::notify() { notifier_->notify(); }

IOPoll *createSelectPoll() { return new SelectPoll(); }
