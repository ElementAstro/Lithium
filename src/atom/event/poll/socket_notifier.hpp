...
public:
    ...
    bool init() override {
        ...
        if (km_set_sock_addr("127.0.0.1", 0, &hints,
        ...
        char ip[128] = {0};
        uint16_t port = 0;
        km_get_sock_addr((struct sockaddr *)&ss_addr, addr_len, ip, sizeof(ip),
                         &port);
        if (km_set_sock_addr("127.0.0.1", port, &hints,
                             (struct sockaddr *)&ss_addr,
                             sizeof(ss_addr)) != 0) {
            return false;
        }
        ...
        if (::connect(fds_[WRITE_FD], (const sockaddr *)&ss_addr,
