...
{
    if (0 == timer_node->tv_index_ && timer_node->next_ != timer_node && timer_node->next_ == timer_node->prev_ && timer_node->next_ == &tv_[0][timer_node->tl_index_])
    {
...
int TimerManager::checkExpire(unsigned long *remain_ms)
{
    ...
    while (JIFFIES_COMPARE(cur_jiffies, next_jiffies) >= 0)
    {
        ...
        {
            if (-1 == delta || ((idx + delta) & TIMER_VECTOR_MASK) < idx)
            {                                    // run over 0
