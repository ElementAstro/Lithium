...
public:
    ...
    NodePtr &front_node() { return head_; }
    ...
    bool remove(const NodePtr &node) {  // make sure the node is in this queue
    ...
    void swap(DLQueue &other) {
        ...
        auto c = count_.exchange(other.count_.load(std::memory_order_relaxed),
