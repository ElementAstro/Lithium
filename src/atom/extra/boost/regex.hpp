...
public:
    ...
    auto searchAll(const T& str) const -> std::vector<std::string> {
        ...
        boost::sregex_iterator iter(std::string_view(str).begin(),
                                    std::string_view(str).end(), regex_);
    ...
    auto split(const T& str) const -> std::vector<std::string> {
        std::vector<std::string> results;
        boost::sregex_token_iterator iter(std::string_view(str).begin(),
                                          std::string_view(str).end(), regex_,
                                          -1);
    ...
    template <typename T>
        requires std::convertible_to<T, std::string_view>
    auto replaceCallback(const T& str,
                         const std::function<std::string(const boost::smatch&)>&
    ...
    [[nodiscard]] static auto escapeString(const std::string& str)
        -> std::string {
        return boost::regex_replace(str, boost::regex(R"([.^$|()\[\]{}*+?\\])"),
                                    R"(\\&)",
