...
namespace Atom::Log
{
    ...
    template <typename Arg, typename... Args>
    void SyslogWrapper::log(LogLevel level, const char *format, Arg &&arg, Args &&...args)
    {
    ...
    void SyslogWrapper::setLogLevel(LogLevel logLevel)
    {
        m_logLevel = logLevel;
    }

    LogLevel SyslogWrapper::getLogLevel() const
    {
        return m_logLevel;
    }
...
#endif
        char buf[40] = {0};
        snprintf(buf, sizeof(buf), "%04d-%02d-%02d %02d:%02d:%02d.%03lld", tm_now.tm_year + 1900, tm_now.tm_mon + 1,
                 tm_now.tm_mday, tm_now.tm_hour, tm_now.tm_min, tm_now.tm_sec, ms);
        return buf;
    }
