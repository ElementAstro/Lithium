...
namespace Lithium
{
    ...
    bool Compiler::CompileToSharedLibraryAllinOne(const std::string &code, const std::string &moduleName, const std::string &functionName)
    {
    ...
    // 编译为共享库
    bool Compiler::CompileToSharedLibrary(const std::string &code, const std::string &moduleName, const std::string &functionName, const std::string &optionsFile)
    {
    ...
    // 检查参数是否有效
    bool Compiler::CheckParameters(const std::string &code, const std::string &moduleName, const std::string &functionName)
    {
        if (code.empty() || moduleName.empty() || functionName.empty())
        {
            LOG_F(ERROR, "Invalid parameters.");
    ...
    // 检查模块是否已经编译并缓存
    bool Compiler::IsModuleCached(const std::string &moduleName, const std::string &functionName, std::unordered_map<std::string, std::string> &cache_)
    {
        std::string key = moduleName + "::" + functionName;
        auto cachedResult = cache_.find(key);
        if (cachedResult != cache_.end())
        {
            LOG_F(WARNING, "Module {}::{} is already compiled.", moduleName, functionName);
            return true;
        }
        return false;
    }
    ...
    std::string Compiler::ReadCompileOptions(const std::string &optionsFile)
    {
        ...
        if (compileOptionFile.is_open())
        {
            ...
            try
            {
                ...
                if (compileOptionsJson.contains("optimization_level") && compileOptionsJson.contains("cplus_version") && compileOptionsJson.contains("warnings"))
                {
                    std::string compileOptions = compileOptionsJson["optimization_level"].get<std::string>() + " " +
                                                 compileOptionsJson["cplus_version"].get<std::string>() + " " +
                                                 compileOptionsJson["warnings"].get<std::string>();
    ...
    // 语法检查
    bool Compiler::SyntaxCheck(const std::string &code, const std::string &compiler)
    {
        std::stringstream syntaxCheckCmd;
        syntaxCheckCmd << compiler << " -fsyntax-only -x c++ -";
        std::string syntaxCheckOutput;
    ...
    // 编译代码
    bool Compiler::CompileCode(const std::string &code, const std::string &compiler, const std::string &compileOptions, const std::string &output)
    {
    ...
    // 缓存已编译的模块
    void Compiler::CacheCompiledModule(const std::string &moduleName, const std::string &functionName, const std::string &output, std::unordered_map<std::string, std::string> &cache_)
    {
        std::string key = moduleName + "::" + functionName;
        cache_[key] = output;
    }
    ...
    bool Compiler::CopyFile_(const std::string &source, const std::string &destination)
    {
        try
        {
            ...
            if (!dst.good())
            {
                LOG_F(ERROR, "Error occurred while writing to destination file: {}", destination);
                return false;
            }
    ...
    int Compiler::RunShellCommand(const std::string &command, const std::string &input, std::string &output)
    {
        ...
        {
            LOG_F(ERROR, "Failed to create output pipe for shell command: {}", command);
            CloseHandle(hStdinRead);
