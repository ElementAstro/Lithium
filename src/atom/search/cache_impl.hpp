...
void ResourceCache<T>::evict()
{
    ...
    std::chrono::steady_clock::time_point oldestAccessTime = std::chrono::steady_clock::now();

    // 查找最久未访问且未过期的条目
    for (const auto &entry : lastAccessTimes)
    {
        if (std::chrono::steady_clock::now() - entry.second > expirationTimes[entry.first] &&
...
template <typename T>
void ResourceCache<T>::insert(const std::string &key, const T &value, std::chrono::seconds expirationTime)
{
...
template <typename T>
std::future<void> ResourceCache<T>::asyncInsert(const std::string &key, const T &value, const std::chrono::seconds &expirationTime)
{
...
bool ResourceCache<T>::isExpired(const std::string &key) const
{
    ...
    return std::chrono::steady_clock::now() - cache.at(key).second >= expirationTimes.at(key);
}

template <typename T>
std::future<void> ResourceCache<T>::asyncLoad(const std::string &key, std::function<T()> loadDataFunction)
{
...
template <typename T>
void ResourceCache<T>::setExpirationTime(const std::string &key, std::chrono::seconds expirationTime)
{
    std::lock_guard<std::recursive_mutex> lock(cacheMutex);
    if (contains(key))
    {
...
template <typename T>
void ResourceCache<T>::readFromFile(const std::string &filePath, const std::function<T(const std::string &)> &deserializer)
{
    ...
    if (inputFile.is_open())
    {
        ...
        while (std::getline(inputFile, line))
        {
            ...
            if (separatorIndex != std::string::npos && separatorIndex != line.length() - 1)
            {
...
template <typename T>
void ResourceCache<T>::writeToFile(const std::string &filePath, const std::function<std::string(const T &)> &serializer)
{
    ...
    if (outputFile.is_open())
    {
        ...
        {
            std::string line = pair.first + ":" + serializer(pair.second.first) + "\n";
            outputFile << line;
        }
        outputFile.close();
    }
...
template <typename T>
void ResourceCache<T>::readFromJsonFile(const std::string &filePath, const std::function<T(const json &)> &fromJson)
{
    std::ifstream inputFile(filePath);
    if (inputFile.is_open())
    {
...
template <typename T>
void ResourceCache<T>::writeToJsonFile(const std::string &filePath, const std::function<json(const T &)> &toJson)
{
    std::ofstream outputFile(filePath);
    if (outputFile.is_open())
    {
