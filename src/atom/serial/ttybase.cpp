...
#ifdef _WIN32
    retval =
        select(static_cast<int>(m_PortFD) + 1, &readout, nullptr, nullptr, &tv);
#else
...
TTYBase::TTY_RESPONSE TTYBase::write(const uint8_t *buffer, uint32_t nbytes,
                                     uint32_t *nbytes_written) {
#ifdef _WIN32
    DWORD bytes_w = 0;
    *nbytes_written = 0;

    if (m_PortFD == INVALID_HANDLE_VALUE)
        return TTY_ERRNO;

    while (nbytes > 0) {
        if (!WriteFile(m_PortFD, buffer + (*nbytes_written), nbytes, &bytes_w,
                       nullptr))
            return TTY_WRITE_ERROR;

        for (uint32_t i = *nbytes_written; i < bytes_w + *nbytes_written; i++)
            DEBUGFDEVICE(m_DriverName, m_DebugChannel,
                         "%s: buffer[%d]=%#X (%c)", __FUNCTION__, i, buffer[i],
                         buffer[i]);
...
TTYBase::TTY_RESPONSE TTYBase::writeString(const char *string,
    ...
    return write(reinterpret_cast<const uint8_t *>(string), nbytes,
                 nbytes_written);
}

TTYBase::TTY_RESPONSE TTYBase::read(uint8_t *buffer, uint32_t nbytes,
    ...
    *nbytes_read = 0;
    ...
    uint32_t num_bytes_to_read = nbytes;
    TTY_RESPONSE timeout_response = TTY_OK;

    if (nbytes <= 0)
        return TTY_PARAM_ERROR;

    DEBUGFDEVICE(m_DriverName, m_DebugChannel,
                 "%s: Request to read %d bytes with %d timeout for m_PortFD %d",
                 __FUNCTION__, nbytes, timeout, m_PortFD);

    while (num_bytes_to_read > 0) {
        if ((timeout_response = checkTimeout(timeout)))
            return timeout_response;

        if (!ReadFile(m_PortFD, buffer + (*nbytes_read), num_bytes_to_read,
                      &bytes_read, nullptr))
            return TTY_READ_ERROR;

        DEBUGFDEVICE(m_DriverName, m_DebugChannel,
                     "%d bytes read and %d bytes remaining...", bytes_read,
                     num_bytes_to_read - bytes_read);
        for (uint32_t i = *nbytes_read; i < (*nbytes_read + bytes_read); i++)
            DEBUGFDEVICE(m_DriverName, m_DebugChannel,
                         "%s: buffer[%d]=%#X (%c)", __FUNCTION__, i, buffer[i],
                         buffer[i]);
...
TTYBase::TTY_RESPONSE TTYBase::readSection(uint8_t *buffer, uint32_t nsize,
                                           uint8_t stop_byte, uint8_t timeout,
    ...
    *nbytes_read = 0;
    ...
    DEBUGFDEVICE(m_DriverName, m_DebugChannel,
                 "%s: Request to read until stop char '%#02X' with %d timeout "
                 "for m_PortFD %d",
    ...
    for (;;) {
        ...
        if (!ReadFile(m_PortFD, (buffer + *nbytes_read), 1, &bytes_read,
                      nullptr))
            return TTY_READ_ERROR;

        DEBUGFDEVICE(m_DriverName, m_DebugChannel, "%s: buffer[%d]=%#X (%c)",
                     __FUNCTION__, (*nbytes_read), *(buffer + *nbytes_read),
                     *(buffer + *nbytes_read));

        (*nbytes_read)++;

        if (*(buffer + *nbytes_read - 1) == stop_byte)
            return TTY_OK;
...
#if defined(BSD) && !defined(__GNU__)
// BSD - OSX version
TTYBase::TTY_RESPONSE TTYBase::connect(const char *device, uint32_t bit_rate,
                                       uint8_t word_size, uint8_t parity,
                                       uint8_t stop_bits) {
    int t_fd = -1;
    int bps;
    int handshake;
...
TTYBase::TTY_RESPONSE TTYBase::connect(const char *device, uint32_t bit_rate,
                                       ...
                                       uint8_t stop_bits) {
#ifdef _WIN32
    HANDLE t_fd = INVALID_HANDLE_VALUE;
    DCB dcb;
    COMMTIMEOUTS timeouts;

    // Open the serial port
    t_fd = CreateFileA(device, GENERIC_READ | GENERIC_WRITE, 0, NULL,
                       OPEN_EXISTING, 0, NULL);
    if (t_fd == INVALID_HANDLE_VALUE) {
        m_PortFD = -1;
        return TTY_PORT_FAILURE;
    }
    ...
    if (!GetCommState(t_fd, &dcb)) {
        DEBUGDEVICE(m_DriverName, m_DebugChannel,
                    "connect: failed getting DCB settings.");
        CloseHandle(t_fd);
        return TTY_PORT_FAILURE;
    }
    ...
    // Set the new DCB settings
    if (!SetCommState(t_fd, &dcb)) {
        DEBUGDEVICE(m_DriverName, m_DebugChannel,
                    "connect: failed setting DCB settings on serial port.");
        CloseHandle(t_fd);
        return TTY_PORT_FAILURE;
    }
    ...
    if (!SetCommTimeouts(t_fd, &timeouts)) {
        DEBUGDEVICE(m_DriverName, m_DebugChannel,
                    "connect: failed setting timeouts on serial port.");
        CloseHandle(t_fd);
        return TTY_PORT_FAILURE;
    }
        ...
        case TTY_PORT_FAILURE:
            if (errno == EACCES)
                snprintf(error_string, 512,
                         "Port failure Error: %s. Try adding your user to the "
                         "dialout group and restart (sudo adduser "
                         "$USER dialout)",
                         strerror(errno));
            else
                snprintf(error_string, 512,
                         "Port failure Error: %s. Check if device is connected "
                         "to this port.",
                         strerror(errno));
            return error_string;
