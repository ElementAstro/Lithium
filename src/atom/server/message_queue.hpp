...
namespace Atom::Async
{
    ...
    public:
        /**
         * @brief The callback function type that will be called when a new message is received.
         */
        ...
        /**
         * @brief Stop the processing thread(s) from receiving and handling messages.
         */
        void stopProcessingThread();

    private:
        /**
         * @brief The Subscriber struct contains information about a subscribed callback function.
         */
        struct Subscriber
        {
            std::string name;      /**< The name of the subscriber. */
            CallbackType callback; /**< The callback function to be called when a new message is received. */
        ...
        std::deque<T> m_messages;                               /**< The queue containing all published messages. */
        std::vector<Subscriber> m_subscribers;                  /**< The vector containing all subscribed callback functions. */
        std::mutex m_mutex;                                     /**< The mutex used to protect access to the message queue. */
        std::mutex m_subscriberMutex;                           /**< The mutex used to protect access to the subscriber vector. */
        std::condition_variable m_condition;                    /**< The condition variable used to notify processing threads of new messages. */
        std::atomic<bool> m_isRunning{true};                    /**< The flag used to indicate whether the processing thread(s) should continue running. */
        std::vector<std::thread> m_processingThreads;           /**< The vector containing all processing threads. */
        int m_numThreads = std::thread::hardware_concurrency(); /**< The number of processing threads to spawn. */
    };

    template <typename T>
    void MessageQueue<T>::subscribe(CallbackType callback, const std::string &subscriberName)
    {
            ...
            m_processingThreads.emplace_back([this]()
                                             {
                                                 while (m_isRunning.load())
                                                 {
...

                                                     {
                                                         ...
                                                         m_condition.wait(lock, [this]() { return !m_messages.empty() || !m_isRunning.load(); });
