...
PYBIND11_MODULE(atom_system, m) {
    ...
    m.def("get_current_cpu_usage", &getCurrentCpuUsage, "Get the current CPU usage as a percentage.");
    m.def("get_current_cpu_temperature", &getCurrentCpuTemperature, "Get the current CPU temperature in degrees Celsius.");
    m.def("get_cpu_model", &getCPUModel, "Get the model of the CPU.");
    m.def("get_processor_identifier", &getProcessorIdentifier, "Get the identifier of the CPU processor.");
    m.def("get_processor_frequency", &getProcessorFrequency, "Get the frequency of the CPU in GHz.");
    m.def("get_number_of_physical_packages", &getNumberOfPhysicalPackages, "Get the number of physical CPU packages.");
    m.def("get_number_of_physical_cpus", &getNumberOfPhysicalCPUs, "Get the number of physical CPUs.");
    ...
    m.def("getCurrentWiredNetwork", &getCurrentWiredNetwork, "Get the current wired network name");
    m.def("isHotspotConnected", &isHotspotConnected, "Check if hotspot is connected");

    //m.def("executeCommand", &executeCommand, "Execute a command and return the output as a string");
    m.def("executeCommands", &executeCommands, "Execute a list of commands");
    //m.def("executeCommand", &executeCommand, "Execute a command and return the process handle");
    m.def("killProcess", &killProcess, "Kill a process");
    m.def("executeCommandWithEnv", &executeCommandWithEnv, "Execute a command with environment variables and return the output as a string");
    m.def("executeCommandWithStatus", &executeCommandWithStatus, "Execute a command and return the output along with the exit status");
