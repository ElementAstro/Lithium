...
namespace Atom::System
{
    ...
    const std::string &Quote::getText() const
    {
        return text;
    }

    const std::string &Quote::getAuthor() const
    {
        return author;
    }
    ...
    std::vector<Quote> QuoteManager::searchQuotes(const std::string &keyword) const
    {
    ...
    std::vector<Quote> QuoteManager::filterQuotesByAuthor(const std::string &author) const
    {
        std::vector<Quote> results;
        for (const auto &quote : quotes)
        {
...
int main()
{
    ...
    // 添加名言
    quoteManager.addQuote(Quote("Be yourself; everyone else is already taken.", "Oscar Wilde"));
    quoteManager.addQuote(Quote("In the end, it's not the years in your life that count. It's the life in your years.", "Abraham Lincoln"));
    quoteManager.addQuote(Quote("The only way to do great work is to love what you do.", "Steve Jobs"));
