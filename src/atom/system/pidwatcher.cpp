...
namespace Atom::System
{
    ...
    void PidWatcher::SetMonitorFunction(Callback callback, std::chrono::milliseconds interval)
    {
        std::lock_guard<std::mutex> lock(mutex_);
    ...
    pid_t PidWatcher::GetPidByName(const std::string &name) const
    {
        ...
        if (EnumProcesses(pid_list, sizeof(pid_list), &cb_needed))
        {
            ...
            {
                HANDLE process_handle = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ, FALSE, pid_list[i]);
                if (process_handle != NULL)
                {
                    char filename[MAX_PATH];
                    if (GetModuleFileNameEx(process_handle, NULL, filename, MAX_PATH))
                    {
