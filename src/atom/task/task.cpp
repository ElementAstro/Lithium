...
SimpleTask::SimpleTask(const std::function<json(const json &)> &func,
    ...
    : m_function(func),
      m_paramsTemplate(params_template),
      m_stopFn(stop_fn),
      m_stopFlag(false) {
    if (stop_fn) {
        m_canStop = true;
...
json SimpleTask::getResult() { return m_returns; }

json SimpleTask::getParamsTemplate() { return m_paramsTemplate; }

void SimpleTask::setParams(const json &params) { m_params = params; }

int SimpleTask::getId() const { return m_id; }

void SimpleTask::setId(int id) { m_id = id; }

const std::string &SimpleTask::getName() const { return m_name; }

void SimpleTask::setName(const std::string &name) { m_name = name; }

const std::string &SimpleTask::getDescription() const { return m_description; }

void SimpleTask::setDescription(const std::string &description) {
    m_description = description;
}

void SimpleTask::setCanExecute(bool can_execute) { m_canExecute = can_execute; }

bool SimpleTask::isExecutable() const { return m_canExecute; }

void SimpleTask::setStopFunction(
    const std::function<json(const json &)> &stop_fn) {
    m_stopFn = stop_fn;
    m_canStop = true;
}

bool SimpleTask::getStopFlag() const { return m_stopFlag; }

void SimpleTask::setStopFlag(bool flag) { m_stopFlag = flag; }
...
bool SimpleTask::validateJsonValue(const json &data,
                                   const json &templateValue) {
    ...
    if (data.is_object()) {
        for (auto it = templateValue.begin(); it != templateValue.end(); ++it) {
            ...
            if (!data.contains(key) ||
                !validateJsonValue(data[key], subTemplateValue)) {
                return false;
            }
        }
...
bool SimpleTask::validateJsonString(const std::string &jsonString,
