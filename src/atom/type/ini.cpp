    ...
    for (const auto &section : data) {
        ...
        for (const auto &entry : section.second) {
            if (entry.second.type() == typeid(int)) {
                ...
                file << entry.first << "="
                     << std::any_cast<double>(entry.second) << "\n";
                ...
                file << entry.first << "="
                     << std::any_cast<const char *>(entry.second) << "\n";
    ...
    for (const auto &section : data) {
        ...
        for (const auto &entry : section.second) {
            try {
                if (entry.second.type() == typeid(int)) {
                    oss << "\"" << entry.first
                        << "\": " << std::any_cast<int>(entry.second) << ", ";
                } else if (entry.second.type() == typeid(float)) {
                    oss << "\"" << entry.first
                        << "\": " << std::any_cast<float>(entry.second) << ", ";
                } else if (entry.second.type() == typeid(double)) {
                    oss << "\"" << entry.first
                        << "\": " << std::any_cast<double>(entry.second)
                        << ", ";
...
std::string INIFile::toXml() const {
    ...
    for (const auto &section : data) {
        ...
        for (const auto &entry : section.second) {
            try {
                if (entry.second.type() == typeid(int)) {
                    oss << "    <entry name=\"" << entry.first
                        << "\" type=\"int\">"
                        << std::any_cast<int>(entry.second) << "</entry>\n";
                } else if (entry.second.type() == typeid(float)) {
                    oss << "    <entry name=\"" << entry.first
                        << "\" type=\"float\">"
                        << std::any_cast<float>(entry.second) << "</entry>\n";
                } else if (entry.second.type() == typeid(double)) {
                    oss << "    <entry name=\"" << entry.first
                        << "\" type=\"double\">"
                        << std::any_cast<double>(entry.second) << "</entry>\n";
                } else if (entry.second.type() == typeid(std::string)) {
                    oss << "    <entry name=\"" << entry.first
                        << "\" type=\"string\">"
                        << std::any_cast<std::string>(entry.second)
