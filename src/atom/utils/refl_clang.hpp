  ...
  struct ElemList
  {
    ...
    constexpr void ForEach(F &&f) const
    {
      Accumulate(0, [&](auto, const auto &field)
                 {std::forward<F>(f)(field); return 0; });
    }
    template <class S>
    static constexpr bool Contains(S = {}) { return (Es::TName::template Is<S>() || ...); }
    template <class F>
    constexpr std::size_t FindIf(F &&f) const { return detail::FindIf(*this, std::forward<F>(f), std::make_index_sequence<size>{}); }
    template <class S>
    constexpr const auto &Find(S = {}) const
    {
      constexpr std::size_t idx = []()
      { constexpr decltype(S::View()) names[]{ Es::name... };
      for (std::size_t i=0; i < size; i++) { if (S::View() == names[i]) return i; } return static_cast<std::size_t>(-1); }();
      return Get<idx>();
    }
    template <class T>
    constexpr std::size_t FindValue(const T &v) const
    {
      return FindIf([&v](auto e)
                    { return e == v; });
    }
    ...
    constexpr const auto &Get() const { return std::get<N>(elems); }
