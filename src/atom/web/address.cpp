...
namespace Atom::Web
{
    ...
    std::vector<std::string> getIPv4Addresses()
    {
        ...
        std::unique_ptr<IP_ADAPTER_ADDRESSES> adapterAddresses(new IP_ADAPTER_ADDRESSES[bufferSize]);
        if (GetAdaptersAddresses(AF_INET, 0, nullptr, adapterAddresses.get(), &bufferSize) == ERROR_SUCCESS)
        {
            for (PIP_ADAPTER_ADDRESSES adapter = adapterAddresses.get(); adapter; adapter = adapter->Next)
            {
                for (PIP_ADAPTER_UNICAST_ADDRESS unicastAddress = adapter->FirstUnicastAddress; unicastAddress; unicastAddress = unicastAddress->Next)
                {
                    sockaddr_in *sockAddrIn = reinterpret_cast<sockaddr_in *>(unicastAddress->Address.lpSockaddr);
                    char addressBuffer[INET_ADDRSTRLEN] = {0};
                    if (inet_ntop(AF_INET, &(sockAddrIn->sin_addr), addressBuffer, INET_ADDRSTRLEN))
                    {
    ...
    std::vector<std::string> getIPv6Addresses()
    {
        ...
        ULONG bufferSize = 0;
        if (GetAdaptersAddresses(AF_INET6, 0, nullptr, nullptr, &bufferSize) != ERROR_BUFFER_OVERFLOW)
        {
            return addresses;
        }

        std::unique_ptr<IP_ADAPTER_ADDRESSES> adapterAddresses(new IP_ADAPTER_ADDRESSES[bufferSize]);
        if (GetAdaptersAddresses(AF_INET6, 0, nullptr, adapterAddresses.get(), &bufferSize) == ERROR_SUCCESS)
        {
            for (PIP_ADAPTER_ADDRESSES adapter = adapterAddresses.get(); adapter; adapter = adapter->Next)
            {
                for (PIP_ADAPTER_UNICAST_ADDRESS unicastAddress = adapter->FirstUnicastAddress; unicastAddress; unicastAddress = unicastAddress->Next)
                {
                    sockaddr_in6 *sockAddrIn6 = reinterpret_cast<sockaddr_in6 *>(unicastAddress->Address.lpSockaddr);
                    char addressBuffer[INET6_ADDRSTRLEN] = {0};
                    if (inet_ntop(AF_INET6, &(sockAddrIn6->sin6_addr), addressBuffer, INET6_ADDRSTRLEN))
                    {
                        addresses.push_back(addressBuffer);
                    }
                }
    ...
    std::vector<std::string> getIPv4Addresses()
    {
        ...
        for (struct ifaddrs *ifa = ifAddrList; ifa != nullptr; ifa = ifa->ifa_next)
        {
            ...
            {
                struct sockaddr_in *sockAddrIn = reinterpret_cast<struct sockaddr_in *>(ifa->ifa_addr);
                char addressBuffer[INET_ADDRSTRLEN] = {0};
                if (inet_ntop(AF_INET, &(sockAddrIn->sin_addr), addressBuffer, INET_ADDRSTRLEN))
                {
                    addresses.push_back(addressBuffer);
    ...
    std::vector<std::string> getIPv6Addresses()
    {
        ...
        for (struct ifaddrs *ifa = ifAddrList; ifa != nullptr; ifa = ifa->ifa_next)
        {
            ...
            {
                struct sockaddr_in6 *sockAddrIn6 = reinterpret_cast<struct sockaddr_in6 *>(ifa->ifa_addr);
                char addressBuffer[INET6_ADDRSTRLEN] = {0};
                if (inet_ntop(AF_INET6, &(sockAddrIn6->sin6_addr), addressBuffer, INET6_ADDRSTRLEN))
                {
                    addresses.push_back(addressBuffer);
