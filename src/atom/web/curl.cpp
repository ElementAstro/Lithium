...
namespace Atom::Web
{
    ...
    void CurlWrapper::setOnResponseCallback(std::function<void(const std::string &)> callback)
    {
    ...
    void CurlWrapper::asyncPerform(std::function<void(const std::string &)> callback)
    {
    ...
    void CurlWrapper::waitAll()
    {
        ...
        do
        {
            ...
            while ((msg = curl_multi_info_read(multiHandle, &msgsLeft)))
            {
                ...
                {
                    LOG_F(ERROR, "Error: curl_multi_info_read() returned message with unexpected CURLMsg.");
                }
            }
            if (stillRunning)
            {
