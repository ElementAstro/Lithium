...
#ifndef CARBON_BOOTSTRAP_HPP
#define CARBON_BOOTSTRAP_HPP

#include "../utils/utility.hpp"
#include "register_function.hpp"

/// \brief Classes and functions useful for bootstrapping of ChaiScript and
/// adding of new types
namespace Carbon::bootstrap {
template <typename T,
...
void array(const std::string &type, Module &m) {
    ...
    m.add(
        fun([](T &t, size_t index) -> ReturnType & {
            constexpr const auto extent = std::extent<T>::value;
            if (extent > 0 && index >= extent) {
                throw std::range_error("Array index out of range. Received: " +
    ...
    m.add(
        fun([](const T &t, size_t index) -> const ReturnType & {
            constexpr const auto extent = std::extent<T>::value;
            if (extent > 0 && index >= extent) {
                throw std::range_error("Array index out of range. Received: " +
                                       std::to_string(index) + " expected < " +
...
/// \param[in] type The name of the type. The copy constructor will be named
/// "type". \param[in,out] m The Module to add the copy constructor to \tparam T
/// The type to add a copy constructor for \returns The passed in Module
template <typename T>
void copy_constructor(const std::string &type, Module &m) {
    m.add(constructor<T(const T &)>(), type);
}

/// \brief Add all comparison operators for the templated type. Used during
/// bootstrap, also available to users. \tparam T Type to create comparison
/// operators for \param[in,out] m module to add comparison operators to
...
Input parse_string(const std::string &i) {
    if constexpr (!std::is_same<Input, wchar_t>::value &&
        ...
        throw std::runtime_error(
            "Parsing of wide characters is not yet supported");
    }
}
...
template <typename Type>
std::shared_ptr<std::remove_const_t<Type>> shared_ptr_unconst_clone(
    const std::shared_ptr<std::add_const_t<Type>> &p) {
...
Boxed_Value ptr_assign(Boxed_Value lhs, const std::shared_ptr<Type> &rhs) {
    if (lhs.is_undef() ||
        (!lhs.get_type_info().is_const() &&
         lhs.get_type_info().bare_equal(
             Get_Type_Info<Type>::get()))) {
...
private:
    ...
    static Boxed_Value unknown_assign(Boxed_Value lhs, Boxed_Value rhs) {
        if (lhs.is_undef()) {
            ...
            throw exception::bad_boxed_cast(
                "boxed_value has a set type already");
        }
    }
    ...
    static Boxed_Value bind_function(const Function_Params &params) {
        ...
        if (f->get_arity() != -1 &&
            size_t(f->get_arity()) != params.size() - 1) {
            throw exception::arity_error(static_cast<int>(params.size()),
                                         f->get_arity());
        }

        return Boxed_Value(
            Const_Proxy_Function(std::make_shared<dispatch::Bound_Function>(
                std::move(f),
    ...
    static std::vector<Boxed_Value> do_return_boxed_value_vector(
        FunctionType f, const dispatch::Proxy_Function_Base *b) {
        auto v = (b->*f)();
    ...
    static bool has_parse_tree(
        const Carbon::Const_Proxy_Function &t_pf) noexcept {
        const auto pf = std::dynamic_pointer_cast<
            const Carbon::dispatch::Dynamic_Proxy_Function>(t_pf);
        return bool(pf);
    }

    static const Carbon::AST_Node &get_parse_tree(
        const Carbon::Const_Proxy_Function &t_pf) {
        const auto pf = std::dynamic_pointer_cast<
            const Carbon::dispatch::Dynamic_Proxy_Function>(t_pf);
        if (pf) {
...
public:
    /// \brief perform all common bootstrap functions for std::string, void and
    /// POD types \param[in,out] m Module to add bootstrapped functions to
    ...
    static void bootstrap(Module &m) {
        ...
        m.add(user_type<dispatch::Assignable_Proxy_Function>(),
              "Assignable_Function");
        ...
        m.add(fun(return_boxed_value_vector(
                  &dispatch::Proxy_Function_Base::get_param_types)),
              "get_param_types");
        m.add(fun(return_boxed_value_vector(
                  &dispatch::Proxy_Function_Base::get_contained_functions)),
        ...
        m.add(Carbon::base_class<std::exception, std::logic_error>());
        m.add(Carbon::base_class<std::logic_error, std::out_of_range>());
        m.add(Carbon::base_class<std::exception, std::out_of_range>());

        m.add(user_type<std::runtime_error>(), "runtime_error");
        m.add(Carbon::base_class<std::exception, std::runtime_error>());

        m.add(constructor<std::runtime_error(const std::string &)>(),
              "runtime_error");

        m.add(user_type<dispatch::Dynamic_Object>(), "Dynamic_Object");
        m.add(constructor<dispatch::Dynamic_Object(const std::string &)>(),
              "Dynamic_Object");
        ...
        m.add(fun(static_cast<Boxed_Value &(
                      dispatch::Dynamic_Object::*)(const std::string &)>(
                  &dispatch::Dynamic_Object::get_attr)),
              "get_attr");
        m.add(fun(static_cast<const Boxed_Value &(
                      dispatch::Dynamic_Object::*)(const std::string &) const>(
                  &dispatch::Dynamic_Object::get_attr)),
              "get_attr");
        ...
        m.add(fun(&shared_ptr_unconst_clone<dispatch::Proxy_Function_Base>),
              "clone");
        m.add(fun(&ptr_assign<
                  std::remove_const<dispatch::Proxy_Function_Base>::type>),
              "=");
        m.add(fun(&ptr_assign<
                  std::add_const<dispatch::Proxy_Function_Base>::type>),
              "=");
        m.add(Carbon::base_class<dispatch::Proxy_Function_Base,
                                     dispatch::Assignable_Proxy_Function>());
        m.add(fun([](dispatch::Assignable_Proxy_Function &t_lhs,
                     const std::shared_ptr<const dispatch::Proxy_Function_Base>
                         &t_rhs) { t_lhs.assign(t_rhs); }),
              "=");

        m.add(fun(&Boxed_Value::type_match), "type_match");

        m.add(Carbon::fun(&has_parse_tree), "has_parse_tree");
        m.add(Carbon::fun(&get_parse_tree), "get_parse_tree");

        m.add(Carbon::base_class<std::runtime_error,
        ...
        m.add(
            Carbon::base_class<std::runtime_error,
                                   Carbon::exception::arithmetic_error>());
        m.add(
            Carbon::base_class<std::exception,
                                   Carbon::exception::arithmetic_error>());

        //        Carbon::bootstrap::standard_library::vector_type<std::vector<std::shared_ptr<Carbon::AST_Node>
        //        >
        //        >("AST_NodeVector", m);

        Carbon::utility::add_class<Carbon::exception::eval_error>(
            m, "eval_error", {},
            {{fun(&Carbon::exception::eval_error::reason), "reason"},
             {fun(&Carbon::exception::eval_error::pretty_print),
              "pretty_print"},
             {fun([](const Carbon::exception::eval_error &t_eval_error) {
                  std::vector<Boxed_Value> retval;
                  std::transform(
                      t_eval_error.call_stack.begin(),
                      t_eval_error.call_stack.end(), std::back_inserter(retval),
                      &Carbon::var<const Carbon::AST_Node_Trace &>);
                  return retval;
              }),
              "call_stack"}});

        Carbon::utility::add_class<Carbon::File_Position>(
            m, "File_Position",
        ...
        Carbon::utility::add_class<AST_Node>(
            m, "AST_Node", {},
            {{fun(&AST_Node::text), "text"},
             ...
             {fun([](const Carbon::AST_Node &t_node)
                      -> std::vector<Boxed_Value> {
                  std::vector<Boxed_Value> retval;
                  const auto children = t_node.get_children();
                  std::transform(children.begin(), children.end(),
                                 std::back_inserter(retval),
                                 &Carbon::var<const std::reference_wrapper<
                                     Carbon::AST_Node> &>);
                  return retval;
              }),
              "children"}});
