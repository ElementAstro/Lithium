...
#ifndef CARBON_BOOTSTRAP_STL_HPP
#define CARBON_BOOTSTRAP_STL_HPP
...
struct Bidir_Range {
    ...
    constexpr Bidir_Range(Container &c) : m_begin(c.begin()), m_end(c.end()) {}
...
void input_range_type_impl(const std::string &type, Module &m) {
    ...
    m.add(constructor<Bidir_Type(typename Bidir_Type::container_type &)>(),
          "range_internal");
...
void input_range_type(const std::string &type, Module &m) {
    detail::input_range_type_impl<
        Bidir_Range<ContainerType, typename ContainerType::iterator>>(type, m);
    detail::input_range_type_impl<Bidir_Range<
        const ContainerType, typename ContainerType::const_iterator>>(
        "Const_" + type, m);
}
...
void random_access_container_type(const std::string & /*type*/, Module &m) {
    ...
    // for [] access, to throw an exception in an out of bounds condition.
    m.add(fun([](ContainerType &c, int index) ->
              typename ContainerType::reference {
                  /// \todo we are preferring to keep the key as 'int' to avoid
                  /// runtime conversions during dispatch. reevaluate
                  return c.at(
                      static_cast<typename ContainerType::size_type>(index));
              }),
          "[]");

    m.add(fun([](const ContainerType &c, int index) ->
...
void reservable_type(const std::string & /*type*/, Module &m) {
    m.add(fun([](ContainerType *a, typename ContainerType::size_type n) {
          ...
          "reserve");
    m.add(fun([](const ContainerType *a) { return a->capacity(); }),
          "capacity");
}
...
void back_insertion_sequence_type(const std::string &type, Module &m) {
    ...
    using push_back =
        void (ContainerType::*)(const typename ContainerType::value_type &);
    m.add(fun(static_cast<push_back>(&ContainerType::push_back)),
          [&]() -> std::string {
              if (typeid(typename ContainerType::value_type) ==
                  typeid(Boxed_Value)) {
                  m.eval(
                      "# Pushes the second value onto the container while "
                      "making a clone of the value\n"
                      "def push_back(" +
                      type +
                      " container, x)\n"
                      "{ \n"
    ...
    m.add(fun(static_cast<push_ptr>(&ContainerType::push_front)),
          [&]() -> std::string {
              if (typeid(typename ContainerType::value_type) ==
                  typeid(Boxed_Value)) {
                  m.eval(
                      "# Pushes the second value onto the front of container "
                      "while making a clone of the value\n"
                      "def push_front(" +
                      type +
                      " container, x)\n"
                      "{ \n"
...
void unique_associative_container_type(const std::string & /*type*/,
    ...
    using erase_ptr =
        size_t (ContainerType::*)(const typename ContainerType::key_type &);
    ...
    m.add(fun(&detail::insert_ref<ContainerType>), []() -> std::string {
        if (typeid(typename ContainerType::mapped_type) ==
            typeid(Boxed_Value)) {
            return "insert_ref";
        } else {
            return "insert";
...
void map_type(const std::string &type, Module &m) {
    ...
    using elem_access = typename MapType::mapped_type &(
        MapType::*)(const typename MapType::key_type &);
    using const_elem_access = const typename MapType::mapped_type &(
        MapType::*)(const typename MapType::key_type &) const;
...
void string_type(const std::string &type, Module &m) {
    ...
    // Special case: add push_back to string (which doesn't support other
    // back_insertion operations
    ...
    m.add(fun([](const String *s, const String &f, size_t pos) {
              return s->find(f, pos);
          }),
          "find");
    m.add(fun([](const String *s, const String &f, size_t pos) {
              return s->rfind(f, pos);
          }),
          "rfind");
    m.add(fun([](const String *s, const String &f, size_t pos) {
              return s->find_first_of(f, pos);
          }),
          "find_first_of");
    m.add(fun([](const String *s, const String &f, size_t pos) {
              return s->find_last_of(f, pos);
          }),
          "find_last_of");
    m.add(fun([](const String *s, const String &f, size_t pos) {
              return s->find_last_not_of(f, pos);
          }),
          "find_last_not_of");
    m.add(fun([](const String *s, const String &f, size_t pos) {
              return s->find_first_not_of(f, pos);
          }),
          "find_first_not_of");

    m.add(fun([](String *s, typename String::value_type c) -> decltype(auto) {
              return (*s += c);
          }),
          "+=");
    ...
    m.add(fun([](const String *s, size_t pos, size_t len) {
