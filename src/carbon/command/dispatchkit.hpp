    ...
    bool has_function(const Proxy_Function &new_f,
        ...
        return std::any_of(
            ...
            [&](const std::pair<Proxy_Function, std::string> &existing_f) {
                return existing_f.second == name &&
                       *(existing_f.first) == *(new_f);
            });
    }
...
public:
    explicit Dispatch_Function(std::vector<Proxy_Function> t_funcs)
        : Proxy_Function_Base(build_type_infos(t_funcs),
                              calculate_arity(t_funcs)),
          m_funcs(std::move(t_funcs)) {}

    bool operator==(
        const dispatch::Proxy_Function_Base &rhs) const noexcept override {
        try {
            const auto &dispatch_fun =
                dynamic_cast<const Dispatch_Function &>(rhs);
    ...
    std::vector<Const_Proxy_Function> get_contained_functions() const override {
        return std::vector<Const_Proxy_Function>(m_funcs.begin(),
                                                 m_funcs.end());
    }
    ...
    static std::vector<Type_Info> build_type_infos(
        ...
        if (begin != end) {
            ...
            while (begin != end) {
                ...
                for (size_t i = 0;
                     i < type_infos.size() && i < param_types.size(); ++i) {
                    if (!(type_infos[i] == param_types[i])) {
                        type_infos[i] =
                            detail::Get_Type_Info<Boxed_Value>::get();
                    }
                }
...
public:
    using Type_Name_Map =
        std::map<std::string, Carbon::Type_Info, str_less>;
    using Scope = utility::QuickFlatMap<std::string, Boxed_Value, str_equal>;
    using StackData = Stack_Holder::StackData;

    struct State {
        utility::QuickFlatMap<std::string,
                              std::shared_ptr<std::vector<Proxy_Function>>,
                              str_equal>
            m_functions;
        utility::QuickFlatMap<std::string, Proxy_Function, str_equal>
            m_function_objects;
        utility::QuickFlatMap<std::string, Boxed_Value, str_equal>
            m_boxed_functions;
    ...
    Boxed_Value &add_get_object(std::string t_name, Boxed_Value obj,
        ...
        if (auto result =
            ...
            // insert failed
            throw Carbon::exception::name_conflict_error(
                result.first->first);
        }
    }
    ...
    void add_global_const(const Boxed_Value &obj, const std::string &name) {
        ...
        if (m_state.m_global_objects.find(name) !=
    ...
    void set_global(Boxed_Value obj, std::string name) {
        Carbon::detail::threading::unique_lock<
            Carbon::detail::threading::shared_mutex>
            l(m_mutex);
        m_state.m_global_objects.insert_or_assign(std::move(name),
                                                  std::move(obj));
    }
    ...
    Boxed_Value get_object(std::string_view name,
        ...
        if (loc == 0) {
            ...
            for (auto stack_elem = stack.rbegin(); stack_elem != stack.rend();
                 ++stack_elem) {
                for (auto s = stack_elem->begin(); s != stack_elem->end();
                     ++s) {
                    if (s->first == name) {
                        t_loc = static_cast<uint_fast32_t>(
                                    std::distance(stack.rbegin(), stack_elem)
                                    << 16) |
    ...
    get_function(std::string_view t_name, const size_t t_hint) const {
        ...
        if (const auto itr = funs.find(t_name, t_hint); itr != funs.end()) {
            return std::make_pair(std::distance(funs.begin(), itr),
                                  itr->second);
        } else {
            return std::make_pair(
                size_t(0), std::make_shared<std::vector<Proxy_Function>>());
        }
    }
...
#pragma warning(disable : 4715)
    ...
    Boxed_Value call_member(const std::string &t_name,
        ...
        const auto do_attribute_call =
            ...
            Boxed_Value bv =
                dispatch::dispatch(l_funs, attr_params, l_conversions);
        ...
        if (is_attribute_call(*funs.second, params, t_has_params,
            ...
            if (!funs.second->empty()) {
                try {
                    return dispatch::dispatch(*funs.second, params,
                                              t_conversions);
            ...
            if (except) {
                ...
                throw Carbon::exception::dispatch_error(
                    params, std::vector<Const_Proxy_Function>(
                                funs.second->begin(), funs.second->end()));
            }
        }
    }
#ifdef CARBON_MSVC
#pragma warning(pop)
    ...
    Boxed_Value call_exists(const Function_Params &params) const {
        if (params.empty()) {
            throw Carbon::exception::arity_error(
    ...
    void add_function(const Proxy_Function &t_f, const std::string &t_name) {
        ...
        Proxy_Function new_func = [&]() -> Proxy_Function {
            ...
            if (itr != funcs.end()) {
                ...
                itr->second =
                    std::make_shared<std::vector<Proxy_Function>>(vec);
        ...
        get_function_objects_int().insert_or_assign(t_name,
                                                    std::move(new_func));
    }
