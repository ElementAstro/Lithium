...
#ifndef CARBON_HANDLE_RETURN_HPP
#define CARBON_HANDLE_RETURN_HPP
...
struct Handle_Return {
    template <typename T, typename = typename std::enable_if_t<
                              std::is_trivial_v<typename std::decay_t<T>>>>
        ...
        return Boxed_Value(
            Carbon::make_shared<dispatch::Proxy_Function_Base,
                                dispatch::Proxy_Function_Callable_Impl<
                                    Ret, std::function<Ret>>>(f));
    }
};

template <typename Ret>
struct Handle_Return<std::function<Ret>>
    : Handle_Return<const std::function<Ret> &> {};
...
template <typename Ret>
struct Handle_Return<std::shared_ptr<Ret>>
    : Handle_Return<std::shared_ptr<Ret> &> {};
...
template <typename Ret>
struct Handle_Return<std::unique_ptr<Ret>>
    : Handle_Return<std::unique_ptr<Ret> &> {
...
struct Handle_Return_Ref<Ret, true> {
    template <typename T>
    static Boxed_Value handle(T &&r) {
        return Boxed_Value(typename std::remove_reference<decltype(r)>::type{r},
                           true);
    }
};
