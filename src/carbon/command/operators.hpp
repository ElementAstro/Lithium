...
#ifndef CARBON_OPERATORS_HPP
#define CARBON_OPERATORS_HPP

#include "../defines.hpp"
#include "dispatchkit.hpp"
#include "register_function.hpp"

namespace Carbon::bootstrap::operators {
template <typename T>
void assign(Module &m) {
    m.add(Carbon::fun([](T &lhs, const T &rhs) -> T & { return lhs = rhs; }),
          "=");
}
...
void prefix_decrement(Module &m) {
    m.add(Carbon::fun([](T &lhs) -> T & { return --lhs; }), "--");
}
...
template <typename T>
void equal(Module &m) {
    m.add(Carbon::fun([](const T &lhs, const T &rhs) { return lhs == rhs; }),
          "==");
}
...
void logical_compliment(Module &m) {
    m.add(Carbon::fun([](const T &lhs) { return !lhs; }), "!");
}
