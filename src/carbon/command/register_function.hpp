...
// this version peels off the function object itself from the function
// signature, when used on a callable object
...
template <typename Func, typename Ret, typename... Param, bool Is_Noexcept,
          bool Is_Member, bool Is_MemberObject>
auto make_callable(
    Func &&func, Function_Signature<Ret, Function_Params<Param...>, Is_Noexcept,
                                    Is_Member, Is_MemberObject, false>
                     fs) {
    return make_callable_impl(std::forward<Func>(func), fs);
}
}  // namespace dispatch::detail

/// \brief Creates a new Proxy_Function object from a free function, member
/// function or data member \param[in] t Function / member to expose
///
/// \b Example:
/// \code
...
/// \brief Creates a new Proxy_Function object from a free function, member
/// function or data member and binds the first parameter of it \param[in] t
/// Function / member to expose \param[in] q Value to bind to first parameter
///
/// \b Example:
/// \code
/// struct MyClass
...
/// MyClass obj;
/// Carbon::Carbon chai;
/// // Add function taking only one argument, an int, and permanently bound to
/// "obj" chai.add(fun(&MyClass::memberfunction, std::ref(obj)),
/// "memberfunction"); \endcode
///
/// \sa \ref adding_functions
