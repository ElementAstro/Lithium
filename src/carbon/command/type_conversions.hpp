...
#ifndef CARBON_DYNAMIC_CAST_CONVERSION_HPP
#define CARBON_DYNAMIC_CAST_CONVERSION_HPP
    ...
    static Boxed_Value cast(const Boxed_Value &t_from) {
        if (t_from.get_type_info().bare_equal(Carbon::user_type<From>())) {
            if (t_from.is_pointer()) {
                ...
                if (t_from.is_const()) {
                    return Boxed_Value([&]() {
                        if (auto data = std::static_pointer_cast<const To>(
                                detail::Cast_Helper<std::shared_ptr<
                    ...
                    return Boxed_Value([&]() {
                        if (auto data = std::static_pointer_cast<To>(
                                detail::Cast_Helper<
                                    std::shared_ptr<From>>::cast(t_from,
                                                                 nullptr))) {
                            return data;
                        } else {
                            throw std::bad_cast();
                ...
                // Pull the reference out of the contained boxed value, which we
                // know is the type we want
                if (t_from.is_const()) {
                    const From &d = detail::Cast_Helper<const From &>::cast(
                        t_from, nullptr);
                    const To &data = static_cast<const To &>(d);
                    return Boxed_Value(std::cref(data));
                } else {
                    From &d =
                        detail::Cast_Helper<From &>::cast(t_from, nullptr);
            ...
            throw Carbon::exception::bad_boxed_dynamic_cast(
                t_from.get_type_info(), typeid(To),
                ...
                if (t_from.is_const()) {
                    ...
                    return Boxed_Value([&]() {
                        if (auto data = std::dynamic_pointer_cast<To>(
                            ...
                            if (std::string(typeid(To).name())
                                    .find("Assignable_Proxy_Function") !=
                                std::string::npos) {
...
public:
    ...
    Boxed_Value convert_down(const Boxed_Value &t_base) const override {
        throw Carbon::exception::bad_boxed_dynamic_cast(
            ...
            "Unable to cast down inheritance hierarchy with non-polymorphic "
            "types");
    }
...
public:
    ...
    Boxed_Value convert_down(const Boxed_Value &) const override {
        throw Carbon::exception::bad_boxed_type_cast("No conversion exists");
    }
...
public:
    ...
    struct Less_Than {
        bool operator()(const std::type_info *t_lhs,
                        const std::type_info *t_rhs) const noexcept {
            return *t_lhs != *t_rhs && t_lhs->before(*t_rhs);
        }
    };

    Type_Conversions()
        : m_mutex(), m_conversions(), m_convertableTypes(), m_num_types(0) {}
    ...
    void add_conversion(
        ...
        Carbon::detail::threading::unique_lock<
            Carbon::detail::threading::shared_mutex>
            l(m_mutex);
        if (find_bidir(conversion->to(), conversion->from()) !=
            m_conversions.end()) {
            throw exception::conversion_error(
                conversion->to(), conversion->from(),
                "Trying to re-insert an existing conversion!");
        }
        m_conversions.insert(conversion);
        m_convertableTypes.insert({conversion->to().bare_type_info(),
    ...
    Boxed_Value boxed_type_conversion(const Type_Info &to,
        ...
        try {
            Boxed_Value ret =
                get_conversion(to, from.get_type_info())->convert(from);
            if (t_saves.enabled) {
            ...
            throw exception::bad_boxed_dynamic_cast(
                from.get_type_info(), *to.bare_type_info(),
    ...
    Boxed_Value boxed_type_down_conversion(const Type_Info &from,
        ...
        try {
            Boxed_Value ret =
                get_conversion(to.get_type_info(), from)->convert_down(to);
            if (t_saves.enabled) {
                t_saves.saves.push_back(ret);
            }
            return ret;
            ...
            throw exception::bad_boxed_dynamic_cast(
                to.get_type_info(), *from.bare_type_info(),
                "Unable to perform dynamic_cast operation");
        }
    }
    ...
    std::shared_ptr<detail::Type_Conversion_Base> get_conversion(
        ...
        if (itr != m_conversions.end()) {
            ...
            throw std::out_of_range(
                std::string("No such conversion exists from ") +
                from.bare_name() + " to " + to.bare_name());
        }
    }
...
private:
    ...
    find_bidir(const Type_Info &to, const Type_Info &from) const {
        return std::find_if(
            ...
            [&to, &from](
                const std::shared_ptr<detail::Type_Conversion_Base> &conversion)
                -> bool {
                return (conversion->to().bare_equal(to) &&
                        conversion->from().bare_equal(from)) ||
                       (conversion->bidir() &&
                        conversion->from().bare_equal(to) &&
    ...
    std::set<std::shared_ptr<detail::Type_Conversion_Base>> get_conversions()
        const {
        Carbon::detail::threading::shared_lock<
            Carbon::detail::threading::shared_mutex>
            l(m_mutex);
    ...
    mutable Carbon::detail::threading::Thread_Storage<
        std::set<const std::type_info *, Less_Than>>
        m_thread_cache;
    mutable Carbon::detail::threading::Thread_Storage<Conversion_Saves>
        m_conversion_saves;
};
...
using Type_Conversion = std::shared_ptr<Carbon::detail::Type_Conversion_Base>;

/// \brief Used to register a to / parent class relationship with ChaiScript.
/// Necessary if you
///        want automatic conversions up your inheritance hierarchy.
///
/// Create a new to class registration for applying to a module or to the
/// ChaiScript engine Currently, due to limitations in module loading on
/// Windows, and for the sake of portability, if you have a type that is
/// introduced in a loadable module and is used by multiple modules (through a
/// tertiary dll that is shared between the modules, static linking the new type
/// into both loadable modules would not be portable), you need to register the
/// type relationship in all modules that use the newly added type in a
/// polymorphic way.
///
/// Example:
/// \code
...
/// chai.add(Carbon::to_class<Base, Derived>());
/// \endcode
///
template <typename Base, typename Derived>
Type_Conversion base_class() {
    // Can only be used with related polymorphic types
    // may be expanded some day to support conversions other than child ->
    // parent
    ...
    if constexpr (std::is_polymorphic<Base>::value &&
        ...
        return Carbon::make_shared<
            detail::Type_Conversion_Base,
...
Type_Conversion type_conversion(const Callable &t_function) {
    auto func = [t_function](const Boxed_Value &t_bv) -> Boxed_Value {
        // not even attempting to call boxed_cast so that we don't get caught in
        // some call recursion
        return Carbon::Boxed_Value(
...
Type_Conversion type_conversion() {
    ...
    auto func = [](const Boxed_Value &t_bv) -> Boxed_Value {
        ...
        // some call recursion
        return Carbon::Boxed_Value(
            To(detail::Cast_Helper<From>::cast(t_bv, nullptr)));
    };
...
Type_Conversion vector_conversion() {
    auto func = [](const Boxed_Value &t_bv) -> Boxed_Value {
        const std::vector<Boxed_Value> &from_vec =
            detail::Cast_Helper<const std::vector<Boxed_Value> &>::cast(
                t_bv, nullptr);

        To vec;
        vec.reserve(from_vec.size());
        for (const Boxed_Value &bv : from_vec) {
            vec.push_back(detail::Cast_Helper<typename To::value_type>::cast(
                bv, nullptr));
        }
...
Type_Conversion map_conversion() {
    auto func = [](const Boxed_Value &t_bv) -> Boxed_Value {
        const std::map<std::string, Boxed_Value> &from_map =
            detail::Cast_Helper<
                const std::map<std::string, Boxed_Value> &>::cast(t_bv,
                                                                  nullptr);

        To map;
        for (const std::pair<const std::string, Boxed_Value> &p : from_map) {
            map.insert(std::make_pair(
                p.first, detail::Cast_Helper<typename To::mapped_type>::cast(
                             p.second, nullptr)));
        }
