...
#ifndef CARBON_DEFINES_HPP
#define CARBON_DEFINES_HPP

#ifdef _MSC_VER
#define CARBON_STRINGIZE(x) "" #x
#define CARBON_STRINGIZE_EXPANDED(x) CARBON_STRINGIZE(x)
#define CARBON_COMPILER_VERSION CARBON_STRINGIZE_EXPANDED(_MSC_FULL_VER)
#define CARBON_MSVC _MSC_VER
#define CARBON_HAS_DECLSPEC

static_assert(_MSC_FULL_VER >= 190024210,
...
#define CARBON_LIBCPP
#endif

#if defined(_WIN32) || defined(__CYGWIN__)
#define CARBON_WINDOWS
#endif

#if defined(_WIN32)
#if defined(__llvm__)
#define CARBON_COMPILER_NAME "clang(windows)"
#elif defined(__GNUC__)
#define CARBON_COMPILER_NAME "gcc(mingw)"
#else
#define CARBON_COMPILER_NAME "msvc"
#endif
#else
#if defined(__llvm__)
#define CARBON_COMPILER_NAME "clang"
#elif defined(__GNUC__)
#define CARBON_COMPILER_NAME "gcc"
#else
#define CARBON_COMPILER_NAME "unknown"
#endif
#endif
...
#ifdef CARBON_HAS_DECLSPEC
#define CARBON_MODULE_EXPORT extern "C" __declspec(dllexport)
#else
#define CARBON_MODULE_EXPORT extern "C"
#endif

#if defined(CARBON_MSVC) || (defined(__GNUC__) && __GNUC__ >= 5) || \
    defined(CARBON_CLANG)
#define CARBON_UTF16_UTF32
#endif

#ifdef _DEBUG
#define CARBON_DEBUG true
#else
#define CARBON_DEBUG false
#endif
...
constexpr static const char *compiler_version = CARBON_COMPILER_VERSION;
constexpr static const char *compiler_name = CARBON_COMPILER_NAME;
constexpr static const bool debug_build = CARBON_DEBUG;

template <typename B, typename D, typename... Arg>
inline std::shared_ptr<B> make_shared(Arg &&...arg) {
#ifdef CARBON_USE_STD_MAKE_SHARED
    return std::make_shared<D>(std::forward<Arg>(arg)...);
#else
    return std::shared_ptr<B>(
        static_cast<B *>(new D(std::forward<Arg>(arg)...)));
#endif
}
...
struct str_equal {
    ...
    [[nodiscard]] constexpr bool operator()(const LHS &t_lhs,
        ...
        return std::equal(t_lhs.begin(), t_lhs.end(), t_rhs.begin(),
                          t_rhs.end());
    }
