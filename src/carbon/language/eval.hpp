...
#ifndef CARBON_EVAL_HPP
#define CARBON_EVAL_HPP
...
namespace detail {
/// Helper function that will set up the scope around a function call, including
/// handling the named function parameters
template <typename T>
Boxed_Value eval_function(
    Carbon::detail::Dispatch_Engine &t_ss, const AST_Node_Impl<T> &t_node,
    ...
    Carbon::detail::Dispatch_State state(t_ss);
    ...
    Carbon::eval::detail::Stack_Push_Pop tpp(state);
...
inline Boxed_Value clone_if_necessary(
    Boxed_Value incoming, std::atomic_uint_fast32_t &t_loc,
    ...
    if (!incoming.is_return_value()) {
        if (incoming.get_type_info().is_arithmetic()) {
            return Boxed_Number::clone(incoming);
                       ...
                       typeid(bool))) {
            return Boxed_Value(
                *static_cast<const bool *>(incoming.get_const_ptr()));
        } else if (incoming.get_type_info().bare_equal_type_info(
                       typeid(std::string))) {
            return Boxed_Value(
                *static_cast<const std::string *>(incoming.get_const_ptr()));
        } else {
            std::array<Boxed_Value, 1> params{std::move(incoming)};
            return t_ss->call_function("clone", t_loc, Function_Params{params},
                                       t_ss.conversions());
        }
    } else {
...
struct AST_Node_Impl : AST_Node {
    AST_Node_Impl(std::string t_ast_node_text, AST_Node_Type t_id,
    ...
    static bool get_scoped_bool_condition(
        ...
        const Carbon::detail::Dispatch_State &t_ss) {
        Carbon::eval::detail::Scope_Push_Pop spp(t_ss);
    ...
    Boxed_Value eval(
        const Carbon::detail::Dispatch_State &t_e) const final {
        try {
            T::trace(t_e, this);
...
protected:
    virtual Boxed_Value eval_internal(
        const Carbon::detail::Dispatch_State &) const {
...
struct Compiled_AST_Node : AST_Node_Impl<T> {
    Compiled_AST_Node(AST_Node_Impl_Ptr<T> t_original_node,
                      std::vector<AST_Node_Impl_Ptr<T>> t_children,
                      std::function<Boxed_Value(
                          const std::vector<AST_Node_Impl_Ptr<T>> &,
                          const Carbon::detail::Dispatch_State &t_ss)>
                          t_func)
        : AST_Node_Impl<T>(t_original_node->text, AST_Node_Type::Compiled,
                           t_original_node->location, std::move(t_children)),
          m_func(std::move(t_func)),
          m_original_node(std::move(t_original_node)) {}

    Boxed_Value eval_internal(
        const Carbon::detail::Dispatch_State &t_ss) const override {
...
struct Fold_Right_Binary_Operator_AST_Node : AST_Node_Impl<T> {
    Fold_Right_Binary_Operator_AST_Node(
        const std::string &t_oper, Parse_Location t_loc,
        std::vector<AST_Node_Impl_Ptr<T>> t_children, Boxed_Value t_rhs)
        : AST_Node_Impl<T>(t_oper, AST_Node_Type::Binary, std::move(t_loc),
                           std::move(t_children)),
...
protected:
    Boxed_Value do_oper(const Carbon::detail::Dispatch_State &t_ss,
                        const std::string &t_oper_string,
                        const Boxed_Value &t_lhs) const {
        try {
            if (t_lhs.get_type_info().is_arithmetic()) {
                // If it's an arithmetic operation we want to short circuit
                // dispatch
                try {
                    return Boxed_Number::do_oper(m_oper, t_lhs, m_rhs);
                } catch (const Carbon::exception::arithmetic_error &) {
                    throw;
                } catch (...) {
                    throw exception::eval_error(
                        "Error with numeric operator calling: " +
                        t_oper_string);
                }
            } else {
                Carbon::eval::detail::Function_Push_Pop fpp(t_ss);
                std::array<Boxed_Value, 2> params{t_lhs, m_rhs};
                fpp.save_params(Function_Params{params});
                return t_ss->call_function(t_oper_string, m_loc,
                                           Function_Params{params},
                                           t_ss.conversions());
            }
        } catch (const exception::dispatch_error &e) {
            throw exception::eval_error(
                "Can not find appropriate '" + t_oper_string + "' operator.",
...
protected:
    Boxed_Value do_oper(const Carbon::detail::Dispatch_State &t_ss,
        ...
        try {
            if (t_oper != Operators::Opers::invalid &&
                t_lhs.get_type_info().is_arithmetic() &&
...
struct Constant_AST_Node final : AST_Node_Impl<T> {
    Constant_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                      Boxed_Value t_value)
        : AST_Node_Impl<T>(t_ast_node_text, AST_Node_Type::Constant,
                           std::move(t_loc)),
    ...
    Boxed_Value eval_internal(
        const Carbon::detail::Dispatch_State &) const override {
        return m_value;
    }
...
struct Id_AST_Node final : AST_Node_Impl<T> {
    Id_AST_Node(const std::string &t_ast_node_text, Parse_Location t_loc)
        : AST_Node_Impl<T>(t_ast_node_text, AST_Node_Type::Id,
                           std::move(t_loc)) {}
...
struct Fun_Call_AST_Node : AST_Node_Impl<T> {
    Fun_Call_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                      std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Fun_Call,
                           std::move(t_loc), std::move(t_children)) {
        assert(!this->children.empty());
    }

    template <bool Save_Params>
    Boxed_Value do_eval_internal(
        const Carbon::detail::Dispatch_State &t_ss) const {
        Carbon::eval::detail::Function_Push_Pop fpp(t_ss);
        ...
        try {
            return (*t_ss->boxed_cast<const dispatch::Proxy_Function_Base *>(
                fn))(Function_Params{params}, t_ss.conversions());
        } catch (const exception::dispatch_error &e) {
            throw exception::eval_error(
                std::string(e.what()) + " with function '" +
            ...
            try {
                ...
                Const_Proxy_Function f =
                    t_ss->boxed_cast<ConstFunctionTypeRef>(fn);
                // handle the case where there is only 1 function to try to call
                // and dispatch fails on it
                throw exception::eval_error(
                    "Error calling function '" + this->children[0]->text + "'",
                    params, make_vector(f), false, *t_ss);
            } catch (const exception::bad_boxed_cast &) {
                throw exception::eval_error(
...
struct Unused_Return_Fun_Call_AST_Node final : Fun_Call_AST_Node<T> {
    Unused_Return_Fun_Call_AST_Node(
        std::string t_ast_node_text, Parse_Location t_loc,
        std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : Fun_Call_AST_Node<T>(std::move(t_ast_node_text), std::move(t_loc),
                               std::move(t_children)) {}
...
struct Arg_AST_Node final : AST_Node_Impl<T> {
    Arg_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                 std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Arg_List,
                           std::move(t_loc), std::move(t_children)) {}
};
...
struct Arg_List_AST_Node final : AST_Node_Impl<T> {
    ...
    static std::vector<std::string> get_arg_names(
    ...
    static std::pair<std::string, Type_Info> get_arg_type(
        const AST_Node_Impl<T> &t_node,
        const Carbon::detail::Dispatch_State &t_ss) {
        if (t_node.children.size() < 2) {
            ...
            return {t_node.children[0]->text,
                    t_ss->get_type(t_node.children[0]->text, false)};
        }
    }

    static dispatch::Param_Types get_arg_types(
        const AST_Node_Impl<T> &t_node,
        const Carbon::detail::Dispatch_State &t_ss) {
...
struct Equation_AST_Node final : AST_Node_Impl<T> {
    Equation_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                      std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Equation,
                           std::move(t_loc), std::move(t_children)),
    ...
    Boxed_Value eval_internal(
        ...
        auto params = [&]() {
            ...
            // if we declare the variable in scope first, then the name lookup
            // fails for the RHS
            auto rhs = this->children[1]->eval(t_ss);
            auto lhs = this->children[0]->eval(t_ss);
        ...
        if (m_oper != Operators::Opers::invalid &&
            ...
            try {
                ...
                throw exception::eval_error(
                    "Error with unsupported arithmetic assignment operation.");
            }
        } else if (m_oper == Operators::Opers::assign) {
            try {
                if (params[0].is_undef()) {
                    if (!this->children.empty() &&
                        ((this->children[0]->identifier ==
                          AST_Node_Type::Reference) ||
                         (!this->children[0]->children.empty() &&
                          this->children[0]->children[0]->identifier ==
                              AST_Node_Type::Reference)))

                    {
                        /// \todo This does not handle the case of an unassigned
                        /// reference variable
                        ...
                        params[1] = detail::clone_if_necessary(
                            std::move(params[1]), m_clone_loc, t_ss);
                    }
                }

                try {
                    return t_ss->call_function(this->text, m_loc,
                                               Function_Params{params},
                                               t_ss.conversions());
                } catch (const exception::dispatch_error &e) {
                    throw exception::eval_error("Unable to find appropriate'" +
                                                    this->text + "' operator.",
                ...
                throw exception::eval_error(
                    "Missing clone or copy constructor for right hand side of "
                    "equation",
                    e.parameters, e.functions, false, *t_ss);
            }
        } else if (this->text == ":=") {
            if (params[0].is_undef() ||
                Boxed_Value::type_match(params[0], params[1])) {
                params[0].assign(params[1]);
                params[0].reset_return_value();
            } else {
            ...
            try {
                ...
                throw exception::eval_error(
                    "Unable to find appropriate'" + this->text + "' operator.",
                    e.parameters, e.functions, false, *t_ss);
            }
        }
...
struct Global_Decl_AST_Node final : AST_Node_Impl<T> {
    Global_Decl_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
        ...
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Global_Decl, std::move(t_loc),
                           std::move(t_children)) {}
...
struct Var_Decl_AST_Node final : AST_Node_Impl<T> {
    Var_Decl_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                      std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Var_Decl,
                           std::move(t_loc), std::move(t_children)) {}
...
struct Assign_Decl_AST_Node final : AST_Node_Impl<T> {
    Assign_Decl_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                         std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Assign_Decl, std::move(t_loc),
                           std::move(t_children)) {}
    ...
    Boxed_Value eval_internal(
        ...
        try {
            Boxed_Value bv(detail::clone_if_necessary(
                this->children[1]->eval(t_ss), m_loc, t_ss));
...
struct Array_Call_AST_Node final : AST_Node_Impl<T> {
    Array_Call_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                        std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Array_Call, std::move(t_loc),
                           std::move(t_children)) {}
    ...
    Boxed_Value eval_internal(
        ...
        std::array<Boxed_Value, 2> params{this->children[0]->eval(t_ss),
                                          this->children[1]->eval(t_ss)};

        try {
            fpp.save_params(Function_Params{params});
            return t_ss->call_function("[]", m_loc, Function_Params{params},
                                       t_ss.conversions());
        } catch (const exception::dispatch_error &e) {
            throw exception::eval_error(
                "Can not find appropriate array lookup operator '[]'.",
                e.parameters, e.functions, false, *t_ss);
        }
    }
...
struct Dot_Access_AST_Node final : AST_Node_Impl<T> {
    Dot_Access_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                        std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Dot_Access, std::move(t_loc),
                           std::move(t_children)),
          m_fun_name(
    ...
    Boxed_Value eval_internal(
        ...
        try {
            retval =
                t_ss->call_member(m_fun_name, m_loc, Function_Params{params},
                                  has_function_params, t_ss.conversions());
        } catch (const exception::dispatch_error &e) {
            if (e.functions.empty()) {
                throw exception::eval_error("'" + m_fun_name +
                                            "' is not a function.");
            } else {
                throw exception::eval_error(
                    std::string(e.what()) + " for function '" + m_fun_name +
                        "'",
        ...
        if (this->children[1]->identifier == AST_Node_Type::Array_Call) {
            try {
                std::array<Boxed_Value, 2> p{
                    retval, this->children[1]->children[1]->eval(t_ss)};
                retval = t_ss->call_function(
                    "[]", m_array_loc, Function_Params{p}, t_ss.conversions());
            } catch (const exception::dispatch_error &e) {
                throw exception::eval_error(
                    "Can not find appropriate array lookup operator '[]'.",
                    e.parameters, e.functions, true, *t_ss);
...
struct Lambda_AST_Node final : AST_Node_Impl<T> {
    Lambda_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                    std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(
              t_ast_node_text, AST_Node_Type::Lambda, std::move(t_loc),
              std::vector<AST_Node_Impl_Ptr<T>>(
                  std::make_move_iterator(t_children.begin()),
                  std::make_move_iterator(std::prev(t_children.end())))),
          m_param_names(
              Arg_List_AST_Node<T>::get_arg_names(*this->children[1])),
    ...
    Boxed_Value eval_internal(
        ...
        const auto captures = [&]() -> std::map<std::string, Boxed_Value> {
            ...
            for (const auto &capture : this->children[0]->children) {
                named_captures.insert(
                    std::make_pair(capture->children[0]->text,
                                   capture->children[0]->eval(t_ss)));
            }
        ...
        const auto param_types =
            Arg_List_AST_Node<T>::get_arg_types(*this->children[1], t_ss);

        std::reference_wrapper<Carbon::detail::Dispatch_Engine> engine(
            *t_ss);

        return Boxed_Value(dispatch::make_dynamic_proxy_function(
            [engine, lambda_node = this->m_lambda_node,
             param_names = this->m_param_names, captures,
             this_capture =
                 this->m_this_capture](const Function_Params &t_params) {
                return detail::eval_function(engine, *lambda_node, param_names,
                                             t_params, &captures, this_capture);
            },
            static_cast<int>(numparams), m_lambda_node, param_types));
    }

    static bool has_this_capture(
...
struct Scopeless_Block_AST_Node final : AST_Node_Impl<T> {
    Scopeless_Block_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                             std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Scopeless_Block, std::move(t_loc),
                           std::move(t_children)) {}
...
struct Block_AST_Node final : AST_Node_Impl<T> {
    Block_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                   std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Block,
                           std::move(t_loc), std::move(t_children)) {}
...
struct Def_AST_Node final : AST_Node_Impl<T> {
    ...
    Def_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                 std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(
              std::move(t_ast_node_text), AST_Node_Type::Def, std::move(t_loc),
              std::vector<AST_Node_Impl_Ptr<T>>(
                  std::make_move_iterator(t_children.begin()),
                  std::make_move_iterator(std::prev(
                      t_children.end(), has_guard(t_children, 1) ? 2 : 1)))),
          // This apparent use after move is safe because we are only moving out
          // the specific elements we need on each operation.
          m_body_node(get_body_node(std::move(t_children))),
          m_guard_node(
              get_guard_node(std::move(t_children),
                             t_children.size() - this->children.size() == 2))

    {}

    static std::shared_ptr<AST_Node_Impl<T>> get_guard_node(
        std::vector<AST_Node_Impl_Ptr<T>> &&vec, bool has_guard) {
        if (has_guard) {
    ...
    static std::shared_ptr<AST_Node_Impl<T>> get_body_node(
        std::vector<AST_Node_Impl_Ptr<T>> &&vec) {
    ...
    Boxed_Value eval_internal(
        ...
        if ((this->children.size() > 1) &&
            (this->children[1]->identifier == AST_Node_Type::Arg_List)) {
        ...
        if (m_guard_node) {
            guard = dispatch::make_dynamic_proxy_function(
                [engine, guardnode = m_guard_node,
                 t_param_names](const Function_Params &t_params) {
                    return detail::eval_function(engine, *guardnode,
                                                 t_param_names, t_params);
                },
        ...
        try {
            ...
            t_ss->add(dispatch::make_dynamic_proxy_function(
                          [engine, func_node = m_body_node,
                           t_param_names](const Function_Params &t_params) {
                              return detail::eval_function(
                                  engine, *func_node, t_param_names, t_params);
                          },
                          static_cast<int>(numparams), m_body_node, param_types,
                          guard),
                      l_function_name);
        } catch (const exception::name_conflict_error &e) {
...
struct While_AST_Node final : AST_Node_Impl<T> {
    While_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                   std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::While,
                           std::move(t_loc), std::move(t_children)) {}
        ...
        try {
            while (this->get_scoped_bool_condition(*this->children[0], t_ss)) {
                try {
                    ...
                    // we got a continue exception, which means all of the
                    // remaining loop implementation is skipped and we just need
                    // to continue to the next condition test
                }
            }
...
struct Class_AST_Node final : AST_Node_Impl<T> {
    Class_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                   std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Class,
                           std::move(t_loc), std::move(t_children)) {}
    ...
    Boxed_Value eval_internal(
        ...
        /// \todo do this better
        // put class name in current scope so it can be looked up by the attrs
        // and methods
        t_ss.add_object("_current_class_name",
...
struct If_AST_Node final : AST_Node_Impl<T> {
    If_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::If,
                           std::move(t_loc), std::move(t_children)) {
...
struct Ranged_For_AST_Node final : AST_Node_Impl<T> {
    Ranged_For_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                        std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Ranged_For, std::move(t_loc),
                           std::move(t_children)) {
        assert(this->children.size() == 3);
    }
    ...
    Boxed_Value eval_internal(
        const Carbon::detail::Dispatch_State &t_ss) const override {
        const auto get_function = [&t_ss](const std::string &t_name,
                                          auto &t_hint) {
        ...
        const auto call_function = [&t_ss](const auto &t_funcs,
                                           const Boxed_Value &t_param) {
            return dispatch::dispatch(*t_funcs, Function_Params{t_param},
                                      t_ss.conversions());
        };
        ...
        const auto do_loop = [&loop_var_name, &t_ss,
                              this](const auto &ranged_thing) {
            try {
                for (auto &&loop_var : ranged_thing) {
                    // This scope push and pop might not be the best thing for
                    // perf but we know it's 100% correct
                    Carbon::eval::detail::Scope_Push_Pop spp(t_ss);
                    /// to-do make this if-constexpr with C++17 branch
                    if (!std::is_same<std::decay_t<decltype(loop_var)>,
                                      Boxed_Value>::value) {
                        t_ss.add_get_object(loop_var_name,
        ...
        if (range_expression_result.get_type_info().bare_equal_type_info(
                typeid(std::vector<Boxed_Value>))) {
            return do_loop(boxed_cast<const std::vector<Boxed_Value> &>(
                range_expression_result));
        } else if (range_expression_result.get_type_info().bare_equal_type_info(
                       typeid(std::map<std::string, Boxed_Value>))) {
            return do_loop(
                boxed_cast<const std::map<std::string, Boxed_Value> &>(
                    range_expression_result));
        } else {
            ...
            const auto pop_front_funcs =
                get_function("pop_front", m_pop_front_loc);

            try {
                const auto range_obj =
                    call_function(range_funcs, range_expression_result);
                while (
                    !boxed_cast<bool>(call_function(empty_funcs, range_obj))) {
                    Carbon::eval::detail::Scope_Push_Pop spp(t_ss);
                    t_ss.add_get_object(loop_var_name,
                                        call_function(front_funcs, range_obj));
                    try {
                        this->children[2]->eval(t_ss);
                    } catch (detail::Continue_Loop &) {
...
struct For_AST_Node final : AST_Node_Impl<T> {
    For_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                 std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::For,
                           std::move(t_loc), std::move(t_children)) {
...
struct Switch_AST_Node final : AST_Node_Impl<T> {
    Switch_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                    std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Switch,
                           std::move(t_loc), std::move(t_children)) {}
    ...
    Boxed_Value eval_internal(
        ...
        while (!breaking && (currentCase < this->children.size())) {
            try {
                if (this->children[currentCase]->identifier ==
                    AST_Node_Type::Case) {
                    // This is a little odd, but because want to see both the
                    // switch and the case simultaneously, I do a downcast here.
                    try {
                        std::array<Boxed_Value, 2> p{
                            match_value,
                            this->children[currentCase]->children[0]->eval(
                                t_ss)};
                        if (hasMatched || boxed_cast<bool>(t_ss->call_function(
                                              "==", m_loc, Function_Params{p},
                                              t_ss.conversions()))) {
                        ...
                        throw exception::eval_error(
                            "Internal error: case guard evaluation not "
                            "boolean");
                    }
                } else if (this->children[currentCase]->identifier ==
...
struct Case_AST_Node final : AST_Node_Impl<T> {
    Case_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                  std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Case,
                           std::move(t_loc), std::move(t_children)) {
        assert(this->children.size() ==
               2); /* how many children does it have? */
    }
...
struct Default_AST_Node final : AST_Node_Impl<T> {
    Default_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                     std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Default,
                           std::move(t_loc), std::move(t_children)) {
...
struct Inline_Array_AST_Node final : AST_Node_Impl<T> {
    Inline_Array_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                          std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Inline_Array, std::move(t_loc),
                           std::move(t_children)) {}
        ...
        try {
            ...
            if (!this->children.empty()) {
                ...
                for (const auto &child : this->children[0]->children) {
                    vec.push_back(detail::clone_if_necessary(child->eval(t_ss),
                                                             m_loc, t_ss));
                }
            }
            ...
            throw exception::eval_error(
                "Can not find appropriate 'clone' or copy constructor for "
                "vector elements");
        }
    }
...
struct Inline_Map_AST_Node final : AST_Node_Impl<T> {
    Inline_Map_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                        std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Inline_Map, std::move(t_loc),
                           std::move(t_children)) {}
        ...
        try {
            ...
            for (const auto &child : this->children[0]->children) {
                retval.insert(std::make_pair(
                    t_ss->boxed_cast<std::string>(
                        child->children[0]->eval(t_ss)),
                    detail::clone_if_necessary(child->children[1]->eval(t_ss),
                                               m_loc, t_ss)));
            }
            ...
            throw exception::eval_error(
                "Can not find appropriate copy constructor or 'clone' while "
                "inserting into Map.",
                e.parameters, e.functions, false, *t_ss);
        }
    }
...
struct Return_AST_Node final : AST_Node_Impl<T> {
    Return_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                    std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Return,
                           std::move(t_loc), std::move(t_children)) {}
...
struct File_AST_Node final : AST_Node_Impl<T> {
    File_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                  std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::File,
                           std::move(t_loc), std::move(t_children)) {}
...
struct Reference_AST_Node final : AST_Node_Impl<T> {
    Reference_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                       std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Reference,
                           std::move(t_loc), std::move(t_children)) {
        assert(this->children.size() == 1);
    }
...
struct Prefix_AST_Node final : AST_Node_Impl<T> {
    Prefix_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                    std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Prefix,
                           std::move(t_loc), std::move(t_children)),
    ...
    Boxed_Value eval_internal(
        ...
        try {
            ...
            if (m_oper != Operators::Opers::invalid &&
                m_oper != Operators::Opers::bitwise_and &&
                bv.get_type_info().is_arithmetic()) {
                if ((m_oper == Operators::Opers::pre_increment ||
                     m_oper == Operators::Opers::pre_decrement) &&
                    bv.is_const()) {
                    throw exception::eval_error(
                        "Error with prefix operator evaluation: cannot modify "
                        "constant value.");
                }
                ...
                return t_ss->call_function(
                    this->text, m_loc, Function_Params{bv}, t_ss.conversions());
            }
        } catch (const exception::dispatch_error &e) {
            throw exception::eval_error(
...
struct Break_AST_Node final : AST_Node_Impl<T> {
    Break_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                   std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Break,
                           std::move(t_loc), std::move(t_children)) {}
...
struct Continue_AST_Node final : AST_Node_Impl<T> {
    Continue_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                      std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Continue,
                           std::move(t_loc), std::move(t_children)) {}
...
struct Map_Pair_AST_Node final : AST_Node_Impl<T> {
    Map_Pair_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                      std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Map_Pair,
                           std::move(t_loc), std::move(t_children)) {}
};
...
struct Value_Range_AST_Node final : AST_Node_Impl<T> {
    Value_Range_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                         std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Value_Range, std::move(t_loc),
                           std::move(t_children)) {}
};
...
struct Inline_Range_AST_Node final : AST_Node_Impl<T> {
    Inline_Range_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                          std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Inline_Range, std::move(t_loc),
                           std::move(t_children)) {}
    ...
    Boxed_Value eval_internal(
        ...
        try {
            ...
            return t_ss->call_function("generate_range", m_loc,
                                       Function_Params{params},
                                       t_ss.conversions());
        } catch (const exception::dispatch_error &e) {
            throw exception::eval_error(
                "Unable to generate range vector, while calling "
                "'generate_range'",
                e.parameters, e.functions, false, *t_ss);
        }
    }
...
struct Try_AST_Node final : AST_Node_Impl<T> {
    Try_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                 std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Try,
                           std::move(t_loc), std::move(t_children)) {}

    Boxed_Value handle_exception(const Carbon::detail::Dispatch_State &t_ss,
        ...
        for (size_t i = 1; i < end_point; ++i) {
            Carbon::eval::detail::Scope_Push_Pop catch_scope(t_ss);
            ...
            if (catch_block.children.size() == 1) {
                ...
                break;
                       ...
                       catch_block.children.size() == 3) {
                const auto name = Arg_List_AST_Node<T>::get_arg_name(
                    *catch_block.children[0]);

                if (dispatch::Param_Types(
                        std::vector<std::pair<std::string, Type_Info>>{
                            Arg_List_AST_Node<T>::get_arg_type(
                                *catch_block.children[0], t_ss)})
                ...
                if (this->children.back()->identifier ==
                    AST_Node_Type::Finally) {
                    this->children.back()->children[0]->eval(t_ss);
                }
                throw exception::eval_error(
                    "Internal error: catch block size unrecognized");
            }
        }
...
struct Catch_AST_Node final : AST_Node_Impl<T> {
    Catch_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                   std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Catch,
                           std::move(t_loc), std::move(t_children)) {}
};
...
struct Finally_AST_Node final : AST_Node_Impl<T> {
    Finally_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                     std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Finally,
                           std::move(t_loc), std::move(t_children)) {}
};
...
struct Method_AST_Node final : AST_Node_Impl<T> {
    ...
    Method_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                    std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(
              std::move(t_ast_node_text), AST_Node_Type::Method,
              std::move(t_loc),
              std::vector<AST_Node_Impl_Ptr<T>>(
                  std::make_move_iterator(t_children.begin()),
                  std::make_move_iterator(std::prev(
                      t_children.end(),
                      Def_AST_Node<T>::has_guard(t_children, 1) ? 2 : 1)))),
          m_body_node(Def_AST_Node<T>::get_body_node(std::move(t_children))),
          m_guard_node(Def_AST_Node<T>::get_guard_node(
              std::move(t_children),
              t_children.size() - this->children.size() == 2)) {}
    ...
    Boxed_Value eval_internal(
        ...
        if ((this->children.size() > 2) &&
            ...
            param_types =
                Arg_List_AST_Node<T>::get_arg_types(*this->children[2], t_ss);
        }
        ...
        if (m_guard_node) {
            guard = dispatch::make_dynamic_proxy_function(
                [engine, t_param_names,
                 guardnode = m_guard_node](const Function_Params &t_params) {
                    return Carbon::eval::detail::eval_function(
                        engine, *guardnode, t_param_names, t_params);
                },
                static_cast<int>(numparams), m_guard_node);
        }
        ...
        try {
            ...
            if (function_name == class_name) {
                ...
                t_ss->add(
                    std::make_shared<
                        dispatch::detail::Dynamic_Object_Constructor>(
                        class_name,
                ...
                // if the type is unknown, then this generates a function that
                // looks up the type at runtime. Defining the type first before
                // this is called is better
...
struct Attr_Decl_AST_Node final : AST_Node_Impl<T> {
    Attr_Decl_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                       std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text), AST_Node_Type::Attr_Decl,
                           std::move(t_loc), std::move(t_children)) {}
...
struct Logical_And_AST_Node final : AST_Node_Impl<T> {
    Logical_And_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                         std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Logical_And, std::move(t_loc),
                           std::move(t_children)) {
        assert(this->children.size() == 2);
    }
    ...
    Boxed_Value eval_internal(
        const Carbon::detail::Dispatch_State &t_ss) const override {
        return const_var(
            this->get_bool_condition(this->children[0]->eval(t_ss), t_ss) &&
...
struct Logical_Or_AST_Node final : AST_Node_Impl<T> {
    Logical_Or_AST_Node(std::string t_ast_node_text, Parse_Location t_loc,
                        std::vector<AST_Node_Impl_Ptr<T>> t_children)
        : AST_Node_Impl<T>(std::move(t_ast_node_text),
                           AST_Node_Type::Logical_Or, std::move(t_loc),
                           std::move(t_children)) {
        assert(this->children.size() == 2);
    }
