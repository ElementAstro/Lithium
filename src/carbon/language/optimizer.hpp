...
#ifndef CARBON_OPTIMIZER_HPP
#define CARBON_OPTIMIZER_HPP
...
template <typename T>
eval::AST_Node_Impl<T> &child_at(eval::AST_Node_Impl<T> &node,
                                 const size_t offset) noexcept {
    if (node.children[offset]->identifier == AST_Node_Type::Compiled) {
        return *(
            dynamic_cast<eval::Compiled_AST_Node<T> &>(*node.children[offset])
                .m_original_node);
    } else {
...
template <typename T>
const eval::AST_Node_Impl<T> &child_at(const eval::AST_Node_Impl<T> &node,
                                       const size_t offset) noexcept {
    if (node.children[offset]->identifier == AST_Node_Type::Compiled) {
        return *(dynamic_cast<const eval::Compiled_AST_Node<T> &>(
  ...
  eval::Compiled_AST_Node<T>&>(*node).m_original_node->children[offset]; } else
  { return node->children[offset];
  }
  */
}
...
auto make_compiled_node(eval::AST_Node_Impl_Ptr<T> original_node,
                        ...
                        Callable callable) {
    return Carbon::make_unique<eval::AST_Node_Impl<T>,
                                   eval::Compiled_AST_Node<T>>(
...
struct Return {
    ...
    auto optimize(eval::AST_Node_Impl_Ptr<T> p) {
        if ((p->identifier == AST_Node_Type::Def ||
             p->identifier == AST_Node_Type::Lambda) &&
            !p->children.empty()) {
                ...
                if (block_last_child->identifier == AST_Node_Type::Return) {
                    if (block_last_child->children.size() == 1) {
                        last_child->children.back() =
                            std::move(block_last_child->children[0]);
                    }
                }
            }
        }
        ...
        if ((node->identifier == AST_Node_Type::Block ||
            ...
            for (size_t i = 0; i < node->children.size() - 1; ++i) {
                ...
                if (child->identifier == AST_Node_Type::Fun_Call) {
                    node->children[i] = Carbon::make_unique<
                        eval::AST_Node_Impl<T>,
            ...
            if (child.identifier == AST_Node_Type::Block ||
                ...
                for (size_t i = 0; i < num_sub_children; ++i) {
                    ...
                    if (sub_child.identifier == AST_Node_Type::Fun_Call) {
                        child.children[i] = Carbon::make_unique<
                            eval::AST_Node_Impl<T>,
                            eval::Unused_Return_Fun_Call_AST_Node<T>>(
                            sub_child.text, sub_child.location,
...
struct Assign_Decl {
    ...
    auto optimize(eval::AST_Node_Impl_Ptr<T> node) {
        if ((node->identifier == AST_Node_Type::Equation) &&
            node->text == "=" && node->children.size() == 2 &&
...
struct If {
    ...
    auto optimize(eval::AST_Node_Impl_Ptr<T> node) {
        if ((node->identifier == AST_Node_Type::If) &&
            ...
            const auto condition = dynamic_cast<eval::Constant_AST_Node<T> *>(
                                       node->children[0].get())
                                       ->m_value;
        ...
        if (node->identifier == AST_Node_Type::Binary &&
            ...
            try {
                ...
                if (parsed != Operators::Opers::invalid) {
                    const auto rhs = dynamic_cast<eval::Constant_AST_Node<T> *>(
                                         node->children[1].get())
                                         ->m_value;
    ...
    auto optimize(eval::AST_Node_Impl_Ptr<T> node) {
        if (node->identifier == AST_Node_Type::Prefix &&
            ...
            try {
                ...
                const auto lhs =
                    dynamic_cast<const eval::Constant_AST_Node<T> *>(
                        node->children[0].get())
                        ->m_value;
                const auto match = oper + node->children[0]->text;

                if (parsed != Operators::Opers::invalid &&
                    parsed != Operators::Opers::bitwise_and &&
                ...
                } else if (lhs.get_type_info().bare_equal_type_info(
                               typeid(bool)) &&
                           oper == "!") {
                    return Carbon::make_unique<eval::AST_Node_Impl<T>,
                                                   eval::Constant_AST_Node<T>>(
                        std::move(match), node->location,
            ...
            try {
                ...
                if (lhs.get_type_info().bare_equal_type_info(typeid(bool)) &&
                    ...
                    const auto match = node->children[0]->text + " " +
                                       node->text + " " +
            ...
            try {
                ...
                if (parsed != Operators::Opers::invalid) {
                    ...
                    const auto rhs =
                        dynamic_cast<const eval::Constant_AST_Node<T> &>(
                            *node->children[1])
                            .m_value;
                    if (lhs.get_type_info().is_arithmetic() &&
                        rhs.get_type_info().is_arithmetic()) {
                        const auto val =
                            Boxed_Number::do_oper(parsed, lhs, rhs);
                        const auto match = node->children[0]->text + " " +
                                           oper + " " + node->children[1]->text;
                        return Carbon::make_unique<
                            eval::AST_Node_Impl<T>, eval::Constant_AST_Node<T>>(
                            std::move(match), node->location, std::move(val));
                    }
                }
            } catch (const std::exception &) {
                // failure to fold, that's OK
                   ...
                   node->children[1]->children[0]->identifier ==
                       AST_Node_Type::Constant) {
            const auto arg = dynamic_cast<const eval::Constant_AST_Node<T> &>(
...
struct For_Loop {
    ...
    auto optimize(eval::AST_Node_Impl_Ptr<T> for_node) {
        ...
        if (child_count(*for_node) == 4 &&
            ...
            binary_node.text == "<" && child_count(binary_node) == 2 &&
            ...
            prefix_node.text == "++" && child_count(prefix_node) == 1 &&
            ...
            if (begin.get_type_info().bare_equal(user_type<int>()) &&
                ...
                // note that we are moving the last element out, then popping
                // the empty shared_ptr from the vector
                        ...
                        try {
                            for (; i < end_int; ++i) {
                                try {
                                    ...
                                    // we got a continue exception, which means
                                    // all of the remaining loop implementation
                                    // is skipped and we just need to continue
...
using Optimizer_Default =
    Optimizer<optimizer::Partial_Fold, optimizer::Unused_Return,
              optimizer::Constant_Fold, optimizer::If, optimizer::Return,
              optimizer::Dead_Code, optimizer::Block, optimizer::For_Loop,
