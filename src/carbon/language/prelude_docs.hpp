/// This file is not technically part of the ChaiScript API. It is used solely
/// for generating Doxygen docs regarding the ChaiScript standard runtime
/// library.
...
/// habits. Few places show this off as clearly as the prelude, itself a name
/// taken as a nod to the popular functional language Haskell. This prelude is
/// available to all standard ChaiScript applications, and provides a simple
/// foundation for using numbers, strings, and ranges (the general category of
/// Range cs and their iteration).
///

/// \brief Generic concept of a value in ChaiScript.
///
/// The Object type exists merely as a concept. All objects in ChaiScript
/// support this concept and have the following methods available to them. All
/// objects are stored internally as Carbon::Boxed_Value types.
///
/// \sa Carbon::Boxed_Value
class Object {
public:
    ...
    /// \brief Returns true if the Object does not contain a value is is
    /// undefined.
...
///
/// \sa Map_Pair
/// \sa Carbon::bootstrap::standard_library::map_type
class Map {
public:
    /// \brief Returns an object that implements the Range concept for the
    /// Map_Pair's in this Map
    Range range();

    /// \brief Returns an object that implements the Const_Range concept for the
    /// Map_Pair's in this Map
    ...
    /// \brief Returns the item at the given key, creating an undefined Object
    /// if the key does not yet exist in the map
    Object operator[](string);

    /// \brief Clears the map of all items
    void clear();

    /// \brief Returns the number of items in the Map with the given key.
    /// Returns 0 or 1 since this is not an std::multimap.
...
/// \brief A concept implemented by string, Vector and Map. It is convertible to
/// Range, default constructable and back_insertable
class Container {
public:
...
/// \brief Prints o to the terminal, without a trailing carriage return. Applies
/// conversions to string automatically. \code eval> puts("hi, "); puts("there")
/// hi, thereeval>
/// \endcode
/// \sa to_string
/// \sa print
...
/// \brief Prints o to the terminal, with a trailing carriage return. Applies
/// conversions to string automatically \code eval> print("hello") hello eval>
/// \endcode
/// \sa to_string
/// \sa puts
void print(Object o);

/// \brief ChaiScript representation of std::string. It is an std::string but
/// only some member are exposed to ChaiScript.
///
/// Because the ChaiScript string object is an std::string, it is directly
/// convertible to and from std::string using the Carbon::boxed_cast and
/// Carbon::var functions.
///
/// With the exception of string::trim, string::rtrim, string::ltrim, all
/// members are direct pass-throughs to the std::string of the same name.
///
/// \note Object and function notations are equivalent in ChaiScript. This means
/// that
///       \c "bob".find("b") and \c find("bob", "b") are exactly the same. Most
///       examples below follow the second formation of the function calls.
/// \sa \ref keyworddef for extending existing C++ classes in ChaiScript
/// \sa Carbon::bootstrap::standard_library::string_type
class string {
public:
    ...
    /// \brief Removes whitespace from the front of the string, returning a new
    /// string
    ///
    /// \note This function is implemented as a ChaiScript function using the
    /// def member function notation.
    ///
    /// \code
    /// eval> ltrim("  bob")
    /// bob
    ...
    /// \brief Removes whitespace from the back of the string, returning a new
    /// string
    ///
    /// \note This function is implemented as a ChaiScript function using the
    /// def member function notation.
    ...
    /// \brief Removes whitespace from the front and back of the string,
    ...
    /// \brief Returns the character at the given index in the string, const
    /// version
    ...
    ///
    /// This function normally returns size_t in C++. In ChaiScript the return
    /// value is cast to int for ease of use.
    int size() const;
...
/// \brief A concept in ChaiScript that is implemented by \ref string, Vector
/// and Map. It provides
///        easy iteration over the elements in a container.
///
/// Implemented by the template
/// Carbon::bootstrap::standard_library::Bidir_Range
///
/// \sa Const_Range
class Range {
public:
    ...
    /// \brief Returns true if the front and back pointers have passed each
    /// other, if no items
...
/// \brief A concept in ChaiScript that is implemented by \ref string, Vector
/// and Map. It provides
///        easy iteration over the elements in a container. Contained values are
///        const.
///
/// Implemented by the template
/// Carbon::bootstrap::standard_library::Const_Bidir_Range
///
/// \sa Range
class Const_Range {
public:
...
/// \endcode
///
/// Implemented with std::vector<Carbon::Boxed_Value>
///
/// \sa Carbon::bootstrap::standard_library::vector_type
class Vector {
public:
    /// \brief returns the Object at the given index. Throws an exception if the
    /// index does not exist
    Object operator[](int t_index);

    /// \brief returns a const Object at the given index. Throws an exception if
    ...
    /// \brief Inserts a new item in the Vector at the given index. The item is
    /// not cloned on insert
    ///
    /// \sa insert_ref
...
class Type_Info {
public:
    /// \brief Compares this Type_Info object with another one and returns true
    ...
    /// after const, pointer, reference is removed.
    ...
    /// \brief Returns the ChaiScript registered name for the type if one
    /// exists.
    string name() const;
};
...
/// \code
/// var f2 = `-`; // represents the unary - as well as the set of binary -
/// operators \endcode
///
...
public:
    ...
    /// \brief Returns the arity of the function, -1 if the function takes a
    ...
    /// \brief Returns a vector of Type_Info objects that represent the param
    /// types for this function.
    ///        The first value in the list is the return type.
    ///
    /// If this function is a conglomerate of several functions
    /// (get_contained_values().size() > 0) then the function returns as many
    /// Type_Info objects as it can. If the functions contained all have the
    /// same arity, then it represents the arity. If they have different
    /// arities, it returns only one value - the return type.
    ///
    /// For each parameter that is the same type, the type is returned. If the
    /// types are different then a Type_Info for Object is returned.
    ///
    /// Example:
    /// \code
    /// eval> f2.get_param_types().size(); // Returns a Type_Info for Object for
    /// the return type
    /// 1
    /// \endcode
    Vector get_param_types() const;

    /// \brief Returns true if the function has a guard to it. Always returns
    /// false for a conglomerate function
    bool has_guard() const;

    /// \brief Calls the function with the given set of parameters and returns
    /// the value;
    ///
    /// Example:
    /// \code
...
/// \brief Starts with the initial value and applies the function f to it and
/// the first element of the Range c.
///        The result is then applied to the second element, and so on until the
...
/// \brief Takes elements from the Range c that match function f, stopping at
/// the first non-match, returning them as a new Vector.
///
/// Example:
/// \code
...
/// \brief Drops elements from the Range c that match f, stopping at the first
/// non-match, returning the remainder.
///
/// Example:
/// \code
...
/// \brief Similar to foldl, this takes the first two elements as its starting
/// values for f. This assumes Range c has at least 2 elements.
///
/// Example:
/// \code
/// eval> reduce([1, 2, 3, 4], `+`)
...
/// \brief Joins the elements of the Range c into a string, delimiting each with
/// the delim string.
///
/// Example:
/// \code
...
/// \brief Generates a new Vector filled with values starting at x and ending
/// with y.
///
...
/// \brief Applies f to elements of x and y, returning a new Vector with the
...
/// \brief returns true if there exists a call to the Function f that takes the
/// given parameters
///
/// Example:
/// \code
...
/// \brief Reverses a Const_Range object so that the elements are accessed in
/// reverse
Const_Range retro(Const_Range);

/// \brief Raises the given object as an exception. Any type of object can be
/// thrown.
///
/// Example:
/// \code
/// eval> try { throw(1); } catch (e) { print("Exception caught: " +
/// to_string(e)); } Exception caught: 1 \endcode
///
/// \sa \ref keywordtry
void throw(Object);
}  // namespace ChaiScript_Language
