    ...
    struct Internal {
        ...
        decltype(auto) visit_or(Visitor &&visitor, Or &&other) const {
            if (type() == Class(ClassValue)) {
                return visitor(
                    std::get<static_cast<std::size_t>(ClassValue)>(d));
            } else {
                return other();
            }
        }
...
public:
    ...
    template <typename T>
    explicit JSON(T b, typename enable_if<is_same<T, bool>::value>::type * =
                           nullptr) noexcept
    ...
    template <typename T>
    explicit JSON(T f, typename enable_if<is_floating_point<T>::value>::type * =
                           nullptr) noexcept
        : internal(static_cast<double>(f)) {}

    template <typename T>
    explicit JSON(
        T s, typename enable_if<is_convertible<T, std::string>::value>::type * =
                 nullptr)
...
#pragma warning(pop)
        ...
        for (; offset < str.size();) {
            ...
            if (str.at(offset) != ':') {
                throw std::runtime_error(
                    std::string("JSON ERROR: Object: Expected colon, found '") +
                    str.at(offset) + "'\n");
            }
            consume_ws(str, ++offset);
            ...
            if (str.at(offset) == ',') {
                ...
                throw std::runtime_error(
                    std::string("JSON ERROR: Object: Expected comma, found '") +
                    str.at(offset) + "'\n");
            }
        }
                    ...
                    case 'u': {
                        ...
                        for (size_t i = 1; i <= 4; ++i) {
                            ...
                            if ((c >= '0' && c <= '9') ||
                                ...
                                throw std::runtime_error(
                                    std::string("JSON ERROR: String: Expected "
                                                "hex character in unicode "
                                                "escape, found '") +
                                    c + "'");
                            }
                        }
    ...
    static JSON parse_number(const std::string &str, size_t &offset) {
        ...
        if (offset < str.size() && (c == 'E' || c == 'e')) {
            ...
            for (; offset < str.size();) {
                ...
                if (c >= '0' && c <= '9') {
                    ...
                    throw std::runtime_error(
                        std::string("JSON ERROR: Number: Expected a number for "
                                    "exponent, found '") +
                        c + "'");
                } else {
                    break;
            ...
            exp = Carbon::parse_num<std::int64_t>(exp_str) *
                  (isExpNegative ? -1 : 1);
        ...
        if (isDouble) {
            return JSON((isNegative ? -1 : 1) * Carbon::parse_num<double>(val) *
                        std::pow(10, exp));
        } else {
            if (!exp_str.empty()) {
                return JSON(
                    (isNegative ? -1 : 1) *
                    static_cast<double>(Carbon::parse_num<std::int64_t>(val)) *
                    std::pow(10, exp));
            } else {
                return JSON((isNegative ? -1 : 1) *
                            Carbon::parse_num<std::int64_t>(val));
            }
        }
    }
