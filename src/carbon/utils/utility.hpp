...
///
/// \code
/// Carbon::utility::add_class<test>(*m,
///      "test",
///      { constructor<test ()>(),
...
///        {fun(static_cast<std::string(test::*)(double)>(&test::function_overload)),
///        "function_overload" },
///        {fun(static_cast<std::string(test::*)(int)>(&test::function_overload)),
///        "function_overload" }, {fun(static_cast<test & (test::*)(const test
///        &)>(&test::operator=)), "=" }
///        }
///      );
///
...
typename std::enable_if<std::is_enum<Enum>::value, void>::type add_class(
    ...
    using namespace Carbon::bootstrap::operators;
    equal<Enum>(t_module);
    not_equal<Enum>(t_module);
    assign<Enum>(t_module);

    t_module.add(
        Carbon::fun([](const Enum &e, const int &i) { return e == i; }), "==");
    t_module.add(
        Carbon::fun([](const int &i, const Enum &e) { return i == e; }), "==");

    for (const auto &constant : t_constants) {
        t_module.add_global_const(Carbon::const_var(Enum(constant.first)),
                                  constant.second);
    }
}
...
typename std::enable_if<std::is_enum<EnumClass>::value, void>::type add_class(
    ...
    for (const auto &constant : t_constants) {
        t_module.add_global_const(Carbon::const_var(EnumClass(constant.first)),
                                  constant.second);
    }
}
}  // namespace Carbon::utility
