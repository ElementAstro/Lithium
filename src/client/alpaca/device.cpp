...
Device::Device(const std::string &address, const std::string &device_type, int device_number, const std::string &protocol)
    : address(address), device_type(device_type), device_number(device_number), protocol(protocol)
{
    api_version = API_VERSION;
    base_url = protocol + "://" + address + "/api/v" + std::to_string(api_version) + "/" + device_type + "/" + std::to_string(device_number);
}

std::string Device::Action(const std::string &ActionName, const std::vector<std::string> &Parameters)
{
    return _put("action", {{"Action", ActionName}, {"Parameters", Parameters}}).at("Value");
}
...
{
    return _put("commandbool", {{"Command", Command}, {"Raw", Raw}}).at("Value");
}

std::string Device::CommandString(const std::string &Command, bool Raw)
{
    return _put("commandstring", {{"Command", Command}, {"Raw", Raw}}).at("Value");
}

bool Device::get_Connected() const
{
    return _get("connected") == "true";
}
...
std::string Device::get_Description() const
{
...
std::vector<std::string> Device::get_DriverInfo() const
{
    ...
    {
        driver_info.push_back(driver_info_str.substr(start_pos, end_pos - start_pos));
        start_pos = end_pos + 1;
...
std::string Device::get_DriverVersion() const
{
...
std::vector<std::string> Device::get_SupportedActions() const
{
    ...
    while (end_pos != std::string::npos)
    {
        supported_actions.push_back(supported_actions_str.substr(start_pos, end_pos - start_pos));
        start_pos = end_pos + 1;
...
json Device::_get(const std::string &attribute, const std::map<std::string, std::string> &data = {}, double tmo = 5.0) const
{
    ...
    httplib::Headers headers = {{"ClientTransactionID", client_trans_id}, {"ClientID", client_id}};
    ...
    {
        throw AlpacaRequestException(response ? response->status : -1, response ? response->body : "Request failed");
    }
}

json Device::_put(const std::string &attribute, const std::map<std::string, std::any> &data = {}, double tmo = 5.0) const
{
    httplib::Client client(address.c_str());
    std::string path = base_url + "/" + attribute;
    ...
    httplib::Result response = client.Put(path.c_str(), headers, body, "application/json");
