...
public:
    ...
    static std::string loadResource(const std::string &path, const std::unordered_set<std::string> &allowedExtensions, bool checkAllowed = true)
    {
        ...
        if (checkAllowed && allowedExtensions.find(fileExtension) == allowedExtensions.end())
        {
        ...
        {
            throw std::runtime_error("Failed to read file: " + fullPath + ". Error: " + e.what());
        }
    }
...
#include OATPP_CODEGEN_BEGIN(ApiController) //<- Begin Codegen
    ...
    ENDPOINT_INFO(IndexRequestHandler)
    {
    ...
    ENDPOINT_ASYNC("GET", "/", IndexRequestHandler)
    {
        ...
        Action act() override
        {
            auto response = controller->createResponse(Status::CODE_200, loadResource("index.html", {"html"}));
            response->putHeader(Header::CONTENT_TYPE, "text/html");
            return _return(response);
        }
    };

    ENDPOINT_INFO(ClientRequestHandler)
    {
        info->summary = "'Client' endpoint";
    }
    ENDPOINT_ASYNC("GET", "/client", ClientRequestHandler)
    {
        ...
        Action act() override
        {
            auto response = controller->createResponse(Status::CODE_200, loadResource("client/index.html", {"html"}));
            response->putHeader(Header::CONTENT_TYPE, "text/html");
            return _return(response);
        }
    };

    ENDPOINT_INFO(NoVNCRequestHandler)
    {
        info->summary = "'NoVNC' endpoint";
    }
    ...
    ENDPOINT_INFO(WebSSHRequestHandler)
    {
        info->summary = "'WebSSH' endpoint";
    }
    ENDPOINT_ASYNC("GET", "/webssh", WebSSHRequestHandler)
    {
        ...
        Action act() override
        {
            auto response = controller->createResponse(Status::CODE_200, loadResource("module/webssh/index.html", {"html"}));
            response->putHeader(Header::CONTENT_TYPE, "text/html");
            return _return(response);
        }
    };
    ...
    {
        info->summary = "'Static File' endpoint";
    }
    ...
    ENDPOINT_INFO(AllStaticCSS)
    {
        info->summary = "All 'CSS File' endpoint";
    }
    ENDPOINT_ASYNC("GET", "/css/*", AllStaticCSS) {
        ...
        Action act() override
        {
            ...
            auto buffer = loadResource(path.getValue(""), {"json","js","css","html","jpg","png","robot","woff2","tff","ico","svg","mp3","oga","woff","ttf"});
            OATPP_ASSERT_HTTP(buffer != "", Status::CODE_500, "Can't read file");
            return _return(controller->createResponse(Status::CODE_200, buffer));
        }
    };
    ...
    ENDPOINT_INFO(AllStaticJSON)
    {
        info->summary = "All 'JSON File' endpoint";
    }
    ...
    ENDPOINT_INFO(AllStaticFont)
    {
        info->summary = "All 'Font File' endpoint";
    }
    ENDPOINT_ASYNC("GET", "/font/*", AllStaticFont) {
        ...
        Action act() override
        {
            ...
            auto buffer = loadResource(path.getValue(""), {"tff", "tff", "woff", "woff2", "eot"});
            OATPP_ASSERT_HTTP(buffer != "", Status::CODE_500, "Can't read file");
            return _return(controller->createResponse(Status::CODE_200, buffer));
        }
    };
    ...
    {
        info->summary = "All 'Node Modules File' endpoint. This need to be changed to normal format";
    }
    ...
    ENDPOINT_ASYNC("GET", "/webfonts/*", AllStaticWebFonts) {
        ...
        Action act() override
        {
            ...
            auto buffer = loadResource(path.getValue(""), {"eot", "svg", "ttf" ,"woff", "woff2"});
            OATPP_ASSERT_HTTP(buffer != "", Status::CODE_500, "Can't read file");
            return _return(controller->createResponse(Status::CODE_200, buffer));
        }
    };
