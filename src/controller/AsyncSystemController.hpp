...
public:
    ...
    ENDPOINT_ASYNC("GET", "/api/system/cpu", getUICpuUsage){
        ENDPOINT_ASYNC_INIT(getUICpuUsage) Action act()
            override{auto res = BaseReturnSystemDto::createShared();
...
ENDPOINT_INFO(getUIMemoryUsage) {
    ...
    // info->addResponse<StatusDto>(Status::CODE_200, "application/json", "Usage
    // of RAM");
}
ENDPOINT_ASYNC("GET", "/api/system/memory", getUIMemoryUsage){
    ENDPOINT_ASYNC_INIT(getUIMemoryUsage) Action act()
        override{auto res = BaseReturnSystemDto::createShared();
...
ENDPOINT_ASYNC("GET", "/api/system/cpu_temp", getUICpuTemperature){
    ENDPOINT_ASYNC_INIT(getUICpuTemperature) Action act()
        override{auto res = BaseReturnSystemDto::createShared();
...
ENDPOINT_ASYNC("GET", "/api/system/disk", getUIDiskUsage){
    ENDPOINT_ASYNC_INIT(getUIDiskUsage) Action act()
        override{auto res = ReturnDiskUsageDto::createShared();
...
ENDPOINT_INFO(getUIProcesses) {
    ...
    // "Processes"); info->addResponse<String>(Status::CODE_404, "text/plain");
}
ENDPOINT_ASYNC("GET", "/api/system/process",
               getUIProcesses){ENDPOINT_ASYNC_INIT(getUIProcesses) Action act()
                                   override{nlohmann::json res;
...
ENDPOINT_ASYNC("GET", "/api/system/shutdown",
               getUIShutdown){ENDPOINT_ASYNC_INIT(getUIShutdown) Action act()
                                  override{Atom::System::Shutdown();
return _return(
    controller->createResponse(Status::CODE_200, "Wtf, how can you do that?"));
}
}
;

ENDPOINT_INFO(getUIReboot) { info->summary = "Reboot system"; }
ENDPOINT_ASYNC("GET", "/api/system/reboot",
               getUIReboot){ENDPOINT_ASYNC_INIT(getUIReboot) Action act()
                                override{Atom::System::Reboot();
return _return(
    controller->createResponse(Status::CODE_200, "Wtf, how can you do that?"));
}
}
