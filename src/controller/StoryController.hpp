...
public:
    ...
    ENDPOINT_INFO(createStory) {
        ...
        info->addResponse<Object<StoryDto>>(Status::CODE_200,
                                            "application/json");
        info->addResponse<Object<StatusDto>>(Status::CODE_500,
                                             "application/json");
    }
    ENDPOINT("POST", "stories", createStory, BUNDLE(String, userId),
             BODY_DTO(Object<StoryDto>, storyDto)) {
        storyDto->id = nullptr;
        return createDtoResponse(Status::CODE_200,
                                 m_storyService.createStory(userId, storyDto));
    }
    ...
    ENDPOINT("PUT", "stories/{storyId}", putStory, BUNDLE(String, userId),
             PATH(String, storyId), BODY_DTO(Object<StoryDto>, storyDto)) {
        storyDto->id = storyId;
        return createDtoResponse(Status::CODE_200,
                                 m_storyService.updateStory(userId, storyDto));
    }
    ...
    ENDPOINT("GET", "stories/{storyId}", getStoryById, BUNDLE(String, userId),
             PATH(String, storyId)) {
        return createDtoResponse(
            Status::CODE_200,
            m_storyService.getStoryByUserIdAndId(userId, storyId));
    }

    ENDPOINT_INFO(getStories) {
        info->summary = "Get All user stories";

        info->addResponse<oatpp::Object<StoryPageDto>>(Status::CODE_200,
                                                       "application/json");
        info->addResponse<Object<StatusDto>>(Status::CODE_500,
                                             "application/json");
    }
    ENDPOINT("GET", "stories/offset/{offset}/limit/{limit}", getStories,
             BUNDLE(String, userId), PATH(UInt32, offset),
             PATH(UInt32, limit)) {
        return createDtoResponse(
            Status::CODE_200,
            m_storyService.getAllUserStories(userId, offset, limit));
    }
    ...
    ENDPOINT("DELETE", "stories/{userId}", deleteStory, BUNDLE(String, userId),
             PATH(String, storyId)) {
        return createDtoResponse(
            Status::CODE_200,
            m_storyService.deleteStoryByUserIdAndId(userId, storyId));
    }
};
