...
bool InMemoryDatabase::LoadDatabase(const char *DeviceName)
{
    ...
    snprintf(DatabaseFileName, MAXRBUF, "%s/.hydrogen/%s_alignment_database.xml", getenv("HOME"), DeviceName);
    ...
    if (strcmp(tagXMLEle(FileRoot), "HYDROGENAlignmentDatabase") != 0)
    {
        return false;
    }
...
bool InMemoryDatabase::SaveDatabase(const char *DeviceName)
{
    ...
    snprintf(ConfigDir, MAXRBUF, "%s/.hydrogen/", getenv("HOME"));
    ...
    fprintf(fp, "<HYDROGENAlignmentDatabase>\n");
