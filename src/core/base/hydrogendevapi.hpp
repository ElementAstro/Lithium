...
/** @file hydrogendevapi.h
 *  @brief Interface to the reference HYDROGEN C API device implementation on the Device Driver side.
 *
 *  @author Elwood C. Downey
 *  @author Jasem Mutlaq
 *
 *  This file is divided into two main sections:\n
 *  <ol><li> Functions the HYDROGEN device driver framework defines which the Driver may
 *  call:</li>
 *
 *  <ul><li>IDxxx functions to send messages to an HYDROGEN client. Please note that it is recommended to use the
 *   HYDROGEN Library wrapper functions instead of calling IDxxx directly.
 *   <ul>
 *    <li>IDMessage: Use @ref HYDROGEN::Logger "HYDROGEN Logging Framework" functions (e.g. LOG_DEBUG..etc) instead.</li>
 *    <li>IDDefXXX: use @ref HYDROGEN::DefaultDevice "HYDROGEN Default Device" defXXX functions intead.</li>
 *   </ul>
 *   </li>
 *  <li>IExxx functions to implement the event driven model.</li>
 *  <li>IUxxx functions to perform handy utility functions.</li></ul>
 *
 *  <li>Functions the HYDROGEN device driver framework calls which the Driver must
 *  define:</li>
 *
 ...
 *
 *  <p>These functions make it much easier to write a compliant HYDROGEN driver than
 *  starting from scratch, and also serve as a concrete
 *  example of the interactions an HYDROGEN driver, in any language, is expected to
 *  accommodate.</p>
 *
 ...
 *
 *  <p>A major point to realize is that an HYDROGEN driver built with this framework
 *  does not contain the C main() function. As soon as a driver begins executing,
 *  it listens on stdin for HYDROGEN messages. Only when a valid and appropriate
 ...
 *******************************************************************************
 *
 *  Functions the HYDROGEN device driver framework defines which the Driver may call
 *
 *******************************************************************************
 *******************************************************************************
 */
 ...
 * HYDROGEN message from its arguments and writes it to stdout. From there, is it
 ...
 * attribute within the HYDROGEN message. If the format argument is NULL, no message
...
#define ATTRIBUTE_FORMAT_PRINTF(A, B)
    ...
    /**
     * @defgroup deventFunctions IE Functions: Functions drivers call to register with the HYDROGEN event utilities.
     ...
     *******************************************************************************
     *
     *   Functions the HYDROGEN Device Driver framework calls which the Driver must
     *   define.
     *
     *******************************************************************************
     *******************************************************************************
     ...
     *  getProperties message from an HYDROGEN client. The argument @param dev is either a
     ...
     *  @note You do not need to call this function, it is called by HYDROGEN when new text values arrive from the client.
     */
     ...
     *  @note You do not need to call this function, it is called by HYDROGEN when new number values arrive from the client.
     */
     ...
     *  @note You do not need to call this function, it is called by HYDROGEN when new switch values arrive from the client.
     */
     ...
     *  @note You do not need to call this function, it is called by HYDROGEN when new blob values arrive from the client.
    ...
    /** @brief Function defined by Drivers that is called when another Driver it is snooping (by having previously called IDSnoopDevice()) sent any HYDROGEN message.
     ...
     *         The buffer size must be at least MAXHYDROGENDEVICE bytes.
     *  @param name pointer to an allocated buffer to save the extracted elemented name attribute.
     *         The buffer size must be at least MAXHYDROGENNAME bytes.
