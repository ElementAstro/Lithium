...
namespace Connection
{
    ...
    ///
    //////////////////////////////////////////////////////////////////////////////////////////////////
    TCP::TCP(HYDROGEN::DefaultDevice *dev) : Interface(dev, CONNECTION_TCP)
    {
        char defaultHostname[MAXHYDROGENNAME] = {0};
        char defaultPort[MAXHYDROGENNAME] = {0};

        // Try to load the port from the config file. If that fails, use default port.
        if (IUGetConfigText(dev->getDeviceName(), HYDROGEN::SP::DEVICE_ADDRESS, "ADDRESS", defaultHostname, MAXHYDROGENNAME) == 0)
            m_ConfigHost = defaultHostname;
        if (IUGetConfigText(dev->getDeviceName(), HYDROGEN::SP::DEVICE_ADDRESS, "PORT", defaultPort, MAXHYDROGENNAME) == 0)
        ...
        // Try to load the port from the config file. If that fails, use default port.
        IUGetConfigOnSwitchIndex(dev->getDeviceName(), HYDROGEN::SP::DEVICE_AUTO_SEARCH, &autoSearchIndex);
        IUFillSwitch(&LANSearchS[HYDROGEN::DefaultDevice::HYDROGEN_ENABLED], "HYDROGEN_ENABLED", "Enabled",
                     autoSearchIndex == 0 ? ISS_ON : ISS_OFF);
        IUFillSwitch(&LANSearchS[HYDROGEN::DefaultDevice::HYDROGEN_DISABLED], "HYDROGEN_DISABLED", "Disabled",
                     autoSearchIndex == 0 ? ISS_OFF : ISS_ON);
        IUFillSwitchVector(&LANSearchSP, LANSearchS, 2, dev->getDeviceName(), HYDROGEN::SP::DEVICE_LAN_SEARCH, "LAN Search",
                           CONNECTION_TAB, IP_RW, ISR_1OFMANY, 60, IPS_IDLE);
    }
    ...
    bool TCP::establishConnection(const std::string &hostname, const std::string &port, int timeout)
    {
        ...
        if (LANSearchS[HYDROGEN::DefaultDevice::HYDROGEN_ENABLED].s == ISS_OFF)
    ...
    bool TCP::Connect()
    {
        ...
        else
        {
            ...
            if (establishConnection(hostname, port))
            {
                ...
                if (handshakeResult == false && LANSearchS[HYDROGEN::DefaultDevice::HYDROGEN_ENABLED].s == ISS_OFF)
                {
            ...
            if (handshakeResult == false &&
                LANSearchS[HYDROGEN::DefaultDevice::HYDROGEN_ENABLED].s == ISS_ON &&
                isIPv4)
            {
        ...
        if (handshakeResult)
        {
            ...
            if (LANSearchS[HYDROGEN::DefaultDevice::HYDROGEN_ENABLED].s == ISS_ON)
            {
                LANSearchS[HYDROGEN::DefaultDevice::HYDROGEN_ENABLED].s = ISS_OFF;
                LANSearchS[HYDROGEN::DefaultDevice::HYDROGEN_DISABLED].s = ISS_ON;
    ...
    {
        LANSearchS[HYDROGEN::DefaultDevice::HYDROGEN_ENABLED].s = enabled ? ISS_ON : ISS_OFF;
        LANSearchS[HYDROGEN::DefaultDevice::HYDROGEN_DISABLED].s = enabled ? ISS_OFF : ISS_ON;
        if (m_Device->isInitializationComplete())
            IDSetSwitch(&LANSearchSP, nullptr);
    }
}
