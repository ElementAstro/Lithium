...
std::list<HYDROGEN::DefaultDevicePrivate *> HYDROGEN::DefaultDevicePrivate::devices;
std::recursive_mutex HYDROGEN::DefaultDevicePrivate::devicesLock;
...
extern "C"
{
    ...
    {
        const std::unique_lock<std::recursive_mutex> lock(HYDROGEN::DefaultDevicePrivate::devicesLock);
        for (auto &it : HYDROGEN::DefaultDevicePrivate::devices)
...
{
    HYDROGEN::DefaultDevice *devPtr = static_cast<HYDROGEN::DefaultDevice *>(t);
    if (devPtr != nullptr)
    {
...
namespace HYDROGEN
{
    ...
    bool DefaultDevice::loadConfig(HYDROGEN::Property &property)
    {
    ...
    bool DefaultDevice::saveConfigItems(FILE *fp)
    {
        ...
        return HYDROGEN::Logger::saveConfigItems(fp);
    }
    ...
    bool DefaultDevice::saveAllConfigItems(FILE *fp)
    {
        ...
        {
            if (oneProperty.getType() == HYDROGEN_SWITCH)
            {
                ...
                if (
                    (svp->isNameMatch(HYDROGEN::SP::CONNECTION)) ||
    ...
    bool DefaultDevice::saveConfig(HYDROGEN::Property &property)
    {
    ...
    bool DefaultDevice::loadDefaultConfig()
    {
        ...
        if (getenv("HYDROGENCONFIG"))
            snprintf(configDefaultFileName, MAXRBUF, "%s.default", getenv("HYDROGENCONFIG"));
        else
    ...
    bool DefaultDevice::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
    {
        ...
        HYDROGEN::PropertySwitch property = getProperty(name, HYDROGEN_SWITCH);
    ...
    bool DefaultDevice::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
    {
        ...
        HYDROGEN::PropertyNumber property = getProperty(name, HYDROGEN_NUMBER);
    ...
    bool DefaultDevice::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
    {
        ...
        HYDROGEN::PropertyText property = getProperty(name, HYDROGEN_TEXT);
                                  ...
                                  char *formats[], char *names[], int n)
    {
        ...
        HYDROGEN::PropertyBlob property = getProperty(name, HYDROGEN_BLOB);
    ...
    bool DefaultDevice::ISSnoopDevice(XMLEle *root)
    {
        D_PTR(DefaultDevice);
        char errmsg[MAXRBUF];
        return d->watchDevice.processXml(HYDROGEN::LilXmlElement(root), errmsg) < 0;
    }
    ...
    void DefaultDevice::ISGetProperties(const char *dev)
    {
        ...
        if (d->isInit == false)
        {
            ...
            {
                char *envDev = getenv("HYDROGENDEV");
                if (envDev != nullptr)
    ...
    void DefaultDevice::setConnected(bool status, IPState state, const char *msg)
    {
        ...
        svp[HYDROGEN_ENABLED].setState(status ? ISS_ON : ISS_OFF);
        svp[HYDROGEN_DISABLED].setState(status ? ISS_OFF : ISS_ON);
        svp.setState(state);
    ...
    bool DefaultDevice::initProperties()
    {
        ...
        // Connection
        d->ConnectionSP[HYDROGEN_ENABLED].fill("CONNECT", "Connect", ISS_OFF);
        d->ConnectionSP[HYDROGEN_DISABLED].fill("DISCONNECT", "Disconnect", ISS_ON);
        d->ConnectionSP.fill(getDeviceName(), HYDROGEN::SP::CONNECTION, "Connection", "Main Control", IP_RW, ISR_1OFMANY, 60, IPS_IDLE);
        d->ConnectionSP.onNewValues([this](const HYDROGEN::PropertySwitch::NewValues &values)
                                    {
        ...
        // Debug
        d->DebugSP[HYDROGEN_ENABLED].fill("ENABLE", "Enable", ISS_OFF);
        d->DebugSP[HYDROGEN_DISABLED].fill("DISABLE", "Disable", ISS_ON);
        ...
        // Simulation
        d->SimulationSP[HYDROGEN_ENABLED].fill("ENABLE", "Enable", ISS_OFF);
        d->SimulationSP[HYDROGEN_DISABLED].fill("DISABLE", "Disable", ISS_ON);
        ...
        HYDROGEN::Logger::initProperties(this);
    ...
    bool DefaultDevice::deleteProperty(HYDROGEN::Property &property)
    {
    ...
    bool DefaultDevice::deleteProperty(const char *propertyName)
    {
        ...
        {
            HYDROGEN::Property prop = getProperty(propertyName);
    ...
    {
        registerProperty(HYDROGEN::Property(property));
    ...
    void DefaultDevice::defineProperty(HYDROGEN::Property &property)
    {
        ...
        if (!d->ConnectionModeSP.isEmpty())
        {
            ...
            if (it != d->connections.end())
            {
                ...
                if (index >= 0)
                {
                    ...
                    HYDROGEN::Property connectionProperty = getProperty(d->ConnectionModeSP.getName(), HYDROGEN_SWITCH);
