...
namespace DSP
{
    ...
    Interface::Interface(HYDROGEN::DefaultDevice *dev, Type type, const char *name, const char *label) : m_Device(dev),
                                                                                                        m_Name(name), m_Label(label), m_Type(type)
    {
        char activatestrname[MAXHYDROGENNAME];
        char activatestrlabel[MAXHYDROGENLABEL];
    ...
    {
        HYDROGEN_UNUSED(buf);
        HYDROGEN_UNUSED(ndims);
        HYDROGEN_UNUSED(dims);
        HYDROGEN_UNUSED(bits_per_sample);
        DEBUG(HYDROGEN::Logger::DBG_WARNING, "Interface::Callback -  Should never get here");
        return nullptr;
    }
    ...
    bool Interface::processBLOB(uint8_t *buffer, uint32_t ndims, int *dims, int bits_per_sample)
    {
        ...
        if (PluginActive)
        {
            bool sendCapture = (m_Device->getSwitch("UPLOAD_MODE")[0].getState() == ISS_ON || m_Device->getSwitch("UPLOAD_MODE")[2].getState() == ISS_ON);
            bool saveCapture = (m_Device->getSwitch("UPLOAD_MODE")[1].getState() == ISS_ON || m_Device->getSwitch("UPLOAD_MODE")[2].getState() == ISS_ON);
    ...
    void Interface::addFITSKeywords(fitsfile *fptr)
    {
        ...
        char fitsString[MAXHYDROGENDEVICE];

        // Telescope
        strncpy(fitsString, m_Device->getText("ACTIVE_DEVICES")[0].getText(), MAXHYDROGENDEVICE);
        fits_update_key_s(fptr, TSTRING, "TELESCOP", fitsString, "Telescope name", &status);

        // Observer
        strncpy(fitsString, m_Device->getText("FITS_HEADER")[0].getText(), MAXHYDROGENDEVICE);
        fits_update_key_s(fptr, TSTRING, "OBSERVER", fitsString, "Observer name", &status);

        // Object
        strncpy(fitsString, m_Device->getText("FITS_HEADER")[1].getText(), MAXHYDROGENDEVICE);
        ...
        if (!nv)
        {
            ...
            char lat_str[MAXHYDROGENFORMAT];
            char lon_str[MAXHYDROGENFORMAT];
            char el_str[MAXHYDROGENFORMAT];
            fs_sexa(lat_str, Lat, 2, 360000);
            fs_sexa(lon_str, Lon, 2, 360000);
            snprintf(el_str, MAXHYDROGENFORMAT, "%lf", El);
        ...
        fits_write_comment(fptr, "Generated by HYDROGEN", &status);
    ...
    dsp_stream_p Interface::loadFITS(char *buffer, int len)
    {
        ...
        char filename[MAXHYDROGENNAME];
        sprintf(filename, "HYDROGEN_DSP_INTERFACE_XXXXXX");
    ...
    bool Interface::sendFITS(uint8_t *buf, bool sendCapture, bool saveCapture)
    {
        ...
        default:
            DEBUGF(HYDROGEN::Logger::DBG_ERROR, "Unsupported bits per sample value %d", getBPS());
            return false;
        }
        ...
        char error_status[MAXHYDROGENNAME];
    ...
    bool Interface::uploadFile(const void *fitsData, size_t totalBytes, bool sendCapture, bool saveCapture, const char *format)
    {
        ...
        DEBUGF(HYDROGEN::Logger::DBG_DEBUG, "Uploading file. Ext: %s, Size: %d, sendCapture? %s, saveCapture? %s",
        ...
        snprintf(FitsB.format, MAXHYDROGENBLOBFMT, ".%s", captureExtention);
        ...
        if (saveCapture)
        {
            ...
            if (maxIndex < 0)
            {
                DEBUGF(HYDROGEN::Logger::DBG_ERROR, "Error iterating directory %s. %s", m_Device->getText("UPLOAD_SETTINGS")[0].getText(),
                       strerror(errno));
                return false;
            }
            ...
            char processedFileName[MAXHYDROGENNAME];

            snprintf(processedFileName, MAXHYDROGENNAME, "%s/%s_%s.%s", m_Device->getText("UPLOAD_SETTINGS")[0].getText(), prefix.c_str(),
                     m_Name, format);

            fp = fopen(processedFileName, "w");
            if (fp == nullptr)
            {
                DEBUGF(HYDROGEN::Logger::DBG_ERROR, "Unable to save image file (%s). %s", processedFileName, strerror(errno));
                return false;
            }
        ...
        DEBUG(HYDROGEN::Logger::DBG_DEBUG, "Upload complete");
    ...
    bool Interface::setMagnitude(void *buf, uint32_t dims, int *sizes, int bits_per_sample)
    {
        if (stream == nullptr)
            return false;
        if (dims != (uint32_t)stream->dims)
            return false;
        for (uint32_t d = 0; d < dims; d++)
            if (sizes[d] != stream->sizes[d])
                return false;
    ...
    {
        snprintf(captureExtention, MAXHYDROGENBLOBFMT, "%s", ext);
    }
}
