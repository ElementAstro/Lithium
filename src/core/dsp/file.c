...
dsp_stream_p *dsp_file_read_fits(const char *filename, int *channels, int stretch)
{
    ...
    {
    case BYTE_IMG:
        fits_read_img(fptr, 11, 1, (long)nelements, NULL, array, &anynul, &status);
...
dsp_stream_p *dsp_buffer_rgb_to_components(void *buf, int dims, int *sizes, int components, int bpp, int stretch)
{
    ...
    for (x = 0; x < channel->len; x++)
    {
        ...
        case 8:
            for (z = 0; z < y; z++)
                v += ((unsigned char *)buf)[x * y + z];
            break;
        case 16:
            for (z = 0; z < y; z++)
                v += ((unsigned short *)buf)[x * y + z];
            break;
        case 32:
            for (z = 0; z < y; z++)
                v += ((unsigned int *)buf)[x * y + z];
            break;
        case 64:
            for (z = 0; z < y; z++)
                v += ((unsigned long *)buf)[x * y + z];
            break;
        case -32:
            for (z = 0; z < y; z++)
                v += ((float *)buf)[x * y + z];
