...
uint16_t HYDROGENWSServer::m_global_port = 11623;
#endif
...
namespace HYDROGEN
{
    ...
    bool CCD::initProperties()
    {
        ...
        IUFillSwitch(&PrimaryCCD.CompressS[HYDROGEN_ENABLED], "HYDROGEN_ENABLED", "Enabled", ISS_OFF);
        IUFillSwitch(&PrimaryCCD.CompressS[HYDROGEN_DISABLED], "HYDROGEN_DISABLED", "Disabled", ISS_ON);
    ...
    IUFillNumberVector(&PrimaryCCD.RapidGuideDataNP, PrimaryCCD.RapidGuideDataN, 3, getDeviceName(),
        ...
        IUFillSwitch(&GuideCCD.CompressS[HYDROGEN_ENABLED], "HYDROGEN_ENABLED", "Enabled", ISS_OFF);
        IUFillSwitch(&GuideCCD.CompressS[HYDROGEN_DISABLED], "HYDROGEN_DISABLED", "Disabled", ISS_ON);
    ...
    IUFillNumberVector(&GuideCCD.RapidGuideDataNP, GuideCCD.RapidGuideDataN, 3, getDeviceName(),
        ...
        if (IUGetConfigOnSwitchName(getDeviceName(), "CCD_CAPTURE_FORMAT", configName, MAXHYDROGENNAME) == 0)
        ...
        IUFillSwitch(&FastExposureToggleS[HYDROGEN_ENABLED], "HYDROGEN_ENABLED", "Enabled",
                     m_ConfigFastExposureIndex == HYDROGEN_ENABLED ? ISS_ON : ISS_OFF);
        IUFillSwitch(&FastExposureToggleS[HYDROGEN_DISABLED], "HYDROGEN_DISABLED", "Disabled",
                     m_ConfigFastExposureIndex == HYDROGEN_DISABLED ? ISS_ON : ISS_OFF);
        ...
        // Load from config
        char telescope[MAXHYDROGENDEVICE] = {"Telescope Simulator"};
        IUGetConfigText(getDeviceName(), "ACTIVE_DEVICES", "ACTIVE_TELESCOPE", telescope, MAXHYDROGENDEVICE);
        char rotator[MAXHYDROGENDEVICE] = {"Rotator Simulator"};
        IUGetConfigText(getDeviceName(), "ACTIVE_DEVICES", "ACTIVE_ROTATOR", rotator, MAXHYDROGENDEVICE);
        char focuser[MAXHYDROGENDEVICE] = {"Focuser Simulator"};
        IUGetConfigText(getDeviceName(), "ACTIVE_DEVICES", "ACTIVE_FOCUSER", focuser, MAXHYDROGENDEVICE);
        char filter[MAXHYDROGENDEVICE] = {"CCD Simulator"};
        IUGetConfigText(getDeviceName(), "ACTIVE_DEVICES", "ACTIVE_FILTER", filter, MAXHYDROGENDEVICE);
        char skyquality[MAXHYDROGENDEVICE] = {"SQM"};
        IUGetConfigText(getDeviceName(), "ACTIVE_DEVICES", "ACTIVE_SKYQUALITY", skyquality, MAXHYDROGENDEVICE);
    ...
    bool CCD::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
    {
        ...
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
            ...
            if (!strcmp(name, ActiveDeviceTP.name))
            {
                ...
                strncpy(EqNP.device, ActiveDeviceT[ACTIVE_TELESCOPE].text, MAXHYDROGENDEVICE);
                strncpy(J2000EqNP.device, ActiveDeviceT[ACTIVE_TELESCOPE].text, MAXHYDROGENDEVICE);
    ...
    bool CCD::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
    {
        ...
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
            if (!strcmp(name, "CCD_EXPOSURE"))
            {
                ...
                // if (PrimaryCCD.ImageExposureNP.s == IPS_BUSY && FastExposureToggleS[HYDROGEN_DISABLED].s == ISS_ON)
    ...
    bool CCD::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
    {
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
            ...
            if (!strcmp(name, FastExposureToggleSP.name))
            {
                ...
                if (FastExposureToggleSP.s == IPS_IDLE && FastExposureToggleS[HYDROGEN_ENABLED].s == ISS_ON)
                    LOG_WARN("Experimental Feature: After a frame is downloaded, the next frame capture immediately starts to avoid any delays.");

                if (FastExposureToggleS[HYDROGEN_DISABLED].s == ISS_ON)
                {
            ...
            if (strcmp(name, PrimaryCCD.CompressSP.name) == 0)
            {
                ...
                PrimaryCCD.SendCompressed = PrimaryCCD.CompressS[HYDROGEN_ENABLED].s == ISS_ON;
                return true;
            }
            ...
            if (strcmp(name, GuideCCD.CompressSP.name) == 0)
            {
                ...
                GuideCCD.SendCompressed = GuideCCD.CompressS[HYDROGEN_ENABLED].s == ISS_ON;
                return true;
            }
    ...
    {
        char dev_name[MAXHYDROGENNAME] = {0};
        ...
        strncpy(dev_name, getDeviceName(), MAXHYDROGENNAME);
        ...
        if (targetChip->getFrameType() == CCDChip::LIGHT_FRAME && !std::isnan(J2000RA) && !std::isnan(J2000DE))
        {
            if (!std::isnan(Latitude) && !std::isnan(Longitude))
            {
                ...
                // Convert from JNow to J2000
                HYDROGEN::J2000toObserved(&J2000Pos, ln_get_julian_from_sys(), &epochPos);
        ...
        fitsKeywords.push_back(FITSRecord("Generated by HYDROGEN"));
    }
    ...
    bool CCD::ExposureCompletePrivate(CCDChip *targetChip)
    {
        ...
        strncpy(exposureStartTime, targetChip->getExposureStartTime(), MAXHYDROGENNAME);
        ...
        if (FastExposureToggleS[HYDROGEN_ENABLED].s != ISS_ON)
                         ...
                         bool saveImage)
    {
        ...
        if (saveImage)
        {
            ...
            snprintf(targetChip->FitsB.format, MAXHYDROGENBLOBFMT, ".%s", targetChip->getImageExtension());
        ...
        if (targetChip->SendCompressed && EncodeFormatSP[FORMAT_XISF].getState() != ISS_ON)
        {
            if (EncodeFormatSP[FORMAT_FITS].getState() == ISS_ON && !strcmp(targetChip->getImageExtension(), "fits"))
            {
                ...
                snprintf(targetChip->FitsB.format, MAXHYDROGENBLOBFMT, ".%s.fz", targetChip->getImageExtension());
            }
            else
            {
                ...
                targetChip->FitsB.blob = compressedData;
                targetChip->FitsB.bloblen = compressedBytes;
                snprintf(targetChip->FitsB.format, MAXHYDROGENBLOBFMT, ".%s.z", targetChip->getImageExtension());
            }
        }
        else
        {
    ...
    void CCD::GuideComplete(HYDROGEN_EQ_AXIS axis)
    {
