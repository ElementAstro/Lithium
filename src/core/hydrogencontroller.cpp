...
namespace HYDROGEN
{
                ...
                for (int i = 0; i < JoystickSettingTP.ntp; i++)
                {
                    ...
                    if (tp)
                    {
                        ...
                        if (cType != myType)
                        {
                            ...
                            DEBUGFDEVICE(dev, HYDROGEN::Logger::DBG_ERROR, "Cannot change controller type to %s.", texts[i]);
                            return false;
                        }
                    }
                }
    ...
    {
        HYDROGEN_UNUSED(button_n);
        HYDROGEN_UNUSED(button_value);
        HYDROGEN_UNUSED(context);
    }
