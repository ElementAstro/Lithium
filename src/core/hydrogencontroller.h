...
namespace HYDROGEN
{
      ...
      *
      * - To use the class in an HYDROGEN::DefaultDevice based driver:
      *
      ...
      *       For example:
      *       \code{.cpp}
      *       mapController("ABORT", "Abort Telescope", HYDROGEN::Controller::CONTROLLER_BUTTON, "BUTTON_1");
      *       \endcode
      ...
      *
      * The class communicates with HYDROGEN joystick driver which in turn enumerates the game pad and provides
      ...
      * \warning Both the indi_joystick driver and the driver using this class must be running in the same HYDROGEN server
      *         (or chained HYDROGEN servers) in order for it to work as it depends on snooping among drivers.
      * \author Jasem Multaq
      */
     class Controller
     {
     public:
           ...
           * @param cdevice HYDROGEN::DefaultDevice device
           */
          Controller(HYDROGEN::DefaultDevice *cdevice);
          virtual ~Controller();
