...
namespace HYDROGEN
{
    ...
    bool Dome::initProperties()
    {
        ...
        //    IUFillSwitch(&AutoParkS[0], "HYDROGEN_ENABLED", "Enable", ISS_OFF);
        //    IUFillSwitch(&AutoParkS[1], "HYDROGEN_DISABLED", "Disable", ISS_ON);
        ...
        IUFillSwitch(&DomeBacklashS[HYDROGEN_ENABLED], "HYDROGEN_ENABLED", "Enabled", ISS_OFF);
        IUFillSwitch(&DomeBacklashS[HYDROGEN_DISABLED], "HYDROGEN_DISABLED", "Disabled", ISS_ON);
    ...
    bool Dome::ISNewNumber(const char *dev, const char *name, double values[], char *names[], int n)
    {
        ...
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
            ...
            {
                if (DomeBacklashS[HYDROGEN_ENABLED].s != ISS_ON)
                {
    ...
    bool Dome::ISNewSwitch(const char *dev, const char *name, ISState *states, char *names[], int n)
    {
        ...
        else if (!strcmp(name, AutoParkSP.name))
        {
            ...
            else if (strcmp(name, DomeBacklashSP.name) == 0)
            {
                ...
                const bool enabled = IUFindOnSwitchIndex(&DomeBacklashSP) == HYDROGEN_ENABLED;
    ...
    bool Dome::ISSnoopDevice(XMLEle *root)
    {
        ...
        if (!strcmp("EQUATORIAL_EOD_COORD", propName))
        {
            ...
            {
                // Do not spam log
                if (std::fabs(mountEquatorialCoords.rightascension - ra) > 0.01 || std::fabs(mountEquatorialCoords.declination - de) > 0.01)
                {
    ...
    */
    ...
    void Dome::SetParkDataType(Dome::DomeParkData type)
    {
        ...
        {
        case PARK_NONE:
            strncpy(DomeMotionS[DOME_CW].label, "Open", MAXHYDROGENLABEL);
            strncpy(DomeMotionS[DOME_CCW].label, "Close", MAXHYDROGENLABEL);
            break;
