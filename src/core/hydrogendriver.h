...
extern "C"
{
     ...
     * <ol>
     * <li>HYDROGENCONFIG environment variable: The functions checks if the envrionment variable is defined, and if so, it shall
     ...
     * </ol>
     * @author Jasem Mutlaq
     * @note Drivers subclassing HYDROGEN::DefaultDevice do not need to call the configuration functions directly as it is handled internally by the class.
     * @version libindi 1.1+
     */
     ...
     *  @param dev device name. This is used if the filename parameter is NULL, and HYDROGENCONFIG environment variable is not set as described in the <b>Detailed Description</b> introduction.
     ...
     *  @param dev device name. This is used if either the source_config or desg_config are NULL, and HYDROGENCONFIG environment variable is not set as described in the <b>Detailed Description</b> introduction.
     ...
     *  A configuration file root XML element is \<HYDROGENDriver\>. This functions add \<HYDROGENDriver\> or \</HYDROGENDriver\> as required.
     *  @param fp file pointer to a configuration file.
     *  @param ctag If 0, \<HYDROGENDriver\> is appened to the configuration file, otherwise \</HYDROGENDriver\> is appeneded and the <i>fp</i> is closed.
     ...
     *  @warning This call is not HYDROGEN protocol compliant. It sends setNumberVector along with updated Min/Max/Step values so that the client
     *  updates the range accordingly for this property. In the HYDROGEN-compliant paradigm, it is NOT possible to update min/max/step step of an existing number
     ...
     *  HYDROGEN Library defines this function to simplify the update process without requiring a complete delete/define cycle.
     */
