...
#include "hydrogenfilterinterface.h"
#include <cstring>
...
namespace HYDROGEN
{
    ...
    bool FilterInterface::processText(const char *dev, const char *name, char *texts[], char *names[], int n)
    {
        if (dev && !strcmp(dev, m_defaultDevice->getDeviceName()) && !strcmp(name, "FILTER_NAME"))
        {
            ...
            if (loadingFromConfig)
            {
                ...
                char filterName[MAXHYDROGENNAME];
                char filterLabel[MAXHYDROGENLABEL];
                ...
                for (int i = 0; i < n; i++)
                {
                    snprintf(filterName, MAXHYDROGENNAME, "FILTER_SLOT_NAME_%d", i + 1);
                    snprintf(filterLabel, MAXHYDROGENLABEL, "Filter#%d", i + 1);
    ...
    bool FilterInterface::loadFilterNames()
    {
        ...
        char filterName[MAXHYDROGENNAME] = {0};
        char errmsg[MAXRBUF];
        LilXML *lp = newLilXML();
        int nelem = 0;
        ...
        for (root = nextXMLEle(fproot, 1); root != nullptr; root = nextXMLEle(fproot, 0))
        {
            ...
            if (!strcmp("FILTER_NAME", rname))
            {
                ...
                for (oneText = nextXMLEle(root, 1); oneText != nullptr; oneText = nextXMLEle(root, 0))
                {
                    ...
                    snprintf(filterName, MAXHYDROGENNAME, "FILTER_SLOT_NAME_%d", counter + 1);
