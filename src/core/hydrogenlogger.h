/*******************************************************************************
 ...
 Adapted to HYDROGEN Library by Jasem Mutlaq & Geehalel.
...
*/
#define DEBUG(priority, msg) HYDROGEN::Logger::getInstance().print(getDeviceName(), priority, __FILE__, __LINE__, msg)
#define DEBUGF(priority, msg, ...) \
    HYDROGEN::Logger::getInstance().print(getDeviceName(), priority, __FILE__, __LINE__, msg, __VA_ARGS__)

#define DEBUGDEVICE(device, priority, msg) HYDROGEN::Logger::getInstance().print(device, priority, __FILE__, __LINE__, msg)
#define DEBUGFDEVICE(device, priority, msg, ...) \
    HYDROGEN::Logger::getInstance().print(device, priority, __FILE__, __LINE__, msg, __VA_ARGS__)
 ...
 */
#define LOG_ERROR(txt) DEBUG(HYDROGEN::Logger::DBG_ERROR, (txt))
#define LOG_WARN(txt) DEBUG(HYDROGEN::Logger::DBG_WARNING, (txt))
#define LOG_INFO(txt) DEBUG(HYDROGEN::Logger::DBG_SESSION, (txt))
#define LOG_DEBUG(txt) DEBUG(HYDROGEN::Logger::DBG_DEBUG, (txt))
#define LOG_EXTRA1(txt) DEBUG(HYDROGEN::Logger::DBG_EXTRA_1, (txt))
#define LOG_EXTRA2(txt) DEBUG(HYDROGEN::Logger::DBG_EXTRA_2, (txt))
#define LOG_EXTRA3(txt) DEBUG(HYDROGEN::Logger::DBG_EXTRA_3, (txt))

#define LOGF_ERROR(fmt, ...) DEBUGF(HYDROGEN::Logger::DBG_ERROR, (fmt), __VA_ARGS__)
#define LOGF_WARN(fmt, ...) DEBUGF(HYDROGEN::Logger::DBG_WARNING, (fmt), __VA_ARGS__)
#define LOGF_INFO(fmt, ...) DEBUGF(HYDROGEN::Logger::DBG_SESSION, (fmt), __VA_ARGS__)
#define LOGF_DEBUG(fmt, ...) DEBUGF(HYDROGEN::Logger::DBG_DEBUG, (fmt), __VA_ARGS__)
#define LOGF_EXTRA1(fmt, ...) DEBUGF(HYDROGEN::Logger::DBG_EXTRA_1, (fmt), __VA_ARGS__)
#define LOGF_EXTRA2(fmt, ...) DEBUGF(HYDROGEN::Logger::DBG_EXTRA_2, (fmt), __VA_ARGS__)
#define LOGF_EXTRA3(fmt, ...) DEBUGF(HYDROGEN::Logger::DBG_EXTRA_3, (fmt), __VA_ARGS__)

namespace HYDROGEN
{
    /**
     * @class HYDROGEN::Logger
     * @brief The Logger class is a simple logger to log messages to file and HYDROGEN clients. This is the implementation of a simple
     ...
     *      -# Errors: Use macro DEBUG(HYDROGEN::Logger::DBG_ERROR, "My Error Message)
     *
     *      -# Warnings: Use macro DEBUG(HYDROGEN::Logger::DBG_WARNING, "My Warning Message)
     *
     *      -# Session: Use macro DEBUG(HYDROGEN::Logger::DBG_SESSION, "My Message) Session messages are the regular status messages from the driver.
     *
     *      -# Driver Debug: Use macro DEBUG(HYDROGEN::Logger::DBG_DEBUG, "My Driver Debug Message)
     *
     * @note Use DEBUGF macro if you have a variable list message. e.g. DEBUGF(HYDROGEN::Logger::DBG_SESSION, "Hello %s!", "There")
     *
     ...
     *
     * Check HYDROGEN Tutorial two for an example simple implementation.
     */
    class Logger
    {
        ...
        static HYDROGEN::DefaultDevice *parentDevice;
    ...
    public:
        ...
        static bool initProperties(HYDROGEN::DefaultDevice *device);
        static bool updateProperties(bool enable);
        static char Tags[nlevels][MAXHYDROGENNAME];
