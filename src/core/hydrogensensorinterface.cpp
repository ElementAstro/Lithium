...
namespace HYDROGEN
{
    ...
    SensorInterface::SensorInterface()
    {
        ...
        strncpy(integrationExtention, "raw", MAXHYDROGENBLOBFMT);
    }
    ...
    bool SensorInterface::processText(const char *dev, const char *name, char *texts[], char *names[], int n)
    {
        ...
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
            ...
            if (!strcmp(name, ActiveDeviceTP.name))
            {
                ...
                strncpy(EqNP.device, ActiveDeviceT[0].text, MAXHYDROGENDEVICE);
                strncpy(LocationNP.device, ActiveDeviceT[0].text, MAXHYDROGENDEVICE);
                strncpy(ScopeParametersNP.device, ActiveDeviceT[0].text, MAXHYDROGENDEVICE);
    ...
    {
        HYDROGEN::DefaultDevice::initProperties(); //  let the base class flesh in what it wants
    ...
    {
        strncpy(integrationExtention, ext, MAXHYDROGENBLOBFMT);
        if (HasDSP())
    ...
    void SensorInterface::addFITSKeywords(fitsfile *fptr, uint8_t *buf, int len)
    {
        ...
        // SENSOR
        strncpy(fitsString, getDeviceName(), MAXHYDROGENDEVICE);
        fits_update_key_s(fptr, TSTRING, "INSTRUME", fitsString, "Sensor Name", &status);

        // Telescope
        strncpy(fitsString, ActiveDeviceT[0].text, MAXHYDROGENDEVICE);
        fits_update_key_s(fptr, TSTRING, "TELESCOP", fitsString, "Telescope name", &status);

        // Observer
        strncpy(fitsString, FITSHeaderT[FITS_OBSERVER].text, MAXHYDROGENDEVICE);
        fits_update_key_s(fptr, TSTRING, "OBSERVER", fitsString, "Observer name", &status);

        // Object
        strncpy(fitsString, FITSHeaderT[FITS_OBJECT].text, MAXHYDROGENDEVICE);
        ...
        if (Latitude != -1000 && Longitude != -1000 && Elevation != -1000)
        {
            ...
            snprintf(el_str, MAXHYDROGENFORMAT, "%lf", Elevation);
                                     ...
                                     bool saveIntegration)
    {
        ...
        snprintf(FitsB.format, MAXHYDROGENBLOBFMT, ".%s", getIntegrationFileExtension());
        if (saveIntegration)
        {
    ...
    {
        HYDROGEN::DefaultDevice::saveConfigItems(fp);
