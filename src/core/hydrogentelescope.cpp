...
namespace HYDROGEN
{
    ...
    bool Telescope::ISSnoopDevice(XMLEle *root)
    {
        ...
        if (isConnected())
        {
            ...
            else if (HasTime() && !strcmp(propName, "TIME_UTC"))
            {
                ...
                char utc[MAXHYDROGENTSTAMP], offset[MAXHYDROGENTSTAMP];
                ...
                for (ep = nextXMLEle(root, 1); ep != nullptr; ep = nextXMLEle(root, 0))
                {
                    ...
                    if (!strcmp(elemName, "UTC"))
                        strncpy(utc, pcdataXMLEle(ep), MAXHYDROGENTSTAMP);
                    else if (!strcmp(elemName, "OFFSET"))
                        strncpy(offset, pcdataXMLEle(ep), MAXHYDROGENTSTAMP);
                }
            ...
            else if (!strcmp(propName, "DOME_PARK") /* || !strcmp(propName, "DOME_SHUTTER")*/)
            {
                ...
                // Mount shouldn't park due to dome closing in HYDROGEN::Telescope
#if 0
            if (strcmp(findXMLAttValu(root, "state"), "Ok"))
            {
    ...
    bool Telescope::MoveNS(HYDROGEN_DIR_NS dir, TelescopeMotionCommand command)
    {
        HYDROGEN_UNUSED(dir);
        HYDROGEN_UNUSED(command);
    ...
    bool Telescope::MoveWE(HYDROGEN_DIR_WE dir, TelescopeMotionCommand command)
    {
        HYDROGEN_UNUSED(dir);
        HYDROGEN_UNUSED(command);
    ...
    ***************************************************************************************/
    ...
    {
        TrackModeS = (TrackModeS == nullptr) ? static_cast<ISwitch *>(malloc(sizeof(ISwitch))) : static_cast<ISwitch *>(realloc(TrackModeS, (TrackModeSP.nsp + 1) * sizeof(ISwitch)));
    ...
    void Telescope::updateObserverLocation(double latitude, double longitude, double elevation)
    {
        ...
        m_Location.latitude = latitude;
        m_Location.elevation = elevation;
        char lat_str[MAXHYDROGENFORMAT] = {0}, lng_str[MAXHYDROGENFORMAT] = {0};

        // Make display longitude to be in the standard 0 to +180 East, and 0 to -180 West.
        // No need to confuse new users with HYDROGEN format.
    ...
    {
        HYDROGEN_UNUSED(Axis1Value);
        HYDROGEN_UNUSED(Axis2Value);
        return true;
    }
    ...
    void Telescope::SetTelescopeCapability(uint32_t cap, uint8_t slewRateCount)
    {
        ...
        if (nSlewRate >= 4)
        {
            ...
            //        strncpy((SlewRateS + (step * 0))->name, "SLEW_GUIDE", MAXHYDROGENNAME);
            //        strncpy((SlewRateS + (step * 1))->name, "SLEW_CENTERING", MAXHYDROGENNAME);
            //        strncpy((SlewRateS + (step * 2))->name, "SLEW_FIND", MAXHYDROGENNAME);
            //        strncpy((SlewRateS + (nSlewRate - 1))->name, "SLEW_MAX", MAXHYDROGENNAME);

            // If number of slew rate is EXACTLY 4, then let's use common labels
            if (nSlewRate == 4)
            {
                strncpy((SlewRateS + (0))->label, "Guide", MAXHYDROGENLABEL);
                strncpy((SlewRateS + (1))->label, "Centering", MAXHYDROGENLABEL);
                strncpy((SlewRateS + (2))->label, "Find", MAXHYDROGENLABEL);
                strncpy((SlewRateS + (3))->label, "Max", MAXHYDROGENLABEL);
            }
    ...
    Telescope::TelescopePierSide Telescope::expectedPierSide(double ra)
    {
        ...
        if (!HasPierSide() && !HasPierSideSimulation())
            return HYDROGEN::Telescope::PIER_UNKNOWN;
        ...
        return hourAngle <= 0 ? HYDROGEN::Telescope::PIER_WEST : HYDROGEN::Telescope::PIER_EAST;
    }
