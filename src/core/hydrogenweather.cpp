...
namespace HYDROGEN
{
    ...
    bool HYDROGEN::Weather::ISNewText(const char *dev, const char *name, char *texts[], char *names[], int n)
    {
        //  first check if it's for our device
        if (dev != nullptr && strcmp(dev, getDeviceName()) == 0)
        {
    ...
    bool HYDROGEN::Weather::ISSnoopDevice(XMLEle *root)
    {
        XMLEle *ep = nullptr;
