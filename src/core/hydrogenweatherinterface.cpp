...
namespace HYDROGEN
{
                                        ...
                                        double percWarning)
    {
        ...
        ParametersN = (ParametersN == nullptr) ? static_cast<INumber *>(malloc(sizeof(INumber))) : static_cast<INumber *>(realloc(ParametersN, (ParametersNP.nnp + 1) * sizeof(INumber)));
    ...
    bool WeatherInterface::setCriticalParameter(std::string param)
    {
        for (int i = 0; i < ParametersNP.nnp; i++)
        {
            ...
            {
                critialParametersL =
                    (critialParametersL == nullptr) ? static_cast<ILight *>(malloc(sizeof(ILight))) : static_cast<ILight *>(realloc(critialParametersL, (critialParametersLP.nlp + 1) * sizeof(ILight)));
    ...
    IPState WeatherInterface::checkParameterState(const INumber &parameter) const
    {
        ...
        else if (((parameter.value < (parameter.min + rangeWarn)) && parameter.min != 0) || ((parameter.value > (parameter.max - rangeWarn)) && parameter.max != 0))
        {
            return IPS_BUSY;
        }
        else
    ...
    {
        ParametersRangeNP =
            (ParametersRangeNP == nullptr) ? static_cast<INumberVectorProperty *>(malloc(sizeof(INumberVectorProperty))) : static_cast<INumberVectorProperty *>(realloc(ParametersRangeNP, (nRanges + 1) * sizeof(INumberVectorProperty)));
        ...
        char propName[MAXHYDROGENNAME];
        char propLabel[MAXHYDROGENLABEL];
        snprintf(propName, MAXHYDROGENNAME, "%s", name.c_str());
        snprintf(propLabel, MAXHYDROGENLABEL, "%s", label.c_str());
