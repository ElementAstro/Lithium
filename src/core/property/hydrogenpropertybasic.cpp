...
#include "hydrogenpropertybasic.h"
...
namespace HYDROGEN
{
    ...
    {
        D_PTR(PropertyBasic);
        return static_cast<PropertyView<T> *>(static_cast<HYDROGEN::PropertyPrivate *>(d)->property);
    }

    template <typename T>
    HYDROGEN::PropertyView<T> PropertyBasic<T>::operator*()
    {
        D_PTR(PropertyBasic);
        return *static_cast<PropertyView<T> *>(static_cast<HYDROGEN::PropertyPrivate *>(d)->property);
    }
