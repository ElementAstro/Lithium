 ...
 *
 * I noticed one odd thing after updating my kdelibs, the LEDs don't change color when state is changed. Try that by starting any HYDROGEN device, and hit connect, if the LED turns to yellow and back to grey then it works fine, otherwise, we've got a problem.
 *
 * CCMAIL: kstars-devel@kde.org
 *
            ...
            { // GRGRGR Row
                ...
                { // Over Green
                    ...
                    { // EDGE
                        if (col == 0)
                        {
                            dst[(row * width + col) * 3 + RED] = src[row * width + col + 1];
                        }
                        if (col == WIDTH - 1)
                        {
                            dst[(row * width + col) * 3 + RED] = src[row * width + col - 1];
                    ...
                    { // EDGE
                        if (row == 0)
                        {
                            dst[(row * width + col) * 3 + BLUE] = src[(row + 1) * width + col];
                        }
                        if (row == WIDTH - 1)
                        {
                            dst[(row * width + col) * 3 + BLUE] = src[(row - 1) * width + col];
                ...
                { // Over RED
                    ...
                    {                                                                                                                                                                                          // NORMAL (Left side and bottom count as normal
                    ...
                    { // EDGE
                        ...
                        if (col == 1 && row != 0)
                        { // Unnecessary Left here to explain why
