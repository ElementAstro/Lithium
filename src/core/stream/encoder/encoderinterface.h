...
namespace HYDROGEN
{
    ...
    public:
        ...
        virtual void init(HYDROGEN::DefaultDevice *mainDevice);

        virtual bool setPixelFormat(HYDROGEN_PIXEL_FORMAT pixelFormat, uint8_t pixelDepth);

        virtual bool setSize(uint16_t width, uint16_t height);

        virtual bool upload(HYDROGEN::WidgetViewBlob *bp, const uint8_t *buffer, uint32_t nbytes, bool isCompressed = false) = 0;

        const char *getName();

    protected:
        HYDROGEN::DefaultDevice *currentDevice;
        const char *name;
        HYDROGEN_PIXEL_FORMAT pixelFormat; // HYDROGEN Pixel Format
