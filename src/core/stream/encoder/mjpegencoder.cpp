...
namespace HYDROGEN
{
    ...
    bool MJPEGEncoder::upload(HYDROGEN::WidgetViewBlob *bp, const uint8_t *buffer, uint32_t nbytes, bool isCompressed)
    {
        ...
        HYDROGEN_UNUSED(nbytes);
        int bufsize = rawWidth * rawHeight * ((pixelFormat == HYDROGEN_RGB) ? 3 : 1);
