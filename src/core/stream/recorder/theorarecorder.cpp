...
namespace HYDROGEN
{
    ...
    bool TheoraRecorder::setPixelFormat(HYDROGEN_PIXEL_FORMAT pixelFormat, uint8_t pixelDepth)
    {
...
#if 0
        if (m_PixelFormat == HYDROGEN_MONO)
        {
            ycbcr[1].width = 0;
            ycbcr[1].stride = 0;
            ycbcr[1].height = 0;
...
#if 0
        ycbcr[0].data = new uint8_t[ycbcr[0].stride * ycbcr[0].height];
        ycbcr[1].data = (m_PixelFormat == HYDROGEN_MONO) ? 0 : new uint8_t[ycbcr[1].stride * ycbcr[1].height];
        ycbcr[2].data = (m_PixelFormat == HYDROGEN_MONO) ? 0 : new uint8_t[ycbcr[2].stride * ycbcr[2].height];
#endif
        }
