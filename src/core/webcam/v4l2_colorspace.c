...
void linearize(float *buf, unsigned int len, struct v4l2_format *fmt)
{
    ...
    case V4L2_COLORSPACE_SRGB:
        ...
        for (i = 0; i < len; i++)
        {
            *src = (*src < -0.04045) ? -pow((-*src + 0.055) / 1.055, 2.4) : ((*src <= 0.04045) ? *src / 12.92 : pow((*src + 0.055) / 1.055, 2.4));
            src++;
        }
        break;
    ...
    default:
        ...
        for (i = 0; i < len; i++)
        {
            *src = (*src <= -0.081) ? -pow((*src - 0.099) / -1.099, 1.0 / 0.45) : ((*src < 0.081) ? *src / 4.5 : pow((*src + 0.099) / 1.099, 1.0 / 0.45));
            src++;
        }
    }
}
