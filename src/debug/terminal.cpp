...
const std::string &CommandManager::runCommand(const std::string &cmd,
    ...
    static const std::string unknown_cmd =
        "\033[31mUnknown command: " + cmd + "\033[0m\n";
    return unknown_cmd;
}

std::future<std::string> CommandManager::runCommandAsync(
    const std::string &cmd, const std::string &arg) {
    std::lock_guard<std::mutex> lock(mutex_);
    auto it = commands_.find(cmd);
    if (it != commands_.end()) {
        std::future<std::string> future =
            std::async(std::launch::async, [this, it, arg]() {
...
const std::string &CommandManager::getPrevCommand() {
    ...
    return (history_index_ >= 0 && history_index_ < command_history_.size())
               ? *(hist_iter_)
               : empty_str;
}
...
#endif
                {
                    case 'B':  // 向上键
                        if (manager.hasNextCommand()) {
                            ...
                            std::cout << "\r\033[K" << "\033[94m>>>\033[0m "
                                      << input
                                      << std::flush;  // 输出命令和提示符
                        }
                        break;
...
void clearTerminal() {
#ifdef _WIN32
    // Windows 平台下使用 System 函数
    std::system("cls");
#else
    // Linux 平台下使用 system 函数
    std::system("clear");
#endif
}
...
std::string systemCommand(const std::string &arg) {
    std::string command = SHELL_COMMAND + "'" + arg + "' 2>&1";
    std::string output;
