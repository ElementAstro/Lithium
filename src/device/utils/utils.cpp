...
std::string executeCommand(const std::string &cmd) {
    ...
    if (!CreateProcess(nullptr, const_cast<char *>(cmd.c_str()), nullptr,
    ...
    DWORD bytesRead;
    while (ReadFile(pipeOutRead, buffer.data(), buffer.size(), &bytesRead,
                    nullptr)) {
        if (bytesRead == 0) {
            break;
        }
...
std::string executeCommand(const std::string &cmd) {
    ...
    if (childPid == -1) {
        ...
        if (dup2(pipeOut[1], STDOUT_FILENO) == -1 ||
            dup2(pipeOut[1], STDERR_FILENO) == -1) {
        ...
        ssize_t bytesRead;
        while ((bytesRead = read(pipeOut[0], buffer.data(), buffer.size())) >
               0) {
            result.append(buffer.data(), bytesRead);
        }
