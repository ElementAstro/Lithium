...
// replace linux headers with windows headers for indilib
#ifdef _WIN32
...
#ifdef _WIN32
#define INDINAMEDPIPE "\\.\pipe\indiserver"
#else
...
void Fifo::processLine(const char *line)
{
    ...
    for (j = 0; j < n_args; j++)
    {
        ...
        else if (arg[j][0] == 'c')
        {
            ...
            if (verbose)
                spdlog::debug("With config: {}", envConfig);
        }
        else if (arg[j][0] == 's')
        {
            ...
            if (verbose)
                spdlog::info("With skeketon: {}", envSkel);
        }
        else if (arg[j][0] == 'p')
        {
            ...
            if (verbose)
                spdlog::debug("With prefix: {}", envPrefix);
        }
    }
    ...
    {
        if (verbose)
            spdlog::info("FIFO: Starting driver {}", tDriver);
        ...
        for (auto dp : DvrInfo::drivers)
        {
            ...
            if (dp->name == tDriver)
            {
                ...
                if (tName[0] && !dp->isHandlingDevice(tName))
                    continue;
                if (verbose)
                    spdlog::warn("FIFO: Shutting down driver: {}", tDriver);
...
void Fifo::read(void)
{
    ...
    if (rd == -1)
    {
        ...
        spdlog::error("Fifo error: {}", strerror(errno));
        close();
        open();
        return;
    }
...
void Fifo::ioCb(ev::io &, int revents)
{
    if (EV_ERROR & revents)
    {
        ...
        if (sockErrno)
        {
            spdlog::error("Error on fifo: {}", strerror(sockErrno));
            close();
            open();
        }
    }
