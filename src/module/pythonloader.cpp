...
namespace OpenAPT
{
    ...
    bool PyModuleLoader::load_module(const std::string &name)
    {
        ...
        if (it != modules_.end()) {
            spdlog::error("Module {} has already been loaded", name);
            return false;
        }
        ...
        Py_DECREF(py_name);
        if (!py_module) {
            spdlog::error("Failed to load module {}", name);
            PyErr_Print();
            return false;
        }
        modules_[name] = py_module;
        spdlog::info("Loaded module {}", name);
        return true;
    }
    ...
    bool PyModuleLoader::load_local_module(const std::string &path)
    {
        ...
        if (it != modules_.end()) {
            spdlog::info("Module {} has been loaded", module_name);
            return true;
        }
        ...
        spdlog::info("Loaded module {}", module_name);
        return true;
    }
    ...
    bool PyModuleLoader::load_py_module(const std::string &name)
    {
        ...
        if (!py_module) {
            spdlog::error("Failed to load Python module {}", name);
            PyErr_Print();
            Py_XDECREF(py_module);
            return false;
        }
        modules_[name] = py_module;
        spdlog::info("Loaded Python module {}", name);
        return true;
    }
    ...
    bool PyModuleLoader::register_cpp_function(const std::string &module_name, const std::string &function_name, PyObject *(*func)(PyObject *, PyObject *))
    {
        ...
        if (!py_func) {
            spdlog::error("Failed to create Python capsule for C++ function {}", function_name);
            PyErr_Print();
            Py_XDECREF(py_func);
            return false;
        }
        ...
        Py_DECREF(py_func);
        spdlog::info("Registered C++ function {} in module {}", function_name, module_name);
        return true;
    }
    ...
    bool PyModuleLoader::cache_py_module(const std::string &path)
    {
        ...
        if (cache_file.is_open()) {
            ...
            spdlog::error("Failed to open cache file {}.cache", path);
        }

        module_cache_[path] = cache;

        Py_DECREF(py_ast);
        spdlog::info("Cached Python module {}", path);
        return true;
    }
    ...
    void PyModuleLoader::unload_module(const std::string &name)
    {
         auto it = modules_.find(name);
        if (it == modules_.end()) {
            spdlog::error("Failed to find module {}", name);
            return;
        }
        Py_DECREF(it->second);
        modules_.erase(it);
        spdlog::info("Unloaded module {}", name);
    }
    ...
    PyObject *PyModuleLoader::get_function(const std::string &module_name, const std::string &function_name)
    {
        ...
        if (!py_func || !PyCallable_Check(py_func)) {
            spdlog::error("Failed to get function {} from module {}", function_name, module_name);
            Py_XDECREF(py_func);
            return nullptr;
        }
    ...
    bool PyModuleLoader::check_function(const std::string &module_name, const std::string &function_name)
    {
        ...
        bool result = func_obj && PyCallable_Check(func_obj);

        if (result) {
            spdlog::debug("Function {} exists in module {}", function_name, module_name);
        } else {
            spdlog::error("Function {} does not exist in module {}", function_name, module_name);
        }
