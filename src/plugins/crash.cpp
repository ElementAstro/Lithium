    ...
    std::string getSystemInfo() {
        ...
        try {
                ...
                if (host_info(mach_host_self(), HOST_BASIC_INFO, (host_info_t)&hostinfo, &count) == KERN_SUCCESS) {
                    ...
                    ss << "Processor architecture: " << "x" << sizeof(void*)*8 << std::endl;
            ...
            // 获取RAM信息
                ...
                ramInfo = "Memory usage: " + std::to_string((memStatus.ullTotalPhys - memStatus.ullAvailPhys) / 1024 / 1024)
                        + "/" + std::to_string(memStatus.ullTotalPhys / 1024 / 1024)
                        + " MB (" + std::to_string(static_cast<float>(memStatus.dwMemoryLoad)) + "%)";
            #elif defined __linux__
                struct sysinfo memInfo;
                sysinfo(&memInfo);
                ramInfo = "Memory usage: " + std::to_string((memInfo.totalram - memInfo.freeram) / 1024 / 1024)
                        + "/" + std::to_string(memInfo.totalram / 1024 / 1024)
                        + " MB (" + std::to_string(static_cast<float>(memInfo.freeram) / memInfo.totalram * 100) + "%)";
            #elif defined __APPLE__
