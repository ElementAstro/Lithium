...
int, executable : str)", [](pkpy::VM *vm, pkpy::ArgsView args)
                 {
...
void PyScriptManager::InjectSystemModule() {
    ...
    vm->bind(m_systemModule, "get_disk_usage() -> dict",
             ...
             [](pkpy::VM *vm, pkpy::ArgsView args) {
                 ...
                 for (auto &item : disk_usage) {
                     ...
                     d.set(pkpy::py_var(vm, item.first),
                           pkpy::py_var(vm, item.second));
                 }
    ...
    vm->bind(m_systemModule, "shutdown() -> bool", "shutdown the system",
             [](pkpy::VM *vm, pkpy::ArgsView args) {
                 Atom::System::Shutdown();
                 return pkpy::py_var(vm, true);
             });

    vm->bind(m_systemModule, "reboot() -> bool", "reboot the system",
             [](pkpy::VM *vm, pkpy::ArgsView args) {
                 Atom::System::Reboot();
                 return pkpy::py_var(vm, true);
             });
