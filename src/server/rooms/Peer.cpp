...
bool Peer::sendPingAsync() {
    ...
    public:
        ...
        Action act() override {
            return oatpp::async::synchronize(
                       m_lock, m_websocket->sendPingAsync(nullptr))
                .next(finish());
        }
    };
...
oatpp::async::CoroutineStarter Peer::onApiError(
    const oatpp::String& errorMessage) {
    ...
    public:
        ...
        Action act() override {
            ...
            return oatpp::async::synchronize(
                       ...
                       std::move(
                           m_websocket->sendOneFrameTextAsync(m_message).next(
...
oatpp::async::CoroutineStarter Peer::validateFilesList(
    const MessageDto::FilesList& filesList) {
    if (filesList->size() == 0)
...
oatpp::async::CoroutineStarter Peer::handleFilesMessage(
    ...
    for (auto& currFile : *files) {
        auto file = m_room->shareFile(m_peerId, currFile->clientFileId,
...
oatpp::async::CoroutineStarter Peer::handleFileChunkMessage(
    const oatpp::Object<MessageDto>& message) {
...
oatpp::async::CoroutineStarter Peer::handleMessage(
    const oatpp::Object<MessageDto>& message) {
    if (!message->code) {
...
oatpp::String Peer::getNickname() { return m_nickname; }

v_int64 Peer::getPeerId() { return m_peerId; }

void Peer::addFile(const std::shared_ptr<File>& file) {
    m_files.push_back(file);
}

const std::list<std::shared_ptr<File>>& Peer::getFiles() { return m_files; }
...
oatpp::async::CoroutineStarter Peer::onPing(
...
oatpp::async::CoroutineStarter Peer::onPong(
    const std::shared_ptr<AsyncWebSocket>& socket,
    const oatpp::String& message) {
    --m_pingPoingCounter;
...
oatpp::async::CoroutineStarter Peer::onClose(
    const std::shared_ptr<AsyncWebSocket>& socket, v_uint16 code,
    const oatpp::String& message) {
    return nullptr;  // do nothing
}

oatpp::async::CoroutineStarter Peer::readMessage(
