...
oatpp::String Room::getName() { return m_name; }
...
void Room::addHistoryMessage(const oatpp::Object<MessageDto>& message) {
    if (!m_appConfig->maxRoomHistoryMessages ||
        *m_appConfig->maxRoomHistoryMessages == 0) {
        return;
    }
...
std::shared_ptr<File> Room::shareFile(v_int64 hostPeerId, v_int64 clientFileId,
    ...
    if (!host)
        throw std::runtime_error("File host not found.");

    v_int64 serverFileId = m_fileIdCounter++;

    auto file = std::make_shared<File>(host, clientFileId, serverFileId,
                                       fileName, fileSize);
    host->addFile(file);
...
bool Room::isEmpty() { return m_peerById.size() == 0; }
