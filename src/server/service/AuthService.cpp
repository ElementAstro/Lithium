...
oatpp::Object<AuthDto> AuthService::signUp(
    const oatpp::Object<SignUpDto>& dto) {
    ...
    auto dbResult = m_database->createUser(userDto);
    if (!dbResult->isSuccess()) {
        OATPP_LOGE("AuthService", "DB-Error: '%s'",
                   dbResult->getErrorMessage()->c_str());
    }
    OATPP_ASSERT_HTTP(dbResult->isSuccess(), Status::CODE_401, "Unauthorized");

    auto result =
        dbResult->fetch<oatpp::Vector<oatpp::Vector<oatpp::String>>>();
...
oatpp::Object<AuthDto> AuthService::signIn(
...
oatpp::Object<StatusDto> AuthService::deleteUserById(
    const oatpp::String& userId) {
    auto dbResult = m_database->deleteUserById(1);
