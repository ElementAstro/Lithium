...
namespace Lithium
{
    ...
    std::optional<std::shared_ptr<SimpleTask>> TaskContainer::getTask(const std::string &name)
    {
        std::lock_guard<std::mutex> lock(mutex);
    ...
    std::vector<std::shared_ptr<SimpleTask>> TaskContainer::findTasks(int priority, bool status)
    {
        std::lock_guard<std::mutex> lock(mutex);
        std::vector<std::shared_ptr<SimpleTask>> result;
        ...
        //              {
        //                  return pair.second->getPriority() == priority && pair.second->getStatus() == status;
        //              });
        return result;
    }

    void TaskContainer::sortTasks(const std::function<bool(const std::shared_ptr<SimpleTask> &, const std::shared_ptr<SimpleTask> &)> &cmp)
    {
        std::lock_guard<std::mutex> lock(mutex);
    ...
    void TaskContainer::batchAddTasks(const std::vector<std::shared_ptr<SimpleTask>> &tasksToAdd)
    {
        std::lock_guard<std::mutex> lock(mutex);
    ...
    void TaskContainer::batchRemoveTasks(const std::vector<std::string> &taskNamesToRemove)
    {
        std::lock_guard<std::mutex> lock(mutex);
    ...
    void TaskContainer::batchModifyTasks(const std::function<void(std::shared_ptr<SimpleTask> &)> &modifyFunc)
    {
        std::lock_guard<std::mutex> lock(mutex);
