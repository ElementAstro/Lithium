 ...
 *
 * @details Provides functionalities to add, retrieve, remove, and manipulate
 * tasks stored within a container.
 *
...
public:
     ...
     * @return std::optional<std::shared_ptr<SimpleTask>> An optional containing
     * the task if found.
     *
     * 通过名称检索任务。
     * @param name 要检索的任务的名称。
     * @return std::optional<std::shared_ptr<SimpleTask>>
     * 如果找到任务，则包含该任务的optional。
     */
     ...
     *
     * @return std::vector<std::shared_ptr<SimpleTask>> A vector containing all
     * tasks.
     *
     * 获取容器中的所有任务。
     ...
     * @return std::vector<std::shared_ptr<SimpleTask>> A vector containing the
     * matching tasks.
     *
     * 查找具有特定优先级和状态的任务。
     ...
     *
     * @param taskNamesToRemove A vector containing the names of the tasks to
     * remove.
     *
     * 通过它们的名称从容器中移除多个任务。
    ...
    /**
     * @brief Modify tasks in the container using a custom modification
     * function.
     *
