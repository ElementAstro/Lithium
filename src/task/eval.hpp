/**
 * @file eval.hpp
 * @brief Python-like eval implementation in C++20.
 *
 * This file provides an implementation of a Python-like `eval` function
 * using C++20 features. It allows for evaluating expressions in a manner
...
#ifndef LITHIUM_TASK_EVAL_HPP
#define LITHIUM_TASK_EVAL_HPP
...
namespace lithium::eval {

class EvalError : public std::runtime_error {
public:
    EvalError(std::string_view expr, std::string_view message)
        : std::runtime_error(std::string(message)), expr_(expr) {}
    [[nodiscard]] auto expression() const noexcept -> std::string_view {
        return expr_;
    }
...
class Eval {
public:
    auto operator()(std::string_view expr) -> T {
        T result = 0;
        index_ = 0;
        expr_ = expr;
        try {
    ...
    auto operator()(char c) -> T { return (*this)(std::string_view(&c, 1)); }
...
private:
    ...
    struct Operator {
        ...
        int precedence;
        char associativity;
        constexpr Operator(OperatorType opr, int prec, char assoc) noexcept
    ...
    struct OperatorValue {
        ...
        constexpr OperatorValue(const Operator &opr, T val) noexcept
            : op(opr), value(val) {}
        [[nodiscard]] constexpr auto getPrecedence() const noexcept -> int {
    ...
    [[nodiscard]] auto checkZero(T value) const -> T {
        if (value == 0) {
            ...
            throw EvalError(expr_, msg.str());
        }
        return value;
    }

    [[nodiscard]] constexpr auto calculate(
        T v1, T v2, const Operator &op) const noexcept -> T {
    ...
    [[nodiscard]] constexpr auto getCharacter() const noexcept -> char {
        return !isEnd() ? expr_[index_] : 0;
    }
    ...
    [[nodiscard]] auto parseOp() -> Operator {
        eatSpaces();
    ...
    [[nodiscard]] static constexpr auto toInteger(char c) noexcept -> T {
        if (c >= '0' && c <= '9') {
            return c - '0';
        }
        if (c >= 'a' && c <= 'f') {
    ...
    [[nodiscard]] constexpr auto getInteger() const noexcept -> T {
        return toInteger(getCharacter());
    }

    [[nodiscard]] auto parseDecimal() -> T {
        T value = 0;
        T fraction = 1;
        bool decimalPoint = false;
        std::size_t length = 0;
        std::size_t maxLength = std::numeric_limits<T>::digits10 + 1;

        while (index_ < expr_.size()) {
            char c = expr_[index_];
            if (std::isdigit(c) != 0) {
                if (length >= maxLength) {
                    throw EvalError(expr_, "Number too large");
                }
                value = value * 10 + (c - '0');
                if (decimalPoint) {
                    fraction *= 10;
                ...
                throw EvalError(expr_, "Multiple decimal points in number");
            } else {
                break;
            }
            index_++;
        }
        return decimalPoint ? value / fraction : value;
    }

    [[nodiscard]] auto parseHex() -> T {
        index_ += 2;
        T value = 0;
        std::size_t length = 0;
        std::size_t maxLength = std::numeric_limits<T>::digits / 4;
    ...
    [[nodiscard]] auto parseValue() -> T {
        ...
        if (std::isalpha(getCharacter())) {
            ...
            if (functions_.count(name) > 0) {
                eatSpaces();
                if (getCharacter() != '(') {
                    throw EvalError(expr_, "Expected '(' after function name");
                }
                index_++;
                T arg = parseExpr();
                if (getCharacter() != ')') {
                    throw EvalError(expr_,
...
template <typename T>
ATOM_INLINE auto eval(std::string_view expression) -> T {
    Eval<T> parser;
    return parser(expression);
}

ATOM_INLINE auto eval(const std::string &expression) -> double {
    return eval<double>(expression);
}

}  // namespace lithium::eval

#endif  // LITHIUM_TASK_EVAL_HPP