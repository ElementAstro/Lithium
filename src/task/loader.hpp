...
namespace Lithium
{
    ...
    public:
         ...
         * @brief 读取JSON文件
         * @param filePath 文件路径
         * @return std::optional<json> 如果文件存在且格式正确，则返回json对象；否则返回std::nullopt
         *
         * Read a JSON file
         * @param filePath Path to the file
         * @return std::optional<json> Returns a json object if the file exists and is correctly formatted; otherwise returns std::nullopt
         */
        ...
        static void asyncReadJsonFile(const std::string &filePath, std::function<void(std::optional<json>)> callback);
        ...
        static void asyncWriteJsonFile(const std::string &filePath, const json &j, std::function<void(bool)> callback);
         ...
         * @param base Base json object, the result of the merge will be stored in this object
         ...
         * @param onComplete Callback function to be called after all files have been processed
         */
        ...
        static void batchAsyncProcess(const std::vector<std::string> &filePaths, std::function<void(std::optional<json>)> process, std::function<void()> onComplete);
        ...
        static void asyncDeleteJsonFile(const std::string &filePath, std::function<void(bool)> callback);
         ...
         * @param callback Callback function to be called upon completion, returning the found value or std::nullopt
         */
        static void asyncQueryJsonValue(const fs::path &filePath, const std::string &key, std::function<void(std::optional<json>)> callback);
        static void asyncQueryJsonValue(const std::string &filePath, const std::string &key, std::function<void(std::optional<json>)> callback);
         ...
         * @param process Processing function to execute on each file
         * @param onComplete Callback function to be called after all files in the directory have been processed
         */
        ...
        static void batchProcessDirectory(const std::string &directoryPath, std::function<void(std::optional<json>)> process, std::function<void()> onComplete);
    };
}
