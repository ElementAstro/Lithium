...
namespace Lithium
{
     ...
     *
     * TaskPool 允许你将任务（即要被工作线程池执行的函数）加入队列。它支持任务窃取，以实现线程间的负载平衡。
     */
    class TaskPool
    {
    private:
    ...
    public:
            ...
            -> std::future<typename std::invoke_result<F, Args...>::type>
        {
            ...
            static_assert(std::is_invocable_v<F, Args...>, "Function must be invocable with arguments");

            auto task = std::make_shared<std::packaged_task<return_type()>>(
                std::bind(std::forward<F>(f), std::forward<Args>(args)...));

            std::future<return_type> res = task->get_future();
            auto wrappedTask = std::make_shared<Task>([task]()
                                                      { (*task)(); });
