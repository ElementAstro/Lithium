...
namespace Lithium
{
    ...
    SingleThreadPool::SingleThreadPool() : d_ptr(std::make_shared<SingleThreadPoolPrivate>()) {}

    SingleThreadPool::~SingleThreadPool() {}

    bool SingleThreadPool::start(const std::function<void(const std::atomic_bool &isAboutToClose)> &functionToRun)
    {
        if (!functionToRun)
            return false;
        ...
        if (std::this_thread::get_id() != d->threads[0].get_id())
            d->released.wait(lock, [d]
                             { return d->pendingFunction == nullptr; });

        return true;
    }

    void SingleThreadPool::startDetach(const std::function<void(const std::atomic_bool &isAboutToClose)> &functionToRun)
    {
        if (!functionToRun)
            return;
    ...
    bool SingleThreadPool::tryStart(const std::function<void(const std::atomic_bool &isAboutToClose)> &functionToRun)
    {
        if (!functionToRun)
            return false;
    ...
    void SingleThreadPool::tryStartDetach(const std::function<void(const std::atomic_bool &isAboutToClose)> &functionToRun)
    {
        if (!functionToRun)
            return;
