...
//      We must keep the status of each connection, and make sure the thread
//      safe We can use oatpp::async::Lock to make sure the thread safe
...
void AsyncWsInstance::sendMessage(const oatpp::String &message) {
    ...
    m_asyncExecutor->execute<SendMessageCoroutine>(&m_writeLock, m_socket,
                                                   message);
}
...
std::shared_ptr<AsyncWsHub> AsyncWsInstance::getHub() { return m_hub; }

oatpp::String AsyncWsInstance::getName() { return m_connection_name; }

v_int32 AsyncWsInstance::getId() { return m_userId; }

oatpp::async::CoroutineStarter AsyncWsInstance::onPing(
    const std::shared_ptr<AsyncWebSocket> &socket,
    const oatpp::String &message) {
    DLOG_F(INFO, "Received ping from {} with message {}",
           m_connection_name->c_str(), message->c_str());
    return oatpp::async::synchronize(&m_writeLock,
                                     socket->sendPongAsync(message));
}

oatpp::async::CoroutineStarter AsyncWsInstance::onPong(
    const std::shared_ptr<AsyncWebSocket> &socket,
    const oatpp::String &message) {
    DLOG_F(INFO, "Received pong from {} with message {}",
           m_connection_name->c_str(), message->c_str());
    return nullptr;  // do nothing
}

oatpp::async::CoroutineStarter AsyncWsInstance::onClose(
    const std::shared_ptr<AsyncWebSocket> &socket, v_uint16 code,
    const oatpp::String &message) {
    DLOG_F(INFO, "Received close from {} with code {} and message {}",
           m_connection_name->c_str(), code, message->c_str());
    return nullptr;  // do nothing
}

oatpp::async::CoroutineStarter AsyncWsInstance::readMessage(
    const std::shared_ptr<AsyncWebSocket> &socket, v_uint8 opcode, p_char8 data,
    oatpp::v_io_size size) {
    if (size == 0) {  // message transfer finished
        auto wholeMessage = m_messageBuffer.toString();
        m_messageBuffer.setCurrentPosition(0);
        DLOG_F(INFO, "Received message from {} with size {}",
               m_connection_name->c_str(), size);

        json res;
        if (!json::accept(wholeMessage->c_str())) {
            RESPONSE_ERROR_C(res, ServerError::InvalidFormat,
                             "Message is not in JSON format");
        } else {
            try {
                ...
                if (jdata.contains("name") && jdata.contains("params")) {
                    ...
                    if (Lithium::MyApp->hasCommand(name)) {
                        res = Lithium::MyApp->DispatchCommand(name,
                                                              jdata["params"]);
                    } else {
                        RESPONSE_ERROR_C(res, ServerError::UnknownCommand,
                                         "Unknown command: " + name);
                    }
                } else {
                    RESPONSE_ERROR_C(res, ServerError::MissingParameters,
                                     "Missing parameter: name or params");
                }
