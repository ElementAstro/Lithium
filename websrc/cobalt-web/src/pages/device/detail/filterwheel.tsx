...
const DeviceFilterDetailedControlPanel = () => {
  const [property_list, set_property_list] = useState([]);

  const process_ws_message = (msg) => {
    // console.log('in filter detail', msg);
    if (msg.device_name === "filter" && msg.instruction === "get_parameter") {
      set_property_list(msg.data);
    }
    if (
      msg.device_name === "filter" &&
...
export default DeviceFilterDetailedControlPanel;
