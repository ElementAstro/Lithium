...
const DeviceFocuserDetailedControlPanel = () => {
  const [property_list, set_property_list] = useState([]);

  const process_ws_message = (msg) => {
    // console.log('in focus detail', msg);
    if (msg.device_name === "focus" && msg.instruction === "get_parameter") {
      set_property_list(msg.data);
    }
    if (
      msg.device_name === "focus" &&
...
export default DeviceFocuserDetailedControlPanel;
