...
export interface IGLobalParametersModels {
  ...
  update_one_paramter: Action <IGLobalParametersModels, Partial<IGlobalParameters>>;
  ...
  get_filter_names_full: Computed<IGLobalParametersModels, IGPFilterSelection[]>;
  get_filter_names_simple: Computed<IGLobalParametersModels, IGPFilterSelection[]>;
}
...
export const GlobalParameterStore = ():IGLobalParametersModels =>({
  ...
  update_one_paramter: action((state, payload) => {
    ...
    state.global_parameter = Object.assign({}, state.global_parameter, {...payload})
  }),
  ...
  set_parameter: thunk(async (actions, payload) => {
    try{
      ...
      const update_res = await AXIOSGP.getGlobalOneParameter(payload.parameter_name);
      let new_setting = Object();
  ...
  get_filter_names_full: computed((state) => {
    ...
    if (state.global_parameter.filter_setting != null){
      for (let i = 0; i< state.global_parameter.filter_setting.filter_info.length; i++){
        ret_struct.push({
          label: state.global_parameter.filter_setting.filter_info[i].filter_name,
          value: i,
        })
      }
      return ret_struct;
  ...
  get_filter_names_simple: computed((state) => {
    ...
    if (state.global_parameter.filter_setting != null){
      for (let i = 0; i< state.global_parameter.filter_setting.filter_info.length; i++){
        ret_struct.push({
          label: state.global_parameter.filter_setting.filter_info[i].filter_name.charAt(0),
          value: i,
        })
      }
      return ret_struct;
